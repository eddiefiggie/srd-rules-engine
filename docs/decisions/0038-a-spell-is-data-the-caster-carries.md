...
| 5 — casting and spending a slot are separate facts | **Built** for Cantrips and for Rituals — `ritual_cast` refuses `at_level` because "a Ritual expends no spell slot", which is p. 187 drawing the consequence itself. Special Abilities and Magic Items are **not built**: [#253](https://github.com/eddiefiggie/srd-rules-engine/issues/253), which needs magic items to exist as something the engine holds |
