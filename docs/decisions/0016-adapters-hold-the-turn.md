...
**Implemented, and complete since 2026-08-24.** `adapters/session.py` and `adapters/mcp.py`, with
the `mcp` extra declared in `pyproject.toml`. Every declared tool now works: `supply_facts` was the
last one that did not, and [#144](https://github.com/eddiefiggie/srd-rules-engine/issues/144) wired it through
`Session.supply_values` — shared with the CLI and HTTP adapters, which had the same hole.

_Issue added 2026-08-24 ([#126](https://github.com/eddiefiggie/srd-rules-engine/issues/126)), closed the same day. The gap
was recorded here and in the adapter's own `NotImplementedError`, which pointed at
[#97](https://github.com/eddiefiggie/srd-rules-engine/issues/97) — closed. Nothing tracked it, which is what #126 was about._
