...
| 5 — hazard state on `Combatant`, not `Conditions` | **Built.** `Hazards`, with `burning` alone at first — Suffocation's field waited on [#178](https://github.com/eddiefiggie/srd-rules-engine/issues/178), and joined it as `suffocating` once that closed gave it a consumer in `core.hazards.suffocation_resolver`. It lives in `core.state` rather than `core.hazards` for the reason `DeathSaves` does: that module imports `core.adjudicate`, which imports this one |
...
| 8 — Dehydration is bookkeeping, Malnutrition is an outcome | **Built** 2026-08-30 ([#315](https://github.com/eddiefiggie/srd-rules-engine/issues/315), [0080](0080-dehydration-is-bookkeeping.md); [#399](https://github.com/eddiefiggie/srd-rules-engine/issues/399), [0081](0081-a-campaign-days-end-is-the-fifth-occasion.md)) — **and the clause was half wrong.** Malnutrition is not *an outcome*: p. 185 states two rules, a DC 10 save for eating too little and an **automatic** level after five days with nothing. The second is bookkeeping exactly like Dehydration, so the two hazards are not opposites and Malnutrition's halves are blocked on different things. Its starvation clause is [#401](https://github.com/eddiefiggie/srd-rules-engine/issues/401) |
