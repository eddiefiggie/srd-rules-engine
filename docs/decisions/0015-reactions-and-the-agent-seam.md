...
| The Reaction offer itself, and the out-of-turn invocation this record says the seam already serves | **Built** 2026-08-30 as `loop.turn.TurnLoop.move` ([#382](https://github.com/eddiefiggie/srd-rules-engine/issues/382), [0072](0072-movement-is-a-phase-the-loop-drives.md)). This row said "unprovable until an offer can be made" for six days; [#381](https://github.com/eddiefiggie/srd-rules-engine/issues/381) made it provable and #382 made it. **The seam needed no change, and the record's other assumption did**: 0015 took for granted that the loop would be present where movement resolved, and `EncounterState.with_movement` had no caller in the engine at all — so movement became a loop-driven phase. `opportunity-attack-detected-but-never-offered` retired with it |
