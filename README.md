...
**Current build:** `08232026.13` — **the effect-shape vocabulary is normalised on mechanism rather than on the feature that exhibits it, and #76 is settled by decision 0013.** 215 shapes become 211: the three recharge shapes merge, because the document defers the mechanism to each feature's own description (pp. 185, 187); the two override shapes merge, because Peerless Aim (p. 88) and Legendary Resistance (p. 258) are one sentence with the test kind changed; and the three reroll shapes merge into `die-replacement`, which is what `replace_die` already exposes as one seam. `ability-score-increase` is admitted, because the exclusion criterion was restated on what state the engine holds rather than on what the adjudication surface happens to touch. Poisons and contagions are **not** inconsistent and both stand — that asymmetry is the document's own. `kind` is now a closed vocabulary guarded in both directions, and the criteria that decide shape from content ship in the data instead of in generator comments.
