...
**Current build:** `08242026.15` — **the sight subsystem's structure is built, and the part that would have been invented refuses instead.** 0025's clauses 2, 3, 4 and 7 land as `core.sight`: light is state on `EncounterState` rather than an argument a caller supplies when an outcome is computed, senses sit on the combatant shaped like `Speeds` with `None` distinct from a range of 0, and visibility is derived from both rather than stored. Clause 5 is the point — `obscurement_at` and `can_see` **raise**, the two mapping tables carry no rows, and a guard fails if a row appears while the verification still says `unverified` ([#150](https://github.com/eddiefiggie/srd-rules-engine/issues/150) is the reading, and nine glossary pages are asserted nowhere in this repository). An unlit encounter reports `None`, not Bright Light — defaulting to daylight would be a rule value nobody supplied, applied invisibly to every roll. **A second guard found a shipped gap while being written**: `situation_payload` is a hand-maintained field list, and 0020's `elapsed_minutes` and `minutes_until_recovery` were on the read surface and on no transport, so an agent driving through any adapter could not see the countdown a Stable creature recovers on. Both now render. No shape resolves — the engine can hold a creature's Darkvision and still cannot say what it does — so coverage stays at 76 of 211. 993 tests.
...
_Last updated: 2026-08-24 — build `08242026.15`._
