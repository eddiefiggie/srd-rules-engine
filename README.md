...
**Current build:** `08252026.32` — **the document disagreeing with itself, and the tiebreak that must not be reached for it.** Two SRD rules have now been found to contradict each other rather than one being absent, and both were re-read in the PDF for [0031](docs/decisions/0031-a-contradiction-in-the-document-is-an-absent-rule.md): p. 236's Potion of Vitality *"removes **any** Exhaustion levels you have"* against p. 181's *"Exhaustion caused by dehydration **can't be removed** until..."* ([#182](https://github.com/eddiefiggie/srd-rules-engine/issues/182)), and — in a **single paragraph** of p. 188 — *"increases or decreases by **an equal amount**"* against *"if your Speed is **halved** ... your Fly Speed is **also halved**"*, which differ the moment two speeds do ([#205](https://github.com/eddiefiggie/srd-rules-engine/issues/205)). Every standing rule here assumed the document was **silent or clear**; this is a third thing. **The dangerous part is that [0030](docs/decisions/0030-an-unanswerable-qualifier-resolves-away-from-invention.md) clause 1 appears to decide both** — resolve away from invention, take the smaller Fly Speed, leave the Exhaustion locked — in one step, from the project's newest and most-reached-for tiebreak. It must not be reached: clause 1 grades the **consequences of a guess**, which is legitimate only when one reading is actually true, and here **both are printed**. 0031 clause 1 makes a contradiction count as R31 silence, so the mechanic is **excluded and disclosed** like a rule the SRD never states; clause 3 prefers **not building** over refusing at runtime, which is where both instances already sit; clause 5 keeps 0030's objection that a mid-ruling refusal hands the choice to the agent (R1). Clause 4 is specified and unreachable — filed as [#209](https://github.com/eddiefiggie/srd-rules-engine/issues/209) rather than left as prose in a record whose gate had closed. Both halves of p. 188's pair are now asserted so they go red **together**, the only way this project can learn the publisher resolved it rather than us. Coverage unchanged at **89 of 211** — clause 3 is a decision to leave two shapes unresolved. Two clauses added (153 total). 1270 tests.
...
_Last updated: 2026-08-25 — build `08252026.32`._
