...
**Current build:** `08232026.15` — **death saving throws, and what damage at 0 hit points costs.** A player character at 0 rolls a DC 10 save tied to no ability score; three of a kind resolves it either way, the marks need not be consecutive, and any healing resets both counts (p. 17). A natural 1 costs two failures and a natural 20 restores a hit point (p. 18) — neither is "the save succeeded", so `Proposal` grew branches keyed on the natural die. Two rules that are easy to get plausibly wrong are modelled as written: **Massive Damage kills on the remainder** after the character reaches 0, not on the whole blow, and **a monster dies instantly** rather than making saves, so `Combatant` now knows which it is. The Unconscious condition, stabilising by the Help action, and the 1d4-hour recovery ship disclosed as absent (#18, #16, #85). Inventory coverage 21 → 23 of 211.
