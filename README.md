...
**Current build:** `08262026.1` — **#228 is misscoped, not wrong: a glossary entry is an index into the rules, not the boundary of one.** #228 read Bright Light's Rules Glossary entry — *"Bright Light is normal illumination"* — saw an entry stating no mechanic, and asked whether `211 of 211` is therefore unreachable, since nothing could ever produce a consequence the entry does not state. **The observation holds. The inference does not.** The entry defines the term and points onward; **p. 11** carries the mechanic, under *Vision and Light → Light*: *"Bright Light lets most creatures see normally."* That is a mechanical statement — Bright Light imposes nothing — and the engine produced it all along: `OBSCUREMENT_BY_LIGHT[BRIGHT]` is `Obscurement.NONE` and `perception_of` says the light obscures nothing. **None of #228's three options is taken**: no vacuous-claim rule, no move to `vocabulary`, and the ceiling is not disclosed as unreachable, because it is reachable. **The rule was already being followed and had never been named.** A sweep of all 155 glossary entries found `healing`, `save`, `damage` and `damage-types` — every one as definitional as Bright Light — already claimed on mechanics stated elsewhere, and `damage-types` is the second instance #228 named. Only the entry nobody noticed had been left out. **The rule is asymmetric, and that clause is load-bearing**: outside text may *supply* a consequence, never *enlarge* the bar — read symmetrically it would unclaim `damage`, `healing` and `save`, and `bright-light` besides, since p. 11 also gives light-source radii this engine does not model. **Seven sites argued the opposite and were corrected**, six of them #228's and one #138's — a sight comment still calling Lightly Obscured *produced by nothing* three lines above the code that claims it. `Obscurement.NONE` is **not** reversed: it really is this engine's word for an absence, and it gains p. 11 as the mechanic's home. Three p. 11 clauses added (170 total), asserting Dim Light and Darkness in **both** places on purpose — the redundancy is what shows the glossary repeats some consequences and not others. Guards proved red three ways, including the counter-direction that stops the rule licensing a claim on anything. Deferred: [#229](https://github.com/eddiefiggie/srd-rules-engine/issues/229), [#230](https://github.com/eddiefiggie/srd-rules-engine/issues/230). Senses and light **6 of 10 → 7 of 10**. **96 of 211.** 1336 tests.
...
**96 of 211 shapes resolve today.** The other 115 are listed, not omitted; run
...
_Last updated: 2026-08-26 — build `08262026.1`._
