...
**Current build:** `08312026.18` — **a readied spell is the reverse of a long cast, on the exact axis [0065](docs/decisions/0065-a-long-cast-spends-its-slot-on-completion.md) settled.** p. 186's Ready looks reusable: `Combatant.long_cast` already models a casting in progress. It is not. 0065's title is *"a long cast spends its slot on **completion**"*, and p. 187 says a readied spell is cast *"as normal (**expending any resources used to cast it**) but hold its energy"* — **paid for at the start, resolved on the trigger**. Opposite where it matters and identical nowhere, so reusing `LongCast` would need a flag that inverts the record. Held energy is also a state this engine lacks — `LongCast` holds an **unfinished** casting; this holds a **finished** one whose effect is suspended — and p. 187 caps its Concentration at *"the start of your next turn"*, a `TurnBoundary` the built Concentration machinery does not attach to one. A break makes the spell *"dissipate without taking effect"*, which is [#224](https://github.com/eddiefiggie/srd-rules-engine/issues/224)'s unbuilt shape. So [#436](https://github.com/eddiefiggie/srd-rules-engine/issues/436), a gate. **The verifier caught a wrong page in the writing of it**: the readied-spell sentence is on p. 187, not p. 186, and a clause that cites the wrong page is the kind of thing that reads correctly forever. **Three gates filed tonight from three pages read whole** — Help, Ready, and the corrected Cover — which is what the remaining surface now looks like. 350 clauses verified against the document, up from 300 this morning. 136 of 210, 14 clauses. 2283 tests.
...
_Last updated: 2026-08-31 — build `08312026.18`._
