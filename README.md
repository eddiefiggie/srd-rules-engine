...
**Current build:** `08312026.24` — **a docstring I wrote this morning was true of the intent and false of the code, and the code was broken.** [#224](https://github.com/eddiefiggie/srd-rules-engine/issues/224) asked what `Proposal` must change to express an outcome that decides a failure and moves nothing. **Nothing**: its guard refuses a proposal with no test *and no outcome*, and an effect in `outcome` satisfies it. The obstacle was never the guard but the assumption that every `EffectKind` is a state transition — which `INFLUENCED` had supposedly already broken. **It had not.** `_apply`'s fallthrough raises, added with #119 because Death used to be the fallback and every kind since would silently have become one — so `INFLUENCED`, in neither a branch nor an exemption, meant **every Influence ruling raised on adjudication** ([#448](https://github.com/eddiefiggie/srd-rules-engine/issues/448)). Its own docstring said *"`_apply` has no branch for it because there is nothing to apply — which is the honest shape, not an omission"*, and the guard does not read docstrings. **Every attitude test asserted the proposal and none applied one**, which is why it shipped green this morning. `RECORDED_ONLY` names both kinds explicitly, and because it is an **exemption from a guard** it gets a guard of its own — a test applying every member and asserting nothing moved, written over the set so a third is checked without anyone remembering to. p. 177's Blinded now **records** its automatic failure instead of refusing, which is the R30 gap #224 was filed for: a review can tell *the observer never looked* from *the observer looked and the rules failed it*. Three corruption proofs. 139 of 210, 14 clauses. 357 clauses verified against the document. 2299 tests.
...
_Last updated: 2026-08-31 — build `08312026.24`._
