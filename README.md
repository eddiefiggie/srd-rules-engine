...
**Current build:** `08292026.23` — **the guard was one file away from the defect it was built for.** [#312](https://github.com/eddiefiggie/srd-rules-engine/issues/312): the **Status rows cite open issues** job reads `docs/decisions/` and had never read `README.md`, on the reasoning that the README is an index rather than a record. The blind spot had an occupant. The **v1.0 — mechanics** cell called p. 89's ammunition recovery *"disclosed and unbuilt"* over **closed** [#301](https://github.com/eddiefiggie/srd-rules-engine/issues/301) — while the recovery had shipped four builds earlier — and named [#273](https://github.com/eddiefiggie/srd-rules-engine/issues/273) and [#289](https://github.com/eddiefiggie/srd-rules-engine/issues/289) as remaining blockers in the same cell that calls their properties complete. Three claims reading as finished work and absent work at once, which `AGENTS.md` calls worse than unfiled. **Pointing the old guard at README does not work, and that is the build.** A record's row is one clause; README's cells are milestone paragraphs that cite closed issues as provenance *on purpose*. So the unit is now the **claim** — a record's row is one, a README cell's is the sentence — and the asymmetry is load-bearing rather than tidiness left undone: sentences everywhere would blind the guard on the corpus's **most common** row, `**Decided, not built.** [#264](https://github.com/eddiefiggie/srd-rules-engine/issues/264)`, whose full stop falls between the phrase and its issue. **The first run found a fourth hit, and it was wrong.** M0's *"an **unbuilt** one now carries an issue of its own"* cites closed #126 correctly — 0027's narrative-append false positive in new clothes. Separated by structure rather than by weakening the phrase: a predicate ends its clause, an attributive is followed by its noun. `unbuilt` joins `not built` as a spelling the guard knows, and that is what found the rest. **Three record rows became claims that never had been**, and one of them — 0027's *"Not built. Part of #140"* — was a second live defect over a **closed** issue, with `dehydration` and `malnutrition` still unimplemented. It survived because `#140` is written **bare**, and the guard had only ever read the *linked* form: the one row in the corpus stating the defect outright was the one row it could not see. Seven rows across five records cite an issue that way. Both forms now, deduped, with `\b` keeping `#1a2b3c` from reading as issue 1. **Two issues filed and four rows repaired** — [#314](https://github.com/eddiefiggie/srd-rules-engine/issues/314) for 0023's damage early-out, decided in 2026-08-24 and tracked by nothing since; [#315](https://github.com/eddiefiggie/srd-rules-engine/issues/315) for the two hazards #140 closed out from under. 0038's row was stale the *harmless* way and said less was built than is: **Rituals ship**, and `ritual_cast` refuses `at_level` because p. 187 draws the consequence itself. `core.hazards`' own docstring was stale the same way and is fixed here: it opened "Two of the five are here" over **three**, and called Suffocation blocked on a removal design that 0028 settled — it was still carrying #180's *"three sources and four rules"*, which 0028 corrected to **seven sources and five removal shapes** by reading the document rather than the issue. **103 of 209, unmoved** — a guard is not a shape. 16 corruptions, each red on its own test. 1655 tests.
...
**103 of 209 shapes resolve today.** The other 106 are listed, not omitted; run
...
_Last updated: 2026-08-29 — build `08292026.23`._
