...
**Current build:** `09042026.2` — **a teleport asks where the creature arrives, and nothing else.** [#444](https://github.com/eddiefiggie/srd-rules-engine/issues/444) was p. 190's Teleportation, held by a destination rule that [0084](docs/decisions/0084-a-space-is-a-control-area-not-a-volume.md) unblocked: *"occupied by another creature"* is p. 191's `is_unoccupied`, the question 0084 built, asked of the destination **point** — reading "destination space" as the arriving creature's whole extent would be the extent-against-extent question 0084 clause 7 declined, and would leave a Gargantuan creature no destination near anyone. **"The nearest unoccupied space of your choice" is enumerated, never picked**: the engine offers every free point at the least straight-line distance and **checks** the caller's choice against them, refusing a landing that is free but not nearest and a landing offered when nothing was in the way. **"Solid" is a barrier that gives cover** — a `Cover.NONE` box is smoke, and a body passes into smoke — stated as the convention it is, because the document does not define the word. The two sentences the issue called the substance are asserted as **behavioural contrasts** rather than as zeros: the same twenty feet walked spends twenty, and the same reach left through `TurnLoop.move` is met with a `ReactionRequest`; then `provocations` is replaced with something that raises, the walk raises, and the teleport does not — so the silence is the absence of a thing that was there to fire. A creature at 0 Hit Points is **not** refused, where a walk is: a teleport is done *to* a creature, and an unconscious body carried by another's spell is the ordinary case. Touching and "must see the destination" are the effect's to state and are disclosed; there are no spells to cause a teleport at all ([#21](https://github.com/eddiefiggie/srd-rules-engine/issues/21)), so what shipped is the transition and the one door it goes through, the position `with_temporary_hit_points` already holds. Seventeen corruption proofs. 143 of 209, 14 clauses. 371 clauses verified against the document. 2345 tests.
...
_Last updated: 2026-09-04 — build `09042026.2`._
