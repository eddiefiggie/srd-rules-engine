...
**Current build:** `08262026.2` — **a file outside the repository is written `/path/to/<name>`, and the rule never said so.** `AGENTS.md` forbids home-relative paths in this public repo and tells an author to make a needed path *relative to the repository root* — which has no answer for the SRD PDF, deliberately outside the tree because the document is not ours to redistribute (`NOTICE.md`). Every mention of it is a path to somewhere on somebody's machine. The convention already existed in `verify_d20_rules.py`'s usage line and in [0013](docs/decisions/0013-effect-shape-normalisation.md) and [0014](docs/decisions/0014-positional-state.md), and was never written down as a rule. **The omission has a cost**: the plan behind [#228](https://github.com/eddiefiggie/srd-rules-engine/issues/228) named the PDF five times at a home-relative path, followed the rule as far as it goes, found it did not reach, and guessed. `tests/test_no_local_leakage.py` caught it at `git add` — the guard working, and also the last possible moment to learn a rule. Documentation only; no mechanic, shape or figure moves. **96 of 211.** 1336 tests.
...
_Last updated: 2026-08-26 — build `08262026.2`._
