...
**Current build:** `08312026.4` — **two entries, one mechanism, and no difficulty in either.** p. 187's Search is a **Wisdom** check "to discern something that isn't obvious"; p. 189's Study is an **Intelligence** check to "call to mind an important piece of information". That is the same sentence twice, so they share a helper — 0013's `mechanism-not-exemplar` — and stay **two shapes**, because the document gives each its own entry, ability and table. **Neither states a DC**, which `perception_resolver` had already met and answered: the caller supplies it with a `basis`, because *a DC the document does not state must say where it came from* (R5). Setting a difficulty is interpretation and belongs to the agent; rolling against it never does. **Both tables only suggest** — the word is in each entry — so a check with **no skill** is legal, and a skill **absent from the table** is accepted: p. 187 lists four and forbids no fifth, so Animal Handling is a legal Search. What is refused is a skill of the **wrong ability**, because a Proficiency Bonus in an Intelligence skill cannot reach a Wisdom check — and that is a **modelling judgement rather than a sentence**, disclosed as one and read off the tables: all four Search skills are Wisdom skills and all five Study skills are Intelligence ones, which a test asserts rather than assumes. [#411](https://github.com/eddiefiggie/srd-rules-engine/issues/411). Five corruption proofs. 122 of 210, 14 clauses. 311 clauses verified against the document. 2193 tests.
...
**122 of 210 shapes resolve today.** The other 88 are listed, not omitted; run
...
_Last updated: 2026-08-31 — build `08312026.4`._
