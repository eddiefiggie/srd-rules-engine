...
**Current build:** `08292026.24` — **a default and the rule that says otherwise are two shapes.** [#316](https://github.com/eddiefiggie/srd-rules-engine/issues/316): p. 89-90 defines **ten** weapon properties and the inventory carried nine. Reach was folded into p. 186's glossary entry with a written reason — *the Rules Glossary already defines it* — which is true of the **term** and false of the **mechanic**: p. 186 gives a creature 5 feet *"unless a rule says otherwise"*, and p. 90 **is** the rule that says otherwise. So one `implemented` flag claimed two rules over an engine that had built one, and a reader consulting the coverage instrument about Reach weapons was told yes for forty-five builds. **The workaround was worse than the gap**: `Combatant.reach` is an `int` and can be set to 10, which p. 90 rules out twice in one sentence — the bonus then reaches a Dagger in the same hand, and Opportunity Attacks made with any weapon, where the document says *"with it"* both times. 0040 clause 2's boundary from the other side. **Three consumers, and the one that mattered withheld the offer**: `_within_weapon_range` never presented the attack, so no later gate could have caught it. **An Opportunity Attack is determined against a set of reaches, not a maximum** — p. 191's Unarmed Strike is always available, so a mover going 5 feet to 7 leaves a Whip-wielder's own reach while staying inside the Whip's, and provokes; a maximum says no at 7 and a bare creature reach says no at 12. **Two defects in the instrument, both found by trying to use it, neither workaroundable.** `IMPLEMENTED_SECTION_SHAPES` had drifted **six** behind the shipped data ([#325](https://github.com/eddiefiggie/srd-rules-engine/issues/325)), so regenerating reported **97 over a shipped 103** — the guard read `KINDS` and covered the glossary shapes only, which is the failure its own docstring describes, one constant from where it was looking. And every section sweep ordered lines by **bounding-box top** ([#326](https://github.com/eddiefiggie/srd-rules-engine/issues/326)): 12pt GillSans-SemiBold headings and 10pt Cambria body declare boxes of very different height, so a heading sorted **after its own first body line** by a constant 7.14pt and every entry's verified text was a window shifted one line late at both ends — `Graze` began mid-word. Nothing went red because every pattern was chosen by someone reading the shifted text, and p. 90's Reach sentence lives entirely in the lost head. Baselines fix it and change no existing row. **104 of 210** — the denominator moved because the split did, the way `save`'s removal moved it in 0035. **Seven mastery issues filed**, which nothing had tracked: [#317](https://github.com/eddiefiggie/srd-rules-engine/issues/317) gates them all on p. 90's unlocking feature, and Graze ships ungated. 7 corruptions, each red on its own test. 1665 tests.
...
**104 of 210 shapes resolve today.** The other 106 are listed, not omitted; run
...
_Last updated: 2026-08-29 — build `08292026.24`._
