...
**Current build:** `09062026.5` — **what the two creatures put on an attack roll is composed once, and every resolver that makes one reads it.** [#464](https://github.com/eddiefiggie/srd-rules-engine/issues/464) was the plainest kind of gap: `attack_resolver` folded the attacker's conditions, the defender's, p. 181's Dodge and p. 90's Vex and Sap into the roll's two flags, and neither the Unarmed Strike nor either improvised use consulted any of it — a Blinded creature punched without Disadvantage, a Paralyzed one was hit by a frying pan without Advantage while the automatic Critical Hit *beside* Paralyzed was read, and a Vex token sat unspent through a punch though p. 90 says *"your **next** attack roll"*. Each is a rule the document states for attack rolls, which all three resolvers make. The composition is `_circumstances` now, read by all three; every source lands on the same pair of flags, so p. 8's cancellation stays the d20's. **p. 16's water reaches the improvised weapon and not the fist**, and both are readings rather than defaults: p. 183 defines the thing as an object *"wielded as a makeshift weapon"* and speaks of *"attack rolls with an improvised weapon"*, so the swing is impeded unless the ruleset's type is Piercing or the wielder swims and the throw misses outright beyond twenty feet — while p. 16 impedes *"a melee attack roll with a weapon"* and p. 177 counts *"a weapon or an Unarmed Strike"* as two things, so a punch underwater rolls as it does on land, excluded rather than extended (R31). **One thing found beside it:** `_impeded_underwater` had quoted p. 16's melee sentence since #446 and the verifier asserted only the ranged one — a quotation is not an assertion — so the sentence the swing now rests on is asserted, patterned from recall until a holder of the document runs the verifier. Coverage does not move. [0091](docs/decisions/0091-an-attack-rolls-circumstances-are-composed-once.md). Eighteen corruption proofs. 146 of 210, 14 clauses. 384 clauses verified against the document. 2468 tests.
...
_Last updated: 2026-09-06 — build `09062026.5`._
