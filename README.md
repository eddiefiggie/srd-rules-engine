...
**Current build:** `08302026.12` — **a field nothing reads is a rule modelled and not applied.** [#357](https://github.com/eddiefiggie/srd-rules-engine/issues/357): `ConditionEffects` is transcribed from the glossary field by field, and **seven of its fields were populated and read by nothing** — every one on a condition marked `implemented: true`, and **not one disclosed**, so both of R17's instruments reported the rule as present. Found by the guard [#356](https://github.com/eddiefiggie/srd-rules-engine/issues/356) proposed, run once by hand; [#344](https://github.com/eddiefiggie/srd-rules-engine/issues/344) had been an eighth until it was fixed, and it was found by accident rather than by anything. **The guard now ships** ([0058](docs/decisions/0058-a-field-nothing-reads-is-a-rule-not-applied.md)) — an AST walk collecting attribute **reads**, because a keyword argument in the `EFFECTS` table is the field being *populated*, which is exactly the state being looked for. **Two of the seven were silently costing damage.** Every hit against a paralyzed or unconscious creature from within five feet was dealing single dice where p. 186 doubles them, and every blow against a Petrified creature landed in full where p. 186 halves it — both consequences already wired (`Critical`, `Defences.resists_all`) with only the occasion missing, the sixth time this repository has found that shape. **A hit, and only a hit**: p. 186 upgrades *"any attack roll that **hits** you"*, so a miss is untouched and a natural 1 still misses regardless (p. 7) — and an **unmeasurable distance does not upgrade**, because a Critical doubles dice and granting one on a distance the engine could not measure manufactures damage. Petrified's Immunity to Poisoned is a **no-op returning the same state object**, not an error: p. 183 says an Immunity means the condition *doesn't affect you in any way*, and a rebuild would move the generation and stale a read token for a change that did not happen. Four of the seven are **disclosed** instead, each waiting on something absent — the two Initiative clauses ([#359](https://github.com/eddiefiggie/srd-rules-engine/issues/359), two strings because they are two clauses and the pin refuses a repeat) and speech and the sense-check flags ([#360](https://github.com/eddiefiggie/srd-rules-engine/issues/360)). A **tautology** was written and removed on the way: `assert after is encounter(petrified) or True` is true by construction. 116 of 210, unchanged — which is #356's whole point, now with a guard behind it. 278 clauses verified against the document, one written from memory and corrected against the page. 10 corruptions, each red on its own test. 1941 tests.
...
**116 of 210 shapes resolve today.** The other 94 are listed, not omitted; run
...
_Last updated: 2026-08-30 — build `08302026.12`._
