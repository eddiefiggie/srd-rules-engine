...
**Current build:** `08312026.13` — **the blow that stops at 1, and two rules that switch themselves off.** p. 184: *"When you would reduce a creature to 0 Hit Points with a **melee attack**, you can instead reduce the creature to 1 Hit Point."* It is its own menu key, following Push, Nick and Cleave — *"you **can** instead"* is a choice, and an option the read surface never offered is one the agent cannot take (R18) — and it is offered for the **Unarmed Strike** too, since p. 190 puts that at 5 feet and knocking someone out with a punch is the canonical use. **Two clauses of p. 17 stop applying, and neither needed a branch**: Monster Death fires *"the instant it drops to 0 Hit Points"* and Massive Damage begins *"when damage reduces a character to 0"* — a subdued creature is at **1**, so neither precondition is met and both fall out of the arithmetic. That is the document's reading rather than an exemption invented here, and it is why a subduing blow knocks out a **monster**: p. 184 says "a creature". The floor is conditioned on `before > 0`, because a creature already down is not being *reduced* to 0 and a bare floor would silently **heal** it. **A corruption proof caught that test being vacuous** — the read surface does not offer an attack on a creature that `is_down`, so the declaration was rejected and no blow ever landed; it passed while asserting nothing and stayed green when the guard was deleted. Third time tonight a failed proof found a real defect. `knocking-out-a-creature` stays **unclaimed**: p. 184's recovery clause needs a condition to carry its cause, and `Conditions.applied` is a bare set ([#428](https://github.com/eddiefiggie/srd-rules-engine/issues/428)). Six corruption proofs. 133 of 210, 14 clauses. 340 clauses verified against the document. 2248 tests.
...
_Last updated: 2026-08-31 — build `08312026.13`._
