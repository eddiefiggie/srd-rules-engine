...
**Current build:** `08252026.42` — **#138's keystone: a Wisdom (Perception) check that reads the obscurement, and the sentence an implementer drops.** `core.sight` could say a space was Lightly or Heavily Obscured from the day #150 filled its tables, and **nothing read the answer** — p. 184's Disadvantage was produced by nothing, so by the sweep's own standard neither `lightly-obscured` nor `dim-light` had resolved while the table that decides them was complete. They were **one blockage**: p. 181's whole entry is *"an area with Dim Light is Lightly Obscured"*, a classification computed and read by nobody. **Three sentences compose, and the third is not about obscurement at all.** p. 182 says a Heavily Obscured space gives the observer the **Blinded condition** while trying to see something in it — and what that costs a check is **p. 177**: *"You can't see and **automatically fail** any ability check that requires sight."* Reading only pp. 184 and 182 leaves Darkness as a worse Disadvantage: plausible, believable in play, and not what the document says. **The chain pays off where no single sentence speaks.** Darkvision reads Darkness *as if it were Dim Light* (p. 180), Dim Light **is** Lightly Obscured (p. 181), and that is Disadvantage (p. 184) — so three entries in sequence turn a check that **cannot be rolled** into one that can. Two flags transcribed correctly since #18 and read by nothing got their first consumer: Blinded's `auto_fail_checks_requiring_sight` and Frightened's and Poisoned's `own_ability_checks` — the latter carrying #192's line-of-sight qualifier by the same route it reaches attack rolls. `Skill` ships p. 9's eighteen with their abilities, and `check_bonus` reproduces p. 186's own worked example (Wisdom 15, proficient, Passive Perception **14**). **`proficiency` stays unclaimed** — p. 186 gives it four kinds and this engine models two. The automatic failure is refused rather than ruled, because a testless proposal with **no effects** is a shape `Proposal` rejects; that guard is right for Falling and wrong here, and relaxing it is a decision rather than a fix ([#224](https://github.com/eddiefiggie/srd-rules-engine/issues/224)). Six clauses added (167 total); three guards proved red, including Heavily Obscured downgraded to a Disadvantage. Senses and light **4 of 10 → 6 of 10**. **95 of 211.** 1333 tests.
...
| **v1.0 — mechanics** | **95 of 211 effect shapes.** Every entry in the inventory ([#14](https://github.com/eddiefiggie/srd-rules-engine/issues/14)) must resolve. Conditions are 15/15 and the d20 test 12/14; senses and light are **6 of 10**. Lightly Obscured and Dim Light resolved together, because they were one blockage: p. 184's Disadvantage was produced by nothing, so p. 181's classification into it was computed and read by nobody. The four that remain each state something nothing consumes — Bright Light's absence of any consequence, Truesight's third piercing, Tremorsense's pinpointing, and Telepathy's languages ([0025](docs/decisions/0025-sight-is-a-relation-over-stored-state.md), [#138](https://github.com/eddiefiggie/srd-rules-engine/issues/138), [#166](https://github.com/eddiefiggie/srd-rules-engine/issues/166)). |
...
**95 of 211 shapes resolve today.** The other 116 are listed, not omitted; run
...
_Last updated: 2026-08-25 — build `08252026.42`._
