...
**Current build:** `08312026.23` — **every unclaimed shape now has a stated reason, and four did not when the pass began.** [#426](https://github.com/eddiefiggie/srd-rules-engine/issues/426) ended by proposing that the signal worth looking for is not evidence a shape **is** built but the **absence of any recorded decision that it is not** — `instant-death` carried none, while `high-jump`, `burrow-speed` and `improvised-weapons` each carried a correct one. All 71 are now grouped: **21** wait on [#21](https://github.com/eddiefiggie/srd-rules-engine/issues/21)'s content, **24** are class or monster data this engine ships none of by design, **10** state something nothing consumes, **9** sit behind filed gates, **4** behind [#337](https://github.com/eddiefiggie/srd-rules-engine/issues/337), and one each behind [#384](https://github.com/eddiefiggie/srd-rules-engine/issues/384), traps, and [#224](https://github.com/eddiefiggie/srd-rules-engine/issues/224). The four that had no reason now do — including two **missing cross-links**: `teleportation` was a fourth shape waiting on #337 that #337 did not list, and p. 16's underwater auto-miss is a second instance of #224 that #224 did not know about, arriving from an **attack** where the first came from a **check** — so the gap is `Proposal`'s vocabulary rather than one resolver's quirk. Underwater Combat is otherwise fully buildable and held entirely by that one clause ([#446](https://github.com/eddiefiggie/srd-rules-engine/issues/446)). **The pass found three real defects after I had twice said the surface was exhausted**, and two of them were written down in the codebase as suspicions nobody had chased — which is the same failure one layer along: a stated reason is worth only what the reading of it is. 357 clauses verified against the document, up from 300 this morning. 139 of 210, 14 clauses. 2296 tests.
...
**139 of 210 shapes resolve today.** The other 71 are listed, not omitted; run
...
_Last updated: 2026-08-31 — build `08312026.23`._
