...
**Current build:** `08312026.8` — **a blocker that was only two-thirds true.** [#141](https://github.com/eddiefiggie/srd-rules-engine/issues/141) has held the five affliction shapes on one stated reason: *exposure is a narrative fact*, so the four poison types wait on a memory-port fact type. **p. 197 says otherwise for half of them.** Injury poison's exposure is *"a creature that takes **Piercing or Slashing** damage from an object coated with the poison"* — a damage-type condition this engine already resolves, conditioned on damage a sibling settled, which is [0049](docs/decisions/0049-advantage-that-outlives-its-roll.md)'s shape and for which `When.DAMAGE_TAKEN` already exists. It even states its own action cost, a **Bonus Action**. Inhaled's exposure is an **area** — a 5-foot Cube whose cloud *"dissipates immediately afterward"* — not a fact. Only Contact and Ingested are genuinely narrative, and those now have #142's demonstrated pattern to follow. **Injury poison is unblocked today**, and the issue should be split. Two sentences are worth having on record before anyone designs it: p. 197 says *"Holding one's breath is ineffective against inhaled poisons"*, an explicit **non**-interaction with p. 189's breath-holding that this engine models — connecting them is the obvious and wrong inference — and Ingested's partial-dose clause is a **GM option** (*"You **may** decide"*) rather than a mechanic. All five sentences are asserted, so whoever takes #141 does not re-read them. 332 clauses, up from 300 this morning. **No shape moves**, deliberately: the reading is the deliverable. 129 of 210, 14 clauses. 2219 tests.
...
_Last updated: 2026-08-31 — build `08312026.8`._
