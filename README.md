...
**Current build:** `08282026.2` — **p. 179 says Concentration *ends*; the engine said suspends.** `Concentration.after_conditions` recomputed the answer from whatever conditions were held when somebody asked, and nothing wrote the field — so the spell came back when Incapacitated lifted ([#238](https://github.com/eddiefiggie/srd-rules-engine/issues/238)). **Not only a read**: `with_damage` asked the same derivation deliberately, so state and the read surface would agree about who is concentrating. They agreed and were wrong together, and damage then compelled a Constitution save to maintain a spell the document had already ended — a save that can *fail*, arriving through the one adjudication entry point with a Ruling, a seed and a ledger entry behind it. **The invariant held and the input was wrong**, which is the failure the ledger cannot catch. The derivation is removed rather than repaired, because a function of the present conditions cannot record that a condition arrived and departed; the end is materialised in `Combatant.__post_init__`, which every writer re-runs through `replace` — an invariant rather than a convention, and it reaches **death** too, which a conditions-only reading structurally could not. [0037](docs/decisions/0037-a-concentration-is-an-early-out-not-an-axis.md) closes the gate behind the rest: a Concentration sustains an effect through a second **early-out** on its duration, beside `save`, **not** a fifth `DurationKind` — spending the one `kind` slot on the early-out would discard the maximum duration p. 179's own third sentence says the description states. Also: p. 179's voluntary end is the caller's outright, so "(no action required)" is now an asserted clause rather than colour — an engine charging an action for it would invent a cost the document declines to state. **95 of 209**, unchanged: this makes the claim correct rather than larger. 1385 tests.
...
_Last updated: 2026-08-28 — build `08282026.2`._
