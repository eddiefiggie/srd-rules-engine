...
**Current build:** `08252026.24` — **a session was driven against the report, and the instrument measures less than the criterion claims.** The contract's primary success criterion is that a session produces *no asserted outcome that did not originate in a Ruling*. `Flag.NARRATION_WITHOUT_RULING` measures that **every narration has a Ruling** — it never compares what the narration asserts against what the Ruling decided, and nothing else does either, because R7 makes the bounds advisory and the report reads structure rather than prose. Demonstrated rather than argued: in `tests/test_skip_guarantee.py` the attack **misses**, the agent narrates *"The club caves its skull in. It dies."*, and `report.flags == ()`. **A green report does not mean the criterion cleared** — which is the same conflation [#42](https://github.com/eddiefiggie/srd-rules-engine/issues/42) warned about for M1, one level further in. The engine did its part: the bounds on that ruling say *"that Boar is dead, unless its hit points reached 0"*, naming the exact claim the narration went on to make. So the sentence is what needs amending, not the code ([#197](https://github.com/eddiefiggie/srd-rules-engine/issues/197)). **#42 stays open, and not for want of effort**: it wants an agent that has not read this repository, and knowing where the challenges are makes a clean run meaningless. That asymmetry cuts one way only — knowing the rubric makes a worse *recall* witness and a better *adversary*, which is why the evasion half could be done and the recall half could not. Every test here is a deliberate "nothing changed" guard and says so. Coverage unchanged at 81 of 211. 1197 tests.
...
_Last updated: 2026-08-25 — build `08252026.24`._
