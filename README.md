...
**Current build:** `08252026.8` — **the five hazard pages are asserted, and they say [#140](https://github.com/eddiefiggie/srd-rules-engine/issues/140) is not the easy coverage win it was filed as.** #140 reasoned that the hazards were cheap because what they resolve *into* already exists — Exhaustion, damage, the campaign clock. That is true and it is not the blocker. Reading the five entries, **every one of them needs an occasion this engine does not have**: Burning fires at the **start** of a turn (p. 178) — the same phase the death save needs and [#124](https://github.com/eddiefiggie/srd-rules-engine/issues/124) is now a `gate` for; Suffocation at the end of one (p. 189), the phase that does exist; Dehydration and Malnutrition at a day's end on the campaign axis (pp. 181, 185) — and they cannot share an implementation, because one inflicts Exhaustion outright and the other is a DC 10 Constitution save. **Falling is worse than blocked, it is inexpressible**: p. 182 gives damage per 10 feet with **no d20 test at all**, and `Proposal.test` is a required field on the one adjudication entry point, which unconditionally rolls it. Nothing in the engine can produce an outcome without rolling a d20. Six clauses now pin all of that (114 total), two proven red against the plausible-wrong occasions — Burning moved to the end of a turn, Malnutrition's DC raised to 15. **No shape is implemented and coverage stays at 76 of 211**: this is the reading that says what building them costs. 1052 tests.
...
_Last updated: 2026-08-25 — build `08252026.8`._
