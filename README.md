...
**Current build:** `08252026.27` — **`from srd_rules_engine.core import *` raised, and nothing looked.** [#200](https://github.com/eddiefiggie/srd-rules-engine/issues/200): three of 159 `__all__` entries — `DeathSaves`, `adjust_roll`, `override_to_success` — were exported by name and never imported. **Worse than untidy.** `core/__init__.py` exists to be R34's re-export surface, so an outer layer may reach a core symbol *only* through it; and all three are named in `ENGINE_SHAPES` as what resolves an inventory shape. The inventory said the engine could do it, R34 said there was one way to get at it, and that way raised. `tests/test_layer_boundaries.py` could never have caught it — it checks the *direction* of imports, and a name missing from a re-export is not an import at all. Nothing else looked, which is how a hand-maintained list of 159 entries drifted. Two guards now, because a hand-maintained list drifts in two directions: every exported name resolves, and nothing is exported twice. The first was **red on `main`**; the second is a deliberate "nothing changed" guard and says so, since checking one direction would have read as checking the list. Both proven red by corruption. Found by accident while building [#19](https://github.com/eddiefiggie/srd-rules-engine/issues/19) — an `__all__` entry added without its import, and the check for that turned out not to exist either. Coverage unchanged at 82 of 211. 1210 tests.
...
_Last updated: 2026-08-25 — build `08252026.27`._
