...
**Current build:** `08282026.10` — **a weapon is something a creature holds, and proficiency was never the weapon's.** [0040](docs/decisions/0040-a-weapon-is-an-item-and-proficiency-is-the-wielders.md) clauses 1-4 ([#258](https://github.com/eddiefiggie/srd-rules-engine/issues/258)): `Weapon` is a subtype of `Item`, an attack is offered **per held weapon and reachable target**, and `attack_resolver()` takes no weapon at all — it reads what was swung off the key the read surface offered. One attack rule replaces one rule per weapon, and the fixture's `_by_actor` dispatcher — two closures picked by actor id — simply deleted. **The rules fix inside the refactor is proficiency.** `Weapon.proficient` was a field on the weapon; p. 89 makes it the wielder's, and storing a creature-weapon relation on the weapon worked exactly while a weapon belonged to one resolver. It is `Combatant.weapon_proficiencies` now, keyed by id because p. 89 grants by *category* and the categories live in the table this repository does not ship. **The read surface stops reporting a distance it declined to judge**: it gated on nothing because it could not know the weapon, and it can now, so an attack out of reach is no longer offered — with a long-range shot staying on the menu, since p. 90 forbids only beyond the *second* range. **And a narrowing worth naming**: a creature holding no weapon is now offered no attack, while p. 177 always allows an Unarmed Strike. Disclosed in the module and filed as [#267](https://github.com/eddiefiggie/srd-rules-engine/issues/267) rather than left for a reader to infer from an empty menu. **95 of 209**, unchanged. 188 clauses. 1475 tests.
...
_Last updated: 2026-08-28 — build `08282026.10`._
