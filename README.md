...
**Current build:** `08252026.39` — **the ledger said the agent declared twice when it declared once, and the report was miscounting because of it.** [#59](https://github.com/eddiefiggie/srd-rules-engine/issues/59): a block is a *suspension* — the loop re-adjudicates the same declaration once the missing facts arrive and the agent is never asked again — but `adjudicate` appends a `declaration` entry per call, so one declaration left two identical entries. The issue judged this cosmetic: *"Nothing is wrong in the engine or the report."* **It was wrong for callers the turn loop does not drive**, which `AGENTS.md` says exist. A blocked ruling neither settles nor closes a slot, so a **fresh** declaration for the same actor lands in the same slot — and the report decided "was the agent asked again?" by *inferring* it from the preceding status, where a block meant a resumption. Adjudicate directly, get blocked, declare something else: against the previous build that reports **1 attempt for two genuine declarations**; it now reports **2**. The fix is the one `testless` already made — *record the fact rather than reconstruct it*, because a resumption and a second declaration look identical from the surrounding entries and only one is the agent trying again. `DECLARATION_VERSION` 2 carries `resumption`, **always present** so a reader can tell "not a resumption" from "written before the field existed". The **caller** states it, because only the caller knows the sequence — and a claim that cannot be true is **refused**, so the flag cannot under-count declarations the way its absence over-counted them. The entries still pair one-to-one with rulings; suppressing the second would strand a ruling with no declaration before it. The report keeps the old inference **only** for v1 entries, and the recorded fact wins where it exists. Three guards proved red. Coverage unchanged at **92 of 211**. 1305 tests.
...
_Last updated: 2026-08-25 — build `08252026.39`._
