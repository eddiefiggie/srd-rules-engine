...
**Current build:** `08302026.36` — **two rules were one check, and coincided by accident.** p. 177 says a creature with a second base AC calculation *chooses*, and [#394](https://github.com/eddiefiggie/srd-rules-engine/issues/394) asked what an **unstated** choice does. Refusing, and the argument is less obvious than it looks: an attack must meet *an* AC, which resembles the initiative tie [0075](docs/decisions/0075-ties-are-a-persons-and-initiative-is-a-dexterity-check.md) had to settle by convention. But the refusal does not happen mid-roll — `effective_armour_class` is read before anything is rolled, and a creature whose AC cannot be determined is one whose ruleset has not finished describing it. So declining *is* possible, and 0075/0076's rule gives the same answer it gave an improvised weapon with no stated damage type. **Both ways of picking are worse, differently**: taking the highest optimises invisibly, and taking the first depends on the order a ruleset listed the equipment — arbitrary rather than optimal, which is better, and still a decision p. 177 assigned to somebody else. Finding this exposed a live defect: the refusal counted **calculations** and called them **suits**, true only while worn armour was the engine's one source of a base. A creature in one suit of plate and a pair of bracers has *one* suit and *two* calculations and was told it wore two suits — false, citing the rule it had not broken. p. 92 is about what a creature **wears**; p. 177 about what it **uses**; they are separate checks now. **And a corruption proof came back green, correctly.** Replacing the selection with `max(...)` — the pick-the-highest invention — changed nothing, because the refusal precedes the selection and there is never more than one base to take a maximum of. Not a vacuous test: picking is **structurally impossible rather than merely avoided**, which is the stronger property and the one #394 wanted. Recorded rather than counted as a proof, because this repository has twice found a green proof meaning the opposite. The mechanism for *stating* a choice is deliberately unbuilt and unfiled — nothing can grant a second base until a feature can, so it would be machinery with no case. [0079](docs/decisions/0079-a-second-base-refuses-rather-than-being-picked-between.md). 118 of 210, 14 clauses. 297 clauses verified against the document. 2139 tests.
...
_Last updated: 2026-08-30 — build `08302026.36`._
