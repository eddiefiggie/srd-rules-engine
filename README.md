...
**Current build:** `08232026.14` — **what a natural 20 or 1 means, and the first two scores the engine derives without rolling.** A natural 20 on an attack roll hits and a natural 1 misses "regardless of any modifiers or the target's AC" (p. 7), so the die settles the attack before the arithmetic is consulted; a Critical Hit then rolls **all** of the attack's damage dice twice and adds the modifiers once, after (p. 179). The rule is read off the die that counted rather than the pair, so a 20 that Disadvantage discarded is not a critical, and replacing a die recomputes it. Checks and saves have no criticals, because the document gives the rule for an attack roll and never extends it. Passive Perception is 10 plus the check bonus, shifted 5 either way by Advantage or Disadvantage (p. 186). Inventory coverage 17 → 21 of 211.
