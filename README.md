...
**Current build:** `08302026.2` — **a turn boundary is shared and a mechanism is not.** [#322](https://github.com/eddiefiggie/srd-rules-engine/issues/322): p. 90's Slow, which borrows Vex's trigger and Sap's window and adds a clause neither has. [0049](docs/decisions/0049-advantage-that-outlives-its-roll.md) built `TurnBoundary` and `is_live` because nothing counted to the **start** of a turn, and put them in a module about **rolls**. Slow needs the same window and reduces a Speed, so the boundary moves to `core.turn_span` and `is_live` becomes structural over a protocol — the two things that satisfy it share **no behaviour**: an advantage token is *spent* by the roll it applies to, a Speed reduction simply **stands** ([0050](docs/decisions/0050-a-turn-boundary-is-shared-and-a-mechanism-is-not.md)). **The reduction is held on the creature, and that reverses 0049 clause 3 knowingly.** Speed is read through `Combatant.effective_speeds`, which by design sees no encounter, so deriving liveness would mean threading the turn order into every reader of a Speed and putting turn-order knowledge inside a property about one creature. `conditions` already modifies Speed from that exact seam and is retired by a turn phase; this joins them. The trade-off lands differently because the reading paths do — and the failure directions differ too: a missed sweep there would grant Advantage past its window, and here it leaves a creature slowed past its own. **One sweep retires both**, through the same function and the same predicate, because two sweeps is how one gets remembered and the other forgotten — and here the sweep *is* the rule for Slow while it is only hygiene for advantage. **The cap is Slow's, not the Speed's**: *"the Speed reduction doesn't exceed 10 feet"* bounds **this property**, so a future rule taking 15 feet is not quietly limited by p. 90's sentence, and two Slow hits take ten feet **between them** while each keeps its own expiry. A per-hit reduction takes twenty, and a single-attacker fixture cannot tell them apart. It reaches the **walking** Speed only — p. 188's Speed — and floors at zero, because a Speed is a distance and not a debt. A stale comment fell out of the proofs: Vex's *"the only mastery whose trigger is not the bare hit"* stopped being true the moment Slow landed beside it. **110 of 210**, masteries **7 of 8** — only Push, behind a Size nothing carries. 235 clauses verified against the document. 10 corruptions, each red on its own test. 1757 tests.
...
**110 of 210 shapes resolve today.** The other 100 are listed, not omitted; run
...
_Last updated: 2026-08-30 — build `08302026.2`._
