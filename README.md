...
**Current build:** `08302026.38` — **the fifth occasion, and a field decided where it lives.** p. 185's Malnutrition compels a DC 10 Constitution save at a day's end, and every occasion that could produce a ruling was **encounter-scale** — a turn's start, its declaration slot, its end, a move. A campaign day is none of them and may happen with no combat at all, which is what made [#399](https://github.com/eddiefiggie/srd-rules-engine/issues/399) a gate. `TurnLoop.end_day` is it: it applies p. 181's bookkeeping, then drains the saves that transition compelled, adjudicating each through the one entry point (R1, R4). **No new machinery** — `with_day_ended` emits a `ForcedSave` and the helper that already drains Concentration's and Topple's drains this, which [0048](docs/decisions/0048-a-forced-save-is-one-shape.md) generalised for exactly that. **Of the gate's three questions, two dissolved on inspection**: who is asked is derived from the caller's statement of consumption, and whether an encounter is the right container was answered four builds ago by every campaign-axis field already on `EncounterState`. The one that mattered was *what drives it*, and the answer came from a **field rather than an argument**: `_owed` — R29's narration debt — is held per loop, so anything that produces a ruling and demands a narration must live on the object tracking that debt. A `CampaignLoop` would have been the tidier diagram and the worse engine. `TurnLoop`'s docstring stops claiming to own the turn, which has been false since `move` landed ([0072](docs/decisions/0072-movement-is-a-phase-the-loop-drives.md)) — two of its five phases are not turn-shaped. **Five occasions now, all found the same way**: a rule the document states plainly with no moment in the loop at which it could happen. `malnutrition` stays **unclaimed** — p. 185 has two sentences and the five-day starvation clause needs consecutive days counted, which nothing does ([#401](https://github.com/eddiefiggie/srd-rules-engine/issues/401)); a shape claimed at half is the overstatement #371 and #264 each found. Seven corruption proofs. [0081](docs/decisions/0081-a-campaign-days-end-is-the-fifth-occasion.md). 119 of 210, 14 clauses. 300 clauses verified against the document. 2162 tests.
...
_Last updated: 2026-08-30 — build `08302026.38`._
