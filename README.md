...
**Current build:** `08302026.37` — **Dehydration, and a clause that was half wrong about its own split.** [0027](docs/decisions/0027-occasions-and-outcomes-without-a-roll.md) clause 8 said *"Dehydration is bookkeeping; Malnutrition is an outcome; they do not share an implementation"*, and [#315](https://github.com/eddiefiggie/srd-rules-engine/issues/315) inherited that framing. Reading p. 185 shows Malnutrition is **both**: a DC 10 Constitution save for eating too little, and an *automatic* level after five days with nothing — no die, no save, bookkeeping exactly like Dehydration. So its two halves are blocked on different things, and the next person to reach for it would have looked for one blocker and found two. p. 181's half is built: `EncounterState.with_day_ended` takes what each creature drank — a narrative fact only the agent holds, the contract `with_time_passed` already states — and inflicts the level outright. No die, so a state transition rather than an adjudication (R1, R4). The table is **`Fraction`, not `float`**: Tiny needs a quarter gallon and the rule turns on *half* of it, and a hazard firing on a rounding error is indistinguishable from one firing on the rule. Two refusals rather than two silent skips — a creature of unknown size has nothing to have drunk less than half of (0051), and only the creatures the caller names are considered, because defaulting the rest to zero would dehydrate every bystander. **A lock that predates the thing it locks worked on the first run**: `LOCKED_EXHAUSTION_RULES` has held `DEHYDRATION_RULE_ID` since [0028](docs/decisions/0028-a-level-carries-the-rule-that-caused-it.md) clause 3, built to the shape the document described rather than to a caller that existed, and four builds later the caller arrived and fitted. Malnutrition's occasion is filed as a gate ([#399](https://github.com/eddiefiggie/srd-rules-engine/issues/399)) and p. 185 is asserted although unbuilt, so whoever takes it does not re-read it. Five corruption proofs. [0080](docs/decisions/0080-dehydration-is-bookkeeping.md). **119 of 210**, 14 clauses. 300 clauses verified against the document. 2151 tests.
...
_Last updated: 2026-08-30 — build `08302026.37`._
