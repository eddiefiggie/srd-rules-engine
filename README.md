...
**Current build:** `08302026.19` — **a record was wrong about its own figure.** An audit of the session's sixteen merges counted the conditions at each commit and found [0059](docs/decisions/0059-initiative-draws-a-pair-for-everyone.md) shipped saying *"six conditions still disclose something, down from eight"* when the answer was **eight, unchanged** — Incapacitated kept `cannot-speak` and Invisible kept `concealed-from-effects-requiring-sight`, so what fell was the **clause** count (12 to 10) and not the condition count. The line got the number and the figure wrong, and survived a merge, a green suite and a review. Corrected in place with a dated note, because a record is a historical document and silently rewriting one is worse than the error. **The general lesson is now beside the one it belongs with**: `AGENTS.md` already says the build stamp's guards do not check that the prose is honest, and a **figure in a record is prose too** — the README's two published numbers are derived and guarded, while a record's Consequences section is counted by hand once and checked by nothing ever again. Derive a figure before writing it down. This is the third self-inflicted defect this session that no guard could have caught, after a disclosure wrong about *why* (0060) and an accessor that defeated the guard it was written to satisfy — and the second found only because something unrelated was being counted. 117 of 210, 18 clauses. 278 clauses verified against the document. No code changes. 1984 tests.
...
_Last updated: 2026-08-30 — build `08302026.19`._
