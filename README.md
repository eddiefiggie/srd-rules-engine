...
**Current build:** `08282026.11` — **p. 177's other half: a creature with empty hands can punch.** #258 made an attack an offer *per held weapon*, which is right for "one attack roll with a weapon" and silently dropped "**or an Unarmed Strike**" — so a creature that lost its sword was offered nothing at all ([#267](https://github.com/eddiefiggie/srd-rules-engine/issues/267), the regression that shipped disclosed rather than hidden). p. 190's **Damage** option is built: an attack roll at Strength plus Proficiency, Bludgeoning damage of 1 plus the Strength modifier, against a target within 5 feet. **Two things a summary gets wrong.** The Proficiency Bonus is **unconditional** — p. 89 adds it only "if you have proficiency with" the weapon, and p. 190 states it flat, with no proficiency in fists to have — so this is a second bonus rule beside the weapon path rather than a case of it, which is why it has its own resolver. And **the damage is not a die**: an implementation reaching for `DamageDice` because every other attack uses one would invent a roll the rules do not call for. The 5 feet is the entry's own, and p. 186 defers to it — "a reach of 5 feet *unless a rule says otherwise*". **`unarmed-strike` stays unclaimed**: p. 190 states three effects and this builds one, and Grapple and Shove both turn on "no more than one size larger than you", which nothing can answer while no creature has a `Size` ([#259](https://github.com/eddiefiggie/srd-rules-engine/issues/259)). Claiming it would be the overstatement R17 exists to prevent — the same call `magic` got in #248. **95 of 209**, unchanged. 191 clauses. 1486 tests.
...
_Last updated: 2026-08-28 — build `08282026.11`._
