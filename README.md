...
**Current build:** `08252026.33` — **the guard that can see a build stamp standing still, and the reference the obvious version of it gets wrong.** `tests/test_build_stamp.py` checks that `__version__` and the README's two stamps *agree* — and **agreement is blind to the pair standing still together**: a PR that forgets the bump leaves a version that never moved matching a README that never moved, and all three assertions pass. That is how build `08232026.39` came to cover two merged PRs ([#147](https://github.com/eddiefiggie/srd-rules-engine/issues/147)). The open question — *does every PR deserve a stamp?* — is answered by the history rather than by opinion: the last PR to merge without one was **#122 on 2026-08-24**, and the **44 since have all bumped**, decision records and test-only changes included. So `AGENTS.md` now says *every pull request*, and a **Build stamp advanced** CI job enforces it. **The reference is the base branch tip, not the merge base** — and this is the part #147's own proposal got wrong. Two branches cut from one commit can each advance past their shared base and still land on the same stamp: **#208 and #210 did exactly that today**, both from `.30`, both to `.31`. Replayed against the merge base, the guard is **green on both**; against the tip it catches the second. It also goes red on #122. The stamp does not sort like a version string — `08242026.9` → `.10` moves forward while `"10" < "9"`, and `12312026.1` → `01012027.1` moves forward while text comparison says **backwards**, because the year is the *last* field of `mmddyyyy` — so ordering is `srd_rules_engine.build_stamp`, unit-tested apart from the git plumbing. One gap ships disclosed: CI re-runs when the PR is pushed, not when `main` moves under it, and closing that is a branch-protection setting no script can assert. Six tests added. Coverage unchanged at **89 of 211** — a guard resolves no shape. 1276 tests.
...
_Last updated: 2026-08-25 — build `08252026.33`._
