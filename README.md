...
**Current build:** `08302026.10` — **Prone crawls or stands, and both were built together.** [#353](https://github.com/eddiefiggie/srd-rules-engine/issues/353): p. 186 states two movement options in **one sentence** and they are different kinds of thing — the crawl restriction is a **refusal**, which 0056 gave a home, and righting yourself is a **capability** that ends a condition, so R1 puts it behind the one adjudication entry point. They shipped together because the refusal alone leaves a creature able to crawl and unable to stand, which is 0052's ordering rule ([0057](docs/decisions/0057-prone-crawls-or-stands.md)). **What had to exist first was a mirror.** `MOVED_BY_FORCE` covers ground and spends nothing; `MOVEMENT_SPENT` spends and moves nobody. `with_movement` couples the two, and p. 190's shove and p. 186's righting each need one half without the other — which is why neither is a *movement*. **The clause a careful reading gets wrong is the Speed-0 one.** p. 186 could be read as stating a cost and letting *"If your Speed is 0, you can't right yourself"* fall out of it: half of nothing is nothing, so standing is free. That grants the ability to precisely the creature the next sentence denies it to, and it is the reading an implementation reaches by **simplifying** — so it is a separate branch, and a corruption proof confirms removing it goes red. **Two refusals for one rule, and a proof showed why**: `can_stand` keeps the key off the menu and `with_movement_spent` refuses the charge, and removing the second left every test green because they all stopped at the menu. The test for it was written after the proof failed. **Both of Prone's disclosures retire** in the change that builds them, so **ten of the fifteen conditions now disclose nothing** — Charmed, Invisible, Petrified, Unconscious and Grappled are what remain. No coverage figure moves — **116 of 210** — because `prone` was already claimed while two of its clauses reached nothing, the third time that shape has been recorded and worth noticing about the instrument. 274 clauses verified against the document, one written from memory, failed and corrected against the page. 9 corruptions, each red on its own test. 1926 tests.
...
_Last updated: 2026-08-30 — build `08302026.10`._
