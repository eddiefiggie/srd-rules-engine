...
**Current build:** `08312026.15` — **I filed this gate tonight and it was wrong.** [#416](https://github.com/eddiefiggie/srd-rules-engine/issues/416) said cover is directional and *"this engine has nowhere to put a relation for the duration of a single attack"*. I had read pp. 15 and 179 and `effective_armour_class`, and **not** `core/obstructions.py` — where `Cover`, its +2/+5 bonuses, `most_protective` with p. 15's worked example, and a line test have existed since 2026-08-23, and where Total Cover has been **refusing attacks through walls** since #20. `total_cover(start, end, obstructions)` is **directional by construction**: it answers for one pair of positions, which is exactly p. 15's *"originates on the opposite side"*. The relation needed nowhere to live because it is computed, not stored. **The real blocker was narrow and was R31**: `Cover.bonus` had no caller because p. 15 earns Half and Three-Quarters by *"covers at least half"* and the document supplies **no method for measuring a fraction**. So an `Obstruction` states its degree, the way `blocks_sight` is already stated per barrier — the geometry stays the engine's and the fraction stays the ruleset's. `Cover.NONE` is **legitimate**, not refused: a barrier that gives no cover and blocks sight is **smoke**, and a first attempt to reject it was caught by the guard forbidding `Obstruction.__post_init__`, which exists because that geometry drifted once. **A corruption proof caught the headline test being vacuous** — it computed `13 + 5 == 18` itself, arithmetic true whatever `core.combat` wrote, and stayed green with the bonus removed. It adjudicates a real attack now. Sixth such catch tonight. Six corruption proofs. 135 of 210, 14 clauses. 341 clauses verified against the document. 2269 tests.
...
**135 of 210 shapes resolve today.** The other 75 are listed, not omitted; run
...
_Last updated: 2026-08-31 — build `08312026.15`._
