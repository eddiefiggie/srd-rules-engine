...
**Current build:** `08292026.28` — **advantage that outlives its roll.** [#318](https://github.com/eddiefiggie/srd-rules-engine/issues/318) and [#319](https://github.com/eddiefiggie/srd-rules-engine/issues/319): p. 90's Vex and Sap, built together because they are **one mechanism reversed on all four axes** — sign, holder, scope, and which end of a turn it dies at ([0049](docs/decisions/0049-advantage-that-outlives-its-roll.md)). Every source of Advantage this engine had was a **standing fact recomputed at the moment of the roll**: a condition held, a target Dodging, a weapon Heavy in these hands. These are granted by one roll, **held**, and **consumed** by another, and nothing here held one. **Sap's boundary had no precedent.** `DurationKind.END_OF_NEXT_TURN` is Vex's window exactly; Sap ends at the **start** of a turn, and `Duration`'s own docstring says the encounter axis counts to *"the end of that creature's turn"*. Both windows are measured against the **attacker's** turns — *"before the start of **your** next turn"* — even though Sap's token belongs to somebody else. **Liveness is derived and the sweep is hygiene**, which is the clause that decided the shape: retiring Sap in a phase would need a start-of-turn hook the loop does not have, and a derived answer **cannot silently outlive its window** — a missed sweep leaves a dead row, a missed retirement grants Advantage in a round the document had ended, and only one of those is visible. **`When` gained a subject rather than a member.** Vex's trigger is p. 182's `DAMAGE_TAKEN` predicate exactly, asked of a **different creature**: the damage is the defender's and the benefit is the attacker's. `_holds` read the effect's own target, so Vex would have looked for damage the *attacker* took, found none, and been withheld on every hit that ever landed — 0032 clause 3 grows the vocabulary one sentence at a time, and this sentence proved the **subject** was what was missing. Both tokens reach the same pair of flags as everything else, so a creature holding a Sap penalty attacking a target it has Vex on rolls **straight** by p. 8, and nobody wrote that. **One proof came back green and found the rule untested**: the expiry test reached its dead token through `advanced_turn`, which sweeps — so the roll saw an empty queue and would have passed however the liveness check behaved. The claim that a dead token changes no roll *even unswept* had nothing behind it. **108 of 210**, masteries **5 of 8**. 231 clauses verified against the document. 11 corruptions, each red on its own test. 1724 tests.
...
**108 of 210 shapes resolve today.** The other 102 are listed, not omitted; run
...
_Last updated: 2026-08-30 — build `08292026.28`._
