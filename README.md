...
**Current build:** `08252026.2` — **terrain now has one route into the engine, and the argument everyone reaches for first turned out to be false.** [0025](docs/decisions/0025-sight-is-a-relation-over-stored-state.md) put light on `EncounterState` while obstructions stayed a query argument, and filed the seam as [#151](https://github.com/eddiefiggie/srd-rules-engine/issues/151) rather than leaving it as prose. [0026](docs/decisions/0026-terrain-enters-as-state.md) settles it: **both are state**, on R1/R4 alone — an input the caller hands over at the moment an outcome is computed is an input the caller *chooses*, and that argument does not wait for cover to reach an attack roll. **The finding worth carrying is what #151 asked to be checked.** It proposed that state-held terrain would make a replay reproduce it; checking that against [0002](docs/decisions/0002-ledger-durability.md) and [0006](docs/decisions/0006-jsonl-and-a-reader-api.md) falsified the premise — `core/report.py` never references `EncounterState`, so replay reads the ruling entry and nothing else. Which means **light being on state does not make light replayable either**; a terrain input reaches replay only by being recorded on the entry, the way `REPLAYABLE_FROM = 2` did for advantage ([#159](https://github.com/eddiefiggie/srd-rules-engine/issues/159)). `core/sight.py`'s note pointing at the seam is corrected here, because a note pointing at a closed issue reads as finished work rather than absent work. **Decided, not built** — the implementation is [#160](https://github.com/eddiefiggie/srd-rules-engine/issues/160) and [#161](https://github.com/eddiefiggie/srd-rules-engine/issues/161), and a routing decision resolves no shape: coverage stays at 76 of 211. 1036 tests.
...
_Last updated: 2026-08-25 — build `08252026.2`._
