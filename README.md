...
**Current build:** `08292026.11` — **a base-tree proof satisfied by a collection error has proved nothing about any individual assertion.** [#298](https://github.com/eddiefiggie/srd-rules-engine/issues/298) adds the standing rule that two PRs earned today. Both followed `AGENTS.md`'s two proof rules, both shipped with a green suite, and **both contained a test that could not fail** — found by `prove_guard_red.sh`, and by nothing else. The gap is structural: a new test module goes red on *import* against the base tree, and that red is **indivisible**, arriving whether the file holds fifteen real assertions or fourteen and a tautology. `prove_against_base.sh` says so itself and is right, which is the problem. **The operative sentence is that a corruption must go red on the test written for it** — one that fires on a neighbouring test has found a different defect and left yours unproved, which is exactly how the first was caught. Two shapes, and the second is the one review cannot see: a tautology (`len(x) == len(set(x))` over a set), and a test **vacuous under its own fixture** — a creature stowed its only weapon, so held nothing, so no attack was offered, so the path under test was never reached. Removing the rule entirely left it green. No new guard: a lint could catch the first shape and nothing general catches the second, so this is a process rule for the reason Status-section accuracy is one. **99 of 209, unmoved.** 214 clauses. 1563 tests.
...
_Last updated: 2026-08-29 — build `08292026.11`._
