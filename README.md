...
**Current build:** `08252026.10` — **an outcome no longer has to roll a d20.** [0027](docs/decisions/0027-occasions-and-outcomes-without-a-roll.md) clause 6 is built ([#170](https://github.com/eddiefiggie/srd-rules-engine/issues/170)): `Proposal.test` was a required field and `adjudicate` called `roll_d20` unconditionally, so **every outcome this engine could express involved a d20** — and Falling deals damage per 10 feet with no test in it. `test` is now optional, `Proposal.outcome` is the branch a testless proposal resolves to, and nothing else moves: one entry point (R1), a seed still drawn, and the declared dice still rolled by the engine (R4). Reaching Falling by inventing a test would have been inventing a roll the rules do not call for. **Two silent failures are closed by construction.** A testless outcome may not be narrated as a success or a failure, because it was neither. And it replays as the new `no-roll` rather than `UNREPLAYABLE` — that verdict means *the record is too thin*, and filing a rule that never rolled under it would make every automatic outcome permanently unverifiable while reading like an ordinary limitation. The two are told apart by a **recorded** `testless` field, not by `roll` being absent, because from the absence alone they are identical — and inferring from an absence is how the advantage gap got in. `RULING_VERSION` moves to 5, additively, and the API version does not move at all. Also refused outright: a proposal with neither a test nor an outcome, and one with both. **Coverage stays at 76 of 211** — this built the capability Falling needs, not Falling. 1062 tests.
...
_Last updated: 2026-08-25 — build `08252026.10`._
