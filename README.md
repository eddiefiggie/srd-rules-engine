...
**Current build:** `08242026.13` — **`CHANGELOG.md` is retired, and the thing that killed it was duplication rather than diligence.** It sat fifteen merged PRs and fourteen builds behind — the CLI and HTTP adapters landed with no entry at all — because the only statement of the rule anywhere was an unenforced checkbox in the pull-request template ([#146](https://github.com/eddiefiggie/srd-rules-engine/issues/146)). Its two neighbours in that checklist are CI-enforced, so an unticked box goes red; this one went green fifteen times. Every one of those PRs *did* write the per-build prose — into this line, where the guard is. So the file is frozen rather than backfilled, and this line is the build record ([0024](docs/decisions/0024-the-build-line-is-the-build-record.md)). Two promises pointed at it and were not decorative: `stability.py` told a consumer of the Provisional surface that a change there is recorded in the changelog, which is that tier's whole compensation for raising no `API_VERSION`, and the plan's M1 criteria listed the file beside the build stamp. Both now name this line. `tests/test_changelog_is_retired.py` refuses a new promise pointing at the dead file — red on the pre-change tree across all four, since a retirement nothing enforces is the same shape as the obligation nothing enforced. The stamp gap the sweep also found — a build stamp can stand still across a merge, and `test_build_stamp.py` cannot see it — is filed as [#147](https://github.com/eddiefiggie/srd-rules-engine/issues/147) rather than fixed here. 972 tests.
...
_Last updated: 2026-08-24 — build `08242026.13`._
