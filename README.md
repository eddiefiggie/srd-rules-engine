...
**Current build:** `08252026.41` — **a creature that acts once and owes one save reports two turns, and now the report says so.** [#120](https://github.com/eddiefiggie/srd-rules-engine/issues/120) asks how `session_report` should group interleaved entries into turns, and names *"whether that is enough to distinguish 'inside turn N' from 'at the end of turn N'"* as the first thing to establish. Established, by reproducing it: an end-of-turn obligation settles the agent's slot and opens **its own**, so `len(report.turns)` counts declaration slots where a reader counting *turns* expects turns. **The facts a grouping rule needs are already in the record** — the obligation's declaration is `improvised` and carries no read token, because [0023](docs/decisions/0023-the-turns-end-is-a-loop-owned-phase.md) clause 2 makes it the engine's artefact rather than the agent's, so its verdict comes back `unread`. **The rule is deliberately not chosen.** Reactions are the second source of the same question ([0015](docs/decisions/0015-reactions-and-the-agent-seam.md)) and do not exist yet, and #120 is explicit that a rule covering only the boundary case means reopening it when reactions land — so picking one now would fit it to one of two cases. What ships instead is the disclosure: `SessionReport.not_measured` gains the entry, and `Turn`'s docstring says it is a **slot, not a game turn**. That list exists for exactly this — *"a report with no flags is the most inviting place in the project to read more than was measured, because a green instrument looks like a cleared bar"* (#197). One test pins the behaviour **and** the disclosure together, so a future attribution rule has to move both rather than one quietly; proved red by deleting the disclosure. Coverage unchanged at **92 of 211**. 1314 tests.
...
_Last updated: 2026-08-25 — build `08252026.41`._
