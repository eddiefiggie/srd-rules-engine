...
**Current build:** `08232026.16` — **the unified d20 test is complete; #15 closes.** A weapon's flat bonus now reaches the attack roll *and* the damage roll, which is what the Berserker Axe says and the half that is invisible when it is missing (p. 213). Dice can be rolled *after* a test resolves and applied to it as a bonus or a penalty — Bardic Inspiration adds after a failure (p. 32), Boon of Fate may subtract (p. 88) — and a failed test can be overridden to a success by Peerless Aim or Legendary Resistance, one shape since decision 0013. A natural 1 still misses through all of it, because p. 7 says regardless of any modifiers and a die applied afterwards is a modifier. Inventory coverage 23 → 27 of 211, and 19 of the 25 d20-test shapes; the remaining six belong to #16, #18 and #19.
