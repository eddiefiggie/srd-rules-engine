...
**Current build:** `08282026.5` — **a caster can cast a spell, and the engine spends what it costs.** The keystone the rest of spellcasting waited on ([#248](https://github.com/eddiefiggie/srd-rules-engine/issues/248), [0038](docs/decisions/0038-a-spell-is-data-the-caster-carries.md)): a `Spell` the caster carries as ruleset data, a **wrapper** the engine owns, and a `Proposal.always` branch for what casting costs regardless of how the roll came out. `spell_resolvers` is the only documented way to register a spell **because** it wraps — a ruleset that expended its own slot could forget to, and a test asserts what that costs: the spell works, the ledger records a Ruling, and only the slot count is wrong. **Casting and spending a slot are separate facts** (p. 104 names four slotless routes), so a cantrip costs no slot and is the case an implementation coupling them gets wrong for the most frequently cast thing in play. **Upcasting is enumerated**: one offer per level `payable_by` returns, so the read surface says not "you may cast bolt" but "at level 1 or 2". p. 105's **one spell slot per turn** is modelled for the first time, in its own structure — same cardinality as `discharged`, different meaning, which is 0036 clause 3 pointed at a resource. And the sweep found the finding: **nothing in the adjudication path had ever spent anything**. `ActionBudget.spend` had no caller outside `dodging()`, so casting is the first act this engine charges — and **an attack still does not cost the Action** ([#252](https://github.com/eddiefiggie/srd-rules-engine/issues/252)), disclosed rather than half-fixed inside a spellcasting change. **95 of 209**, unchanged: the inventory carries no shape for p. 104-105 at all, and `magic` stays unclaimed while half of p. 185's entry is unbuilt ([#253](https://github.com/eddiefiggie/srd-rules-engine/issues/253)). 182 clauses verified. 1431 tests.
...
_Last updated: 2026-08-28 — build `08282026.5`._
