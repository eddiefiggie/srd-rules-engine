...
**Current build:** `09142026.1` — **the plan for the playable set, and the sharpest claim behind it was wrong.** [#475](https://github.com/eddiefiggie/srd-rules-engine/issues/475) was scoped to unblock [#403](https://github.com/eddiefiggie/srd-rules-engine/issues/403), the recall half of the skip guarantee that README names as M1's primary criterion, and paused before a plan was written. [`docs/plans/2026-09-14-001-feat-minimal-playable-ruleset-plan.md`](docs/plans/2026-09-14-001-feat-minimal-playable-ruleset-plan.md) is that plan, and it settles the issue's four open decisions: the set ships **in `core`, in Python**, because `core.save_ends` and `core.hazards` are the precedent rather than `data/`'s JSON; creature attacks live **beside the bestiary, not in it**, so `traits_modelled: false` stays a claim consumers can rely on; the recorded live run is **#403's acceptance, not this plan's**; and coverage may stand still, which the counter's own blind spot already explains. **The issue said no SRD-derived rule had ever reached the engine, and `tests/test_hazards.py` has loaded p. 178's burning and p. 189's suffocation through `load_ruleset` since they shipped** — the gap is a playable *set*, an attack a shipped creature can make, a catalogue with `cited` rows, and an entry point, and planning against the stronger claim would have rebuilt a loader that works. Two stale sentences came off on the way: `tests/fixtures/__init__.py` said [#3](https://github.com/eddiefiggie/srd-rules-engine/issues/3) was open three weeks after it closed, and `core.turn_span` cited 0019 for a rule 0019 does not contain ([#472](https://github.com/eddiefiggie/srd-rules-engine/issues/472)) — copied from 0049 clause 7, which carried the same error and now cites [0039](docs/decisions/0039-equipment-is-what-a-creature-holds-wears-and-carries.md) option 3, where the rule actually lives. A third was found and left for the build: the fixture quotes p. 89 on monster weapon proficiency and the verifier has never asserted that sentence, which is [#371](https://github.com/eddiefiggie/srd-rules-engine/issues/371)'s shape and is U1's first job. No corruption proofs: a plan and three citations add no code. Coverage does not move. 146 of 210, 14 clauses. 384 clauses verified against the document. 2476 tests.
...
_Last updated: 2026-09-14 — build `09142026.1`._
