...
**Current build:** `08302026.33` — **a gate whose three questions were answered on the page.** [#380](https://github.com/eddiefiggie/srd-rules-engine/issues/380) asked what an AC's base is, whether a caller-supplied total survives, and what R31 permits, and said each had no default answer. Reading pp. 177 and 92 answered all three. p. 177: *"Your base AC calculation is 10 plus your Dexterity modifier. If a rule gives you another base AC calculation, **you choose which calculation to use; you can't use more than one**."* So AC is a **chosen base plus bonuses**, and those are different kinds of thing — armour supplies an alternative base, a Shield supplies `+2` on top of whichever won. The p. 92 table invites the opposite reading, since `+2` sits in the same column as `11 + Dex modifier`, and an engine that adds a base to a base puts a character in Plate with a Shield at 28 instead of 20. A **supplied total survives**, and the document is why: p. 254 provides a stat block's AC and p. 92 gives a monster training with any armour in it, so a stat-block number is not an unverified value to replace — it is precisely the other base calculation p. 177 permits. **The gate label was right and its premise was wrong**: the design was undecided because nobody had read the pages, not because the pages left it open, which is the standing rule added one build earlier arriving immediately. Clause 6 lands the decline-or-declare rule ([0075](docs/decisions/0075-ties-are-a-persons-and-initiative-is-a-dexterity-check.md), [0076](docs/decisions/0076-improvised-is-a-use-not-an-object.md)) on a **third** branch: an attack must meet *an* AC so declining is impossible, but unlike a tie the choice is knowable in advance — so it is neither refused nor conventional, but asked for once and stored. Nothing is built; a gate closes by producing a record, and its clauses are filed at that moment: [#393](https://github.com/eddiefiggie/srd-rules-engine/issues/393) and [#394](https://github.com/eddiefiggie/srd-rules-engine/issues/394). [#367](https://github.com/eddiefiggie/srd-rules-engine/issues/367)'s Shield half is unblocked, and never needed the armour *category* it appeared to. Four sentences asserted, so nobody re-reads them. [0077](docs/decisions/0077-armour-class-is-a-chosen-base-and-bonuses.md). 118 of 210, 15 clauses. 295 clauses verified against the document. 2116 tests.
...
_Last updated: 2026-08-30 — build `08302026.33`._
