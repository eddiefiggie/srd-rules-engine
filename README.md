...
**Current build:** `08252026.6` — **the nine sight pages are read, and one of them settled a design question nobody expected it to.** [#150](https://github.com/eddiefiggie/srd-rules-engine/issues/150) is closed: all nine shapes are clauses in `scripts/verify_d20_rules.py` (107 total), `SIGHT_VERIFICATION` is `verified`, and both tables carry rows — Dim Light **is** Lightly Obscured and Darkness **is** Heavily Obscured, which the glossary states as identity rather than relating the two by some further rule. **p. 182 answered the open half of [0025](docs/decisions/0025-sight-is-a-relation-over-stored-state.md) clause 4**: Heavily Obscured gives Blinded *while trying to see something* in the space — scoped to the attempt, so it is a relation per observer and target and never a condition written to state. Had it read the other way, this subsystem would have had to write to state through a ruling (#119). **The other finding is a table that should not be filled**: `SENSE_LIGHT_SHIFTS` models a sense as re-reading a light level, and exactly one does. Blindsight sees without physical sight and is bounded by *Total Cover*; Truesight *pierces* Darkness rather than converting it; Tremorsense "doesn't count as a form of sight" outright. Modelling those three as light shifts would have been a wrong number that looked right, so the table holds Darkvision alone and the rest is [#166](https://github.com/eddiefiggie/srd-rules-engine/issues/166). `obscurement_at` now answers, taking a distance because Darkvision converts only within its range. **`can_see` still refuses**, and more narrowly: Blindsight's Total Cover bound is `EncounterState` geometry since 0026, and answering for the unaided case alone would return a confident `False` for a creature with Blindsight standing in the dark. **Coverage stays at 76 of 211** — no shape is marked, because nothing consumes the answer: no roll is modified, and the penalties Lightly and Heavily Obscured carry are computed nowhere. A value nothing reads has resolved nothing, which is the same argument `opportunity-attacks` ships under. 1049 tests.
...
_Last updated: 2026-08-25 — build `08252026.6`._
