...
**Current build:** `08282026.4` — **a spell is data the *caster* carries, and the engine spends what casting costs.** [#244](https://github.com/eddiefiggie/srd-rules-engine/issues/244) asked what a spell is as something the engine holds; the weapon precedent answers half of it and the half it misses is the one that matters. `attack_resolver(weapon)` closes over ruleset data and `legal_actions` enumerates attacks **without consulting a weapon at all** — but whether a caster may cast Fireball is a fact about the *caster*, and `legal_actions(state, actor_id)` takes state and nothing else. Passing the spell list in is a caller deciding what may be cast, one call at a time, which is exactly what [0026](docs/decisions/0026-terrain-enters-as-state.md) clause 1 refused for light and walls. So the list rides on `Combatant`, beside `slots` ([0038](docs/decisions/0038-a-spell-is-data-the-caster-carries.md)). **The effect is a ruleset resolver the engine wraps**: a ruleset that expended its own slot could forget to, and the failure is invisible — the spell works, the ledger records a Ruling, only the slot count is wrong. **Upcasting is enumerated, not parameterised**, because `payable_by` already computes the payable levels and offering them beats validating against them. **School is excluded because p. 105 excludes it** — "no rules of their own" — and components are not held until something can check them, since a field with no consumer is the decay #228 keeps finding. The sweep also turned up that **nothing in the adjudication path spends anything**: `ActionBudget.spend` has no caller outside `dodging()`, so an attack does not cost the Action and a spell slot will be the first resource an adjudication ever spends. Decided here, built by [#248](https://github.com/eddiefiggie/srd-rules-engine/issues/248). **95 of 209**, unchanged: a record decides and builds nothing. 1404 tests.
...
_Last updated: 2026-08-28 — build `08282026.4`._
