...
**Current build:** `08312026.10` — **a hard problem that was not one, settled by the line above the rule.** [#409](https://github.com/eddiefiggie/srd-rules-engine/issues/409) held `short-rest` on p. 187's *"An interrupted Short Rest confers no benefits"*, and the issue reasoned that the benefits here are hit points **already applied through Rulings and written to the ledger** — so honouring it would need an un-applying this engine does not have, and inventing one would be a second path to a hit point total. **That premise is false, and the sentence that falsifies it sits one line above the spend**: *"Benefits of the Rest. **When you finish the rest**, you gain the following benefits."* Benefits are conferred at the **finish**. An interruption stops the rest before it gets there, so nothing was ever applied and there is nothing to take back — and `TurnLoop.short_rest` *is* the finish, not a simulation of the hour. An interrupted rest now returns having offered no die, and `interrupted` is **on the result** rather than inferred from an empty one, because "the rest was broken" and "the creature spent nothing" are different facts. **`short-rest` is claimed; `long-rest` is not.** p. 185 states two mechanics this engine could express and does not — an interruption after an hour confers *"the benefits of a Short Rest"*, with resumption costing an hour per interruption, and *"During sleep, you have the Unconscious condition"* over a condition set that is 15 of 15 ([#422](https://github.com/eddiefiggie/srd-rules-engine/issues/422)). Three corruption proofs. 131 of 210, 14 clauses. 334 clauses verified against the document. 2233 tests.
...
**131 of 210 shapes resolve today.** The other 79 are listed, not omitted; run
...
_Last updated: 2026-08-31 — build `08312026.10`._
