...
**Current build:** `08232026.34` — **conditions now end, on the encounter axis (#18).** The finding shaped the code: **duration is not a property of a condition.** All fifteen glossary entries state *effects*; only Prone (p. 186) and Exhaustion (p. 181) carry an ending rule of their own, and everything else ends because whatever imposed it says when. So a duration is supplied by the application, and it reduces to one expiry point — **the end of a named creature's turn, in a named round** — computed once when the condition lands. Spans in rounds, "until the end of your next turn", and minutes converted at ten rounds each per `0021`. **Implication is recomputed rather than subtracted**, so a condition implied by two sources survives losing one, and p. 191's "when this condition ends, you remain Prone" is carved out. **A repeated save is reported, never rolled** — that is an outcome, and R1 leaves outcomes to adjudication. The verifier is now 97 clauses.
