...
**Current build:** `08252026.29` — **84 of 211: jumping, and the pair of numbers that is easiest to swap.** A Long Jump is the Strength **score** in feet (pp. 184-185). A High Jump is 3 plus the Strength **modifier** (p. 183). The two entries read almost identically — both need a 10-foot run-up, both halve standing, both cost a foot of movement per foot — and the one thing that differs is which number they take. **A Strength 16 creature long-jumps 16 feet and high-jumps 6**; an engine using the modifier for both would say 3 and 3, and nothing about that would look wrong. A test pins them together for exactly that reason. The **landing** is the half an implementation drops: p. 185 makes landing a Long Jump in Difficult Terrain a DC 10 Dexterity (Acrobatics) check *or the Prone condition* — and unlike Falling this one **is** a test, because the document states a DC, so the d20 decides it rather than a distance. It deals no damage, which is the other way the two entries blend. **`high-jump` is deliberately not claimed**: p. 183 also says what a creature can *reach* — the jump plus one and a half times its height — and nothing here knows how tall anything is. The arithmetic ships and the shape does not, because the entry says more than it computes. Four clauses added (142 total), three guards proven red including the score-for-modifier swap. 1236 tests.
...
**84 of 211 shapes resolve today.** The other 127 are listed, not omitted; run
...
_Last updated: 2026-08-25 — build `08252026.29`._
