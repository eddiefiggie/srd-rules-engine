...
**Current build:** `08302026.8` — **a creature moved by something other than itself.** [#349](https://github.com/eddiefiggie/srd-rules-engine/issues/349), the forced-movement gate: twenty-odd rules across a Barbarian feature, a Fighter one, an Invocation, a weapon mastery, four spells, a magic item and **fourteen stat blocks**, so it is settled once rather than per caller. **The geometry was the gate.** [0014](docs/decisions/0014-positional-state.md) makes a `Position` three **integer** feet and forbids floats, and *"straight away from yourself"* lands on integers only when the ray is axis-aligned — a source at (0,0,0) pushing a creature at (5,5,0) ten feet lands at (12.07…, 12.07…, 0). So the destination is the **lattice point nearest the exact one that is never further than the rule allows**: one objective and one hard constraint. Nearest-the-exact-point serves both goals at once, since that point is on the ray at exactly the stated distance; never-further is 0030 clause 1, because overshooting carries a creature past a boundary the rule did not reach while falling short only withholds ([0055](docs/decisions/0055-a-creature-moved-by-something-other-than-itself.md)). **"Up to N feet" is a menu and "N feet" is not** — p. 90 lets the wielder choose and p. 190 states the distance — offered in five-foot steps because **every push and pull distance the document names is a multiple of five**, its own vocabulary rather than the grid's ([#351](https://github.com/eddiefiggie/srd-rules-engine/issues/351)). **A push is not a movement**: p. 185 provokes an Opportunity Attack only when a creature leaves your reach *using its action, its Bonus Action, its Reaction, or one of its speeds*, and a shove uses none of them — so it is its own effect kind and spends nothing. **Three corruption proofs failed and two were findings**: the nearest-point objective was **untested**, because on almost every push the distance bound leaves exactly one legal corner and the objective never runs; and the ordering key was computed in **two places** that could disagree with nothing noticing. Both fixed, and a genuinely multi-candidate case now covers the objective. **116 of 210, and weapon masteries reach 8 of 8** — [#324](https://github.com/eddiefiggie/srd-rules-engine/issues/324)'s Push was the last of p. 90's eight, blocked since the cluster began. Shove can push as well as knock prone, so its disclosure retires in the change that builds its rule. 270 clauses verified against the document. 11 corruptions, each red on its own test. 1900 tests.
...
_Last updated: 2026-08-30 — build `08302026.8`._
