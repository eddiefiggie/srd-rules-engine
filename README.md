...
**Current build:** `09062026.3` — **a lock that never opened was a stronger claim than either page makes.** [#461](https://github.com/eddiefiggie/srd-rules-engine/issues/461) was found by the change before this one: p. 181 and p. 185 each end *"can't be removed until the creature drinks the full amount of water required for a day"* — food, on p. 185 — and [0028](docs/decisions/0028-a-level-carries-the-rule-that-caused-it.md) clause 3 had built the **can't be removed** half as a constant over rules, so a dehydration level was unremovable forever; a creature that drank its fill for a week could not sleep it off. Every test asserted the closed state and none asserted that it could open. **The "until" is `Hazards.exhaustion_unlocked`**, per creature and per rule: `with_day_ended` lifts it when the day's water or food meets the table's row — the **full** amount, not the half that avoids a level, two thresholds in one row — and `with_exhaustion` closes it again when a new level of that rule arrives, because a creature that is thirsty again holds Exhaustion dehydration caused and has not drunk its fill since. Every gained level passes through that method, the failed save's included, so the close needs one site. `with_long_rest` reads it, and 0028's most-recent-first order runs over what the lift admits. A day the caller says nothing about lifts nothing. No die, no ruling, no shape moves. [0089](docs/decisions/0089-a-lock-lifts-on-the-day-the-requirement-is-met.md). Ten corruption proofs. 145 of 210, 14 clauses. 381 clauses verified against the document. 2438 tests.
...
_Last updated: 2026-09-06 — build `09062026.3`._
