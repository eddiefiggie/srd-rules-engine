...
**Current build:** `08312026.1` — **prose that outlived its issues, and the guard learned the second spelling.** `AGENTS.md` names one direction of the status-claim problem as the one no machine can catch — *"An **open** issue sitting over work that shipped reads as outstanding"* — and counts four occasions met by hand. This is the fifth, and it was worse than a process failure: **the Status rows cite open issues job was red on `main`**, because 0080 clause 7 still said Malnutrition was `not built` over [#399](https://github.com/eddiefiggie/srd-rules-engine/issues/399), which the previous build closed. **Five sites in five files**, all one claim: work asserted outstanding over an issue that had closed. p. 185 is two rules, so clause 7 becomes 7a and 7b rather than losing its unbuilt half ([#401](https://github.com/eddiefiggie/srd-rules-engine/issues/401) still holds the starvation count). **Three of the five were invisible to the guard by spelling alone**: `UNBUILT` knows `not built` and `unbuilt`, and those three asserted the same thing about the **issue** instead — "is still open", "stays open", "while #178 is open". `status_rows.py` had already written the argument against itself — *"a guard that knows one spelling of the thing it exists to catch is a guard the next author defeats by accident"* — so `Claim.asserts_open` joins `Claim.unbuilt` under a `outstanding` union. They stay separate predicates because **0027 clause 5 said `**Built.**` and was stale anyway**: a claim that a cited issue is open is false the moment it closes, whatever the row says about building. **The pattern found a fifth site nobody had read** — 0034 clause 6, over #230, closed for five days. It matches **present tense only**: 0034's replacement says #230 *was* open when that shipped, which is true and is the honest way to write it, and admitting `was` would make it unwritable. The worst of the five is not in a table at all and no guard will ever read it — `core.state.Hazards` said *"Suffocation belongs here and is not built"* three lines above the `suffocating` field, with `core.hazards.suffocation_resolver` consuming it. M1's row stops calling [#42](https://github.com/eddiefiggie/srd-rules-engine/issues/42) open: it closed having **split** the criterion, answered evasion badly — a kill narrated over a recorded miss, report clean, which is R7 working as designed and is why `Report.not_measured` exists ([#197](https://github.com/eddiefiggie/srd-rules-engine/issues/197)) — and left **recall** unanswered and, until now, tracked by nothing ([#403](https://github.com/eddiefiggie/srd-rules-engine/issues/403)). [#404](https://github.com/eddiefiggie/srd-rules-engine/issues/404). One corruption proof. 119 of 210, 14 clauses. 300 clauses verified against the document. 2167 tests.
...
**119 of 210 shapes resolve today.** The other 91 are listed, not omitted; run
...
_Last updated: 2026-08-31 — build `08312026.1`._
