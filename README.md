...
**Current build:** `09062026.4` — **the ability was settled by the sentence that had already settled the swing.** [#390](https://github.com/eddiefiggie/srd-rules-engine/issues/390) was p. 183's fourth rule — *"If you throw the weapon, it has a normal range of 20 feet and a long range of 60 feet"* — read, asserted, held in `core.read_surface` and consumed by nothing, which is [0058](docs/decisions/0058-a-field-nothing-reads-is-a-rule-not-applied.md)'s shape. The issue said the ability was the real question, and it is answered by p. 15 rather than by analogy: *"the ability modifier used for a melee attack is Strength, and the ability modifier used for a ranged attack is Dexterity"*, and the exception is for *"weapons that have the Finesse or Thrown property"* — which an improvised weapon lacks by definition, since throwing a Melee weapon without Thrown is what makes it one. **So a thrown chair is Dexterity where a thrown Dagger keeps its Strength**, and both halves of p. 15 are asserted now; [0076](docs/decisions/0076-improvised-is-a-use-not-an-object.md) had rested the swing on the first half without asserting it. The throw is its own key on the swing's resolver: 1d4 of the ruleset's type, no Proficiency Bonus, refused beyond sixty feet and Disadvantage beyond twenty as p. 90 reads a Range, held only because *"draw that weapon as part of the attack"* is the Thrown property's, and the object leaves the hand whether it hits or misses and lands nowhere the document states. **Two things found beside it.** The read surface's weapon offer had stayed point-to-point through 0086 — its test exercised the resolver's bound and not the menu's — so a Medium fighter could hit a Huge giant from ten feet and was never offered the swing; the offer reads the resolver's measure now. And neither improvised use reads the Advantage and Disadvantage that conditions, Dodging and p. 16's water put on every weapon attack, which predates the throw and is [#464](https://github.com/eddiefiggie/srd-rules-engine/issues/464). `improvised-weapons` is claimed. [0090](docs/decisions/0090-a-thrown-improvised-weapon-is-a-ranged-attack.md). Thirteen corruption proofs. 146 of 210, 14 clauses. 383 clauses verified against the document. 2454 tests.
...
_Last updated: 2026-09-06 — build `09062026.4`._
