...
**Current build:** `08252026.17` — **79 of 211: Suffocation, and the provenance that made it buildable.** [0028](docs/decisions/0028-a-level-carries-the-rule-that-caused-it.md) clauses 1-2 are built ([#183](https://github.com/eddiefiggie/srd-rules-engine/issues/183)): an Exhaustion level now carries the **rule id that caused it** and `Conditions.exhaustion_level` is a derived sum. p. 189 removes "all levels of Exhaustion it gained from suffocating" — a question a count could not answer, which is why Suffocation could not be built beside Burning. **Removal is a rule, not a subtraction**: `with_exhaustion_removed` takes the rule whose levels go, never a number, because a caller that could say "remove two" would be choosing which two and the four removal rules disagree about that. A creature that suffocated *and* marched through the night keeps the march. Suffocation fires at the **end** of a turn (p. 189) where Burning fires at the start — #140 had those in one phase. Recovery is not an occasion: "can breathe again" is an event, so it resolves where the state change happens (`EncounterState.with_breath_regained`), which is 0023 clause 5 applied as 0027 clause 7 applied it to Falling. **`breath_seconds` returns seconds, not minutes**, because p. 189 floors a breath at 30 seconds and `core.clock` counts whole minutes — 0020 is right that nothing at *campaign* scale is finer, and a breath is not campaign scale; rounding to 0 starts suffocation at once and rounding to 1 doubles the air. Both hazard rule ids moved beside `Hazards` for the reason `save_ends_rule_id` moved to `core.conditions`: since 0028 a level literally holds the id, so it keys state and must sit where state can reach it. Coverage 78 → 79. 1132 tests.
...
**79 of 211 shapes resolve today.** The other 132 are listed, not omitted; run
...
_Last updated: 2026-08-25 — build `08252026.17`._
