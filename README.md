...
**Current build:** `08312026.3` — **a Short Rest is an offer, and that is a third kind of occasion.** p. 187 has one benefit this engine can reach, and eleven words of it decided the design: *"You can decide to spend an additional Hit Point Die **after each roll**."* Every occasion here was a **drain** — `end_day`, Concentration, Topple, where the engine compels and nobody is choosing — or a **declaration slot**, which asks **once**. That sentence rules out both, and it rules out the implementation the rule invites: `spend(n)` reads naturally, resolves in one ruling, and has the creature deciding on three dice before the first roll p. 187 gives it. So `TurnLoop.short_rest` offers, adjudicates, and offers **again**, and `SpendHitDie` carries **no count** — a count would re-admit the rejected option through the response type. **The gate's three questions dissolved two-to-one, as [#399](https://github.com/eddiefiggie/srd-rules-engine/issues/399)'s did**: whether each die is its own Ruling was answered by [0027](docs/decisions/0027-occasions-and-outcomes-without-a-roll.md) clause 6, which had permitted a testless outcome since #124 and had never been used in a loop that asks again; whether refusing a spend at full hit points invents a rule was answered by reading p. 187 and finding no prohibition, so a die spent for nothing stays offered. **`HealingDice` is new and is not `DamageDice` with a sign flipped**: damage passes through p. 17's Immunity, Resistance and Vulnerability and healing passes through none of them, so sharing the type would put a `damage_type` on a Hit Point Die. Its `minimum` is p. 187's floor rather than a guard — a negative Constitution modifier can total less than one — and defaults to 0 so no other rule inherits it (R31). **`hit-point-dice` is claimed; `short-rest` is not.** p. 183's mechanic is the spend and the spend is whole. p. 187 states a second — *"An interrupted Short Rest confers no benefits"* — and unlike a benefit with no antecedent, its antecedents are ones this engine **can** see: rolling Initiative, taking damage, casting a non-cantrip spell ([#409](https://github.com/eddiefiggie/srd-rules-engine/issues/409)). Six corruption proofs, one of which exists because a proof failed. [0082](docs/decisions/0082-a-short-rest-is-an-offer-repeated.md). 120 of 210, 14 clauses. 307 clauses verified against the document, up from 300 two builds ago. 2185 tests.
...
**120 of 210 shapes resolve today.** The other 90 are listed, not omitted; run
...
_Last updated: 2026-08-31 — build `08312026.3`._
