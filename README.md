...
**Current build:** `08292026.25` — **a mastery property is unlocked by the wielder.** [#317](https://github.com/eddiefiggie/srd-rules-engine/issues/317): p. 90 opens its Mastery Properties section by gating all eight — *usable only by a character who has a feature, such as Weapon Mastery, **that unlocks the property for the character*** — and p. 89 says it again in the Weapons table's column list. **Graze fired on `weapon.graze` alone**, so any creature handed a Graze weapon got Graze, for the whole life of the property. That is the permissive direction: a mechanic given to creatures the rules do not give it to. **The issue was filed assuming the unlocking feature was content outside the SRD, and that was wrong.** Five classes ship Weapon Mastery in this document — Barbarian p. 29, Fighter p. 48, Paladin p. 54, Ranger p. 59, Rogue p. 62 — all phrasing the grant identically, all letting the choice change on a Long Rest, each with a **Weapon Mastery** column widening the count by level. So the permission is a set of weapon kinds the character chose, and the document supplies it rather than leaving it to be invented. `Combatant.mastery_weapons`, by id, holding the **result** and never the source — p. 90 writes *"such as"* Weapon Mastery and leaves the set of unlocking features open. 0040 clause 2's split, for a second permission. **It is not a subset of proficiency, and that is the clause worth having**: Paladin, Ranger and Rogue grant mastery over weapons *"with which you have proficiency"*; **Barbarian and Fighter do not**. Deriving one from the other would be right for three classes and invented for two, so both directions are asserted — proficient-without-mastery adds the bonus and no Graze, master-without-proficiency grazes and adds no bonus. **Empty by default, and that is also the answer for every monster**: p. 89 gives proficiency an explicit monster rule and p. 90 gives mastery **none**, saying *"a character"*. Reading one across from the other would hand the bestiary a mastery property on the engine's own authority. Graze is retro-fitted here rather than left drifting, and no shipped data loses anything — no bestiary entry and no fixture carries a Graze weapon. **104 of 210, unmoved** — the mechanic did not change, the set of creatures entitled to it did. [0047](docs/decisions/0047-a-mastery-property-is-unlocked-by-the-wielder.md) also constrains the seven unbuilt masteries: none is added without this gate. 4 corruptions, each red on its own test — and the fourth came back **green** first, which is how clause 3 got an assertion: seeding the default with a weapon id changed nothing, because every test reaching Graze set the field by hand. 1669 tests.
...
_Last updated: 2026-08-29 — build `08292026.25`._
