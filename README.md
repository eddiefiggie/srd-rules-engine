...
**Current build:** `08232026.21` — **the fifteen SRD conditions, with their mechanical effects attached rather than their names alone.** R18 asks for effects, so every one is a typed field and none is prose. Conditions imply other conditions transitively — Unconscious carries both Incapacitated and Prone — and implication resolves when the set is built. **Prone works in both directions**: Advantage on attacks within 5 feet, *Disadvantage beyond*, decided by the position the engine already holds; played as a flat Advantage it helps the attacker at every range. One consequence a table almost always misses now falls out for free: an unconscious creature attacked from range has **neither** Advantage nor Disadvantage, because Unconscious grants one, implied Prone imposes the other, and p. 8 cancels them. Frightened's line-of-sight qualifier is unmodelled and applies the stricter reading, disclosed in `unenforced_clauses`. Inventory coverage 51 → 66 of 211.
