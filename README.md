...
**Current build:** `08232026.24` — **the read surface reports everything the engine now knows, so the agent can act on it.** R18 asks for "active conditions *with their mechanical effects*", and a name alone puts the agent back to recalling 5e from training — so a `Situation` reports typed values: the conditions held with implication resolved, what its attacks and attacks against it currently have, Speed after Exhaustion, movement remaining, which of action, Bonus Action and Reaction are available, spell slots left by level, and the rules the engine holds but does not enforce. Dash, Dodge and Disengage are offered while an Action remains and withdrawn once it is spent; an Incapacitated creature is offered only the end of its turn, because a creature that can do nothing must still be able to stop. No new shapes — coverage stays 76 of 211, and the capability is materially different.
