...
**Current build:** `09062026.1` — **the path was the new thing, and the rest was deciding which sentence is a ruling.** [#451](https://github.com/eddiefiggie/srd-rules-engine/issues/451) was p. 14's *Moving around Other Creatures*, four sentences built once, set aside on [#456](https://github.com/eddiefiggie/srd-rules-engine/issues/456), and rebuilt on [0086](docs/decisions/0086-a-range-runs-from-the-edge-of-a-space.md)'s reach: a move is now a straight segment clipped against every other creature's square, so the engine can say which spaces it passes through, for how many feet, and where it ends. **Three of the four sentences are refusals and a price in `with_movement`, not rulings** ([0056](docs/decisions/0056-a-move-is-refused-where-it-is-made.md)): the passage is refused unless the other creature is an ally, Incapacitated, Tiny or two sizes apart; the stretch inside a non-exempt space is Difficult Terrain, merged before it is floored so p. 181's *"isn't cumulative"* holds by construction; and no move may end in a space with another creature — an ally's included, because that sentence names no exception where the first names four. **The fourth is the one outcome, and it takes Suffocation's shape**: a creature that *"somehow"* ends its turn in a space with another owes a Prone the loop derives and the one door applies, unless it is Tiny or the larger. *"In a space with"* is read as **either point in the other's square** — not the overlap of the squares, which would put two Medium creatures five feet apart together and knock every melee pair Prone at the end of every turn. Ally is p. 176's, and p. 176 makes it a designation four ways, so it is `Combatant.side`, stated, and a creature with no side is nobody's. Every unstated size withholds, in both directions. **Two fixtures stood at exactly five feet from a Large creature — on the inclusive boundary of its square — and moving them to seven found three sites 0086 had missed**: the read surface's unarmed reach was still point-to-point, so a Medium creature could grapple a Large one only from inside its space, the position p. 14 refuses to walk into. Fixed here, with the test 0086 promised every site. [0087](docs/decisions/0087-a-move-is-a-path-through-spaces-and-a-turn-ended-in-one-is-prone.md). p. 185's object clause, the half of #451 that is not expressible, is split out as [#459](https://github.com/eddiefiggie/srd-rules-engine/issues/459). Twenty corruption proofs. 144 of 210, 14 clauses. 380 clauses verified against the document. 2412 tests.
...
**144 of 210 shapes resolve today.** The other 66 are listed, not omitted; run
...
_Last updated: 2026-09-06 — build `09062026.1`._
