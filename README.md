...
**Current build:** `08312026.7` — **the first core fact type, and four design questions the document dissolved.** [#142](https://github.com/eddiefiggie/srd-rules-engine/issues/142) had sat over five shapes because the memory port — built since #9, thorough, with R22's `DefaultKind` in place — had **never carried a core fact type at all**. Every `FactType` in the repository was a fixture, so the machinery for rules that ship had only ever been exercised by rules that do not. Attitude is the first, and README named it as the example from the beginning. **All four of #142's questions dissolved on reading pp. 182-184**, including the one it warned could grow the work: p. 184 ends *"On a successful check, the monster does as urged"* — the monster **complies**, it does not come to like you — so nothing writes an attitude and #119's shape is not needed one layer along. The default was the other: #142 insisted it be checked rather than assumed, and p. 184 states it outright — *"Indifferent is the default attitude of a monster"* — making it `SRD_PRESCRIBED` rather than the `ENGINE_CHOSEN` guess. **The DC is the monster's Intelligence *score*, not its modifier**, which is the one number here that would have been quietly wrong by about seven. Two of p. 184's three determinations **throw no die** — a willing monster complies and an unwilling one refuses — and both are still Rulings, because an agent deciding off its own bat that the guard stood aside has produced an outcome no rule resolved. p. 184's 24-hour bar on urging the same way again is **disclosed and unbuilt** ([#418](https://github.com/eddiefiggie/srd-rules-engine/issues/418)). Six corruption proofs. 129 of 210, 14 clauses. 327 clauses verified against the document. 2219 tests.
...
**129 of 210 shapes resolve today.** The other 81 are listed, not omitted; run
...
_Last updated: 2026-08-31 — build `08312026.7`._
