...
**Current build:** `08252026.38` — **a castle wall shrugging off a 9-point blow, and an ordering the document never states.** p. 180's Damage Threshold, [#214](https://github.com/eddiefiggie/srd-rules-engine/issues/214) — split out of [0032](docs/decisions/0032-an-outcome-conditional-on-its-own-damage.md)'s sweep, which found it **is not** a conditional effect. It reads like one, being keyed on the size of a blow; the entry says *"has **Immunity** to all damage unless…"*, and what it modifies is **the damage itself**. Building it as a conditional would have put a defence outside the one place defences are applied, where the death save for "any damage" and Massive Damage's remainder would both read a number it had not yet zeroed. **The hard part is where it sits.** p. 17's *Order of Application* names three steps — adjustments, Resistance, Vulnerability — and the threshold is **not one of them**, so no ordering is stated and picking one would be inferring a rule value (R31). It does not have to be picked: p. 180 **classifies** the threshold instead of sequencing it, Immunity is already first, and the same sentence names what it is asked *about* — *"an amount of damage **from a single attack or effect**"*, repeated as *"that entire **instance**"*, and the example says it twice more in the passive (*"is **dealt** to it"*). A derivation from a stated classification, not a chosen order — and the difference is live: 12 against a threshold of 10 with Resistance gets through and halves to **6**, where comparing the halved 6 would make the creature **Immune**. **0030 clause 1 must not be reached here**, which is exactly the trap 0031 clause 2 names: resolving "away from invention" would compare *after* Resistance but *before* Vulnerability — not one rule read two ways, but a thumb on the scale picked per case to minimise a number. Both operative sentences say **>=** (*"equal to or greater than"*, and damage that *"fails to **meet or exceed**"* is superficial); only the example's gloss abbreviates to "exceed", about a 9 against 10 where the readings cannot disagree — so exactly 10 lands, and that boundary is a test. Three clauses added (161 total), three guards proved red including the `>` boundary and the post-Resistance comparison. **92 of 211.** 1300 tests.
...
| **v1.0 — mechanics** | **92 of 211 effect shapes.** Every entry in the inventory ([#14](https://github.com/eddiefiggie/srd-rules-engine/issues/14)) must resolve. Conditions are 15/15 and the d20 test 12/14; senses and light are **4 of 10**, two of them claimed by reading the entries rather than by writing code: Heavily Obscured, whose Blinded is scoped to *the attempt* and so **is** the relation `can_see` already answers, and Darkness, whose whole entry is the mapping that feeds it. The six that remain each state something nothing consumes — Lightly Obscured's Disadvantage on Perception, Dim Light's classification into it, Bright Light's absence of any consequence, Truesight's third piercing, Tremorsense's pinpointing, and Telepathy's languages ([0025](docs/decisions/0025-sight-is-a-relation-over-stored-state.md), [#138](https://github.com/eddiefiggie/srd-rules-engine/issues/138), [#166](https://github.com/eddiefiggie/srd-rules-engine/issues/166)). |
...
**92 of 211 shapes resolve today.** The other 119 are listed, not omitted; run
...
_Last updated: 2026-08-25 — build `08252026.38`._
