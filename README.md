...
**Current build:** `08312026.26` — **the gate was a gate because of what extent might have changed, not what it does.** [#337](https://github.com/eddiefiggie/srd-rules-engine/issues/337) held four shapes and listed distance, areas, cover and sight as things a creature's space would touch. **Every one of those is a question the document answers with a grid and does not answer without one** — and grid-as-default is a declined non-goal, so measuring between two extents by their nearest parts would be a rule value R31 forbids, arrived at by geometry the document never describes. **The second sentence of p. 14 is the whole decision**: a space is *"the area that it effectively **controls** in combat"* — a control area, not a volume — which is why it answers who is standing where and does **not** answer how far away anything is. `within`, `core.areas`, `core.sight` and `core.obstructions` are untouched, and clause 7 is **pinned by a test** so that if range ever becomes extent-aware, something fails and makes someone say so. **Tiny is 2½ feet**, which decided the type: `Position` is integer feet, an `int` loses the half and a `float` rounds it, so the table is `Fraction` — p. 181's water ration already needed that. **A corruption proof caught the boundary test not reaching its boundary**: a Medium creature's half-width is 2½ feet and no integer position can land on it, so `<=` and `<` were indistinguishable. Large's edge is a whole 5, and the test uses it. Two creatures may share a point, because p. 14 says what happens if you *"somehow end a turn in a space with another creature"* and a model refusing to represent that makes the rule unaskable. [0084](docs/decisions/0084-a-space-is-a-control-area-not-a-volume.md). Four corruption proofs. 142 of 210, 14 clauses. 361 clauses verified against the document. 2324 tests.
...
**142 of 210 shapes resolve today.** The other 68 are listed, not omitted; run
...
_Last updated: 2026-08-31 — build `08312026.26`._
