...
**Current build:** `08302026.32` — **a standing rule the last three builds paid for.** `AGENTS.md` already said never to infer a rule value, and said that a seeded dataset is not a verified one. Between those two sits the case that actually kept happening: a value that is **correct and still unverified**, indistinguishable from a guess from inside the repository. [#371](https://github.com/eddiefiggie/srd-rules-engine/issues/371) found `core.casting` **quoting** p. 105's ritual clause word-perfectly in a docstring while the verifier had never read that sentence — an entire issue resting on a clause nobody had asserted. [#385](https://github.com/eddiefiggie/srd-rules-engine/issues/385) found `initiative_order`'s `ability="dex"`: right, honest about being unread, and unfalsifiable for as long as nobody checked. [#264](https://github.com/eddiefiggie/srd-rules-engine/issues/264) is the counter-example — the page was read first and the reading *changed the design*, because p. 183's second sentence is what makes improvised-ness a use rather than an object. So the rule is now written down: **read the page before building the mechanic, and do not rely on a quotation already in the tree, however exact.** A docstring quoting the SRD is evidence that somebody believed something and nothing more; if the sentence a change rests on is not in `verify_d20_rules.py`, adding it is part of the change. Docs only — 118 of 210, 15 clauses. 291 clauses verified against the document. 2116 tests.
...
_Last updated: 2026-08-30 — build `08302026.32`._
