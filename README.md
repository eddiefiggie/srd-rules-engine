...
**Current build:** `08302026.1` — **a second swing at a creature beside the first.** [#323](https://github.com/eddiefiggie/srd-rules-engine/issues/323): p. 90's Cleave, which waited on Reach — *"also within **your** reach"* is the wielder's reach **with this weapon**, and a Halberd's is 10 feet ([#316](https://github.com/eddiefiggie/srd-rules-engine/issues/316)). **Two positional tests with different origins**, and that is the pair an implementation collapses: *"within 5 feet of **the first**"* is measured from the creature that was hit, *"also within your reach"* from the attacker. On a straight line they covary, so the fixtures put the third creature on a **second axis** — otherwise a test cannot show which of them refused an offer, and one of them not being asked at all would look identical. **Its cap is its own.** *"You can make **this** extra attack only once per turn"* is p. 90's sentence and not p. 89's, so a creature may take the Light property's extra attack **and** a Cleave in one turn — `is_extra` covers Cleave for the damage exception and the Multiattack tally, and `is_p89_extra` does not, because reusing one for the other spends a cap by exercising the other. **A melee hit, said twice**, so a Thrown weapon carrying Cleave opens nothing at sixty feet — and a Cleave opens no further Cleave, which would chain through a crowd off one hit. **Three bugs the tests found in code written the same hour.** The offer compared with `distance_feet`, whose own docstring says it is *"for the record, never for a comparison"* — it rounds down, so two creatures 5.9 feet apart read as 5 and a swing the document forbids was offered; `within` is exact and is what the other range checks already use. `Effect` refused the new kind's `weapon_id` and `source_id`, and the source refusal turned out to bundle **two** rules in one message — a Cleave names a second creature and states no span, so the duration and source halves are separate refusals now, with the source half getting the assertion it never had. And the action-cost fixture was **unreachable by play**: an opening without the attack that made it, in which a Cleave *does* charge the Action. **109 of 210**, masteries **6 of 8** — only Slow, and Push behind a Size nothing carries. 233 clauses verified against the document. 14 corruptions, each red on its own test. 1743 tests.
...
**109 of 210 shapes resolve today.** The other 101 are listed, not omitted; run
...
_Last updated: 2026-08-30 — build `08302026.1`._
