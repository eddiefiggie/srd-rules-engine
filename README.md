...
**Current build:** `08312026.16` — **p. 183's Hide, and the third value of a three-value enum doing its job.** Hide needed everything at once — Three-Quarters and Total Cover ([#416](https://github.com/eddiefiggie/srd-rules-engine/issues/416)), the sight chain, the Invisible condition, and [0083](docs/decisions/0083-a-condition-carries-the-rule-that-caused-it.md)'s causes to scope its ending — and asks for **two** conditions joined by *and*: obscurement **or** Three-Quarters/Total Cover, **and** out of any enemy's line of sight. Cover does not imply concealment: a railing gives Three-Quarters and hides nothing. **Half Cover is not among the degrees p. 183 names**, and admitting it would help the hider by a rule the document does not state. **The check's total becomes the DC to find you**, so the engine fills that number from the roll it just made — a resolver returning one would be supplying a *result* (R4), and it could not know it anyway. That is **not** [#216](https://github.com/eddiefiggie/srd-rules-engine/issues/216)'s gap, which is a magnitude from a *sibling's* settled damage. **The subtle part is `UNSTATED`**: an encounter with no positions cannot say whether an enemy sees you, and p. 182's third value is *the document does not say* rather than "no" — reading it as a no let a creature hide in an open room, which is what I wrote first. It refuses now, and says why. **A regression caught on the way**: adding cover degrees left `line_is_blocked` degree-blind, so a Half-Cover barrier would have stopped a **Fireball** — p. 177 blocks a line of *effect* with **Total** Cover, and before #416 every obstruction meant Total so the two questions were one. Three clauses of p. 183. [#432](https://github.com/eddiefiggie/srd-rules-engine/issues/432). 136 of 210, 14 clauses. 344 clauses verified against the document. 2283 tests.
...
**136 of 210 shapes resolve today.** The other 74 are listed, not omitted; run
...
_Last updated: 2026-08-31 — build `08312026.16`._
