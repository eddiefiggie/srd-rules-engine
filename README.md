...
**Current build:** `08262026.3` — **a term the document defines and never uses is not a shape.** [#229](https://github.com/eddiefiggie/srd-rules-engine/issues/229) asked whether `weapon-attack` names a second shape or double-counts `attack-roll`, and the document supplies a third answer: SRD 5.2 defines *Weapon Attack* on p. 191 and **never uses the term again** — three occurrences in the whole document, two in its own entry and one on p. 217 that is a noun and a verb. Nothing gates a mechanic on it, so it renames `attack-roll` with a parameter fixed and is filed as vocabulary ([0034](docs/decisions/0034-a-term-the-document-defines-and-never-uses.md)). **The test is consumers, not phrasing** — `spell-attack`'s entry reads the same way and stays a shape, because p. 106 gives it a bonus formula `attack-roll` does not state. Both branches #229 offered were bad: claiming it may double-count, and leaving it unclaimed makes the ceiling unreachable — [#228](https://github.com/eddiefiggie/srd-rules-engine/issues/228)'s option 3, which [0033](docs/decisions/0033-a-glossary-entry-is-an-index-not-a-shapes-boundary.md) rejected as false. A declassification resting on an absence has to **assert** the absence, so the verifier gains a document-wide clause table with a control row. **The denominator was wrong, not the engine**: 211 → 210 with the numerator unchanged and `ENGINE_SHAPES` untouched. **96 of 210.** 1338 tests.
...
**96 of 210 shapes resolve today.** The other 114 are listed, not omitted; run
...
is editorial and lives in that script where it can be reviewed. Twenty-one entries are
...
_Last updated: 2026-08-26 — build `08262026.3`._
