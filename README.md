...
**Current build:** `08302026.31` — **improvised is a use, not an object.** p. 183's second sentence forecloses the alternative outright: *"A Simple or Martial weapon also counts as an improvised weapon **if it's wielded in a way contrary to its design**"*, so an `ImprovisedWeapon` type could not express a longbow swung as a club — the document's own example. It is a property of the **attack**: its own key, its own resolver, 1d4 whatever the object's dice are, and no Proficiency Bonus (a prohibition, not a proficiency the wielder lacks). **[#264](https://github.com/eddiefiggie/srd-rules-engine/issues/264) posed the damage type as a three-way choice** — a supplied fact, a refusal, or untyped damage disclosed — and the answer is two of the three, which are not alternatives: the ruleset supplies what it has ruled on, and the engine refuses what nobody has. Untyped is rejected because it is not the absence of a ruling but a ruling — that Resistance to Bludgeoning does not apply — made by whoever left the field blank. **Two builds, two questions the document hands to a person, two different answers**, and the pair is the interesting part: [0075](docs/decisions/0075-ties-are-a-persons-and-initiative-is-a-dexterity-check.md) declared a convention for initiative ties because a tie *must* resolve — there is no refusing to order the combatants — while an improvised swing need not happen at all. **When declining is possible, decline; when it is not, declare a convention.** Neither record could have stated that rule alone. p. 183's thrown 20/60 is read, asserted, and **not built** — offering a throw without its range is a rule half-applied, so it is [#390](https://github.com/eddiefiggie/srd-rules-engine/issues/390) and the `improvised-weapons` shape stays unclaimed at three-quarters, which is the overstatement #371 found in this same file one build ago. Seven corruption proofs. [0076](docs/decisions/0076-improvised-is-a-use-not-an-object.md). 118 of 210, 15 clauses. 291 clauses verified against the document. 2116 tests.
...
_Last updated: 2026-08-30 — build `08302026.31`._
