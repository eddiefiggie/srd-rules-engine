...
**Current build:** `08252026.3` — **obstructions are state now, so a caller can no longer decide who a Fireball reaches by deciding which walls exist for that one call.** [0026](docs/decisions/0026-terrain-enters-as-state.md) clause 1 is built: `EncounterState.obstructions` sits beside `lighting`, and the composed question is `EncounterState.creatures_in(area)`. **The composition moved rather than gaining a parameter** — `core.areas` has no `creatures_in` at all any more, which keeps `Area.contains` what it always claimed to be, pure volume with no opinion about walls. The route is enforced rather than noted: a guard asserts no public callable in `core.areas` takes an `obstructions` parameter, so re-introducing the dial turns red, and it was proven red by re-introducing one. `core.obstructions.line_is_blocked` is deliberately outside that guard — a pure predicate over three explicit arguments has no encounter to be wrong about. A combatant with no position is reached by nothing, because reporting it as caught *or* as spared would invent the answer the engine cannot compute. Still unbuilt and still filed: the shared box ([#161](https://github.com/eddiefiggie/srd-rules-engine/issues/161), now unblocked) and recording terrain-derived values on the ruling entry ([#159](https://github.com/eddiefiggie/srd-rules-engine/issues/159)). No effect shape resolves — coverage stays at 76 of 211. 1039 tests.
...
_Last updated: 2026-08-25 — build `08252026.3`._
