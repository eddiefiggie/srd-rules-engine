...
**Current build:** `08252026.7` — **`MAX_SPELL_LEVEL` was right and cited the wrong page, and the right page was already in the reference.** [#130](https://github.com/eddiefiggie/srd-rules-engine/issues/130) asked for p. 26 — the class slot table — to be asserted. Reading the document says it should not be: p. 104 states the bound outright, *every spell has a level from 0 to 9*, and p. 104 was **already** cited by `SPELLCASTING_VERIFICATION`. So the fix removes a dependency on class content rather than adding a check for it. `core.spellcasting` refuses to ship p. 26's table on principle, and `MAX_SPELL_LEVEL` was the one value in the module resting on that page anyway — which is the shape of the defect, not the number. Nine is right; a right number and a wrong one are indistinguishable once inside a finished ruling, and nothing was checking this one. The slot floor is now a **derivation from two asserted sentences** rather than a bound read off a table: p. 104 puts a spell's level in 0-9, p. 178 puts a level 0 spell outside the slot economy, so slots run 1 to 9 — and neither sentence has to say so. Clause 108 in `scripts/verify_d20_rules.py`, proven red against "0 to 10". **This was the last gap filed as blocked on the document**; the remaining `srd-fidelity` issue ([#124](https://github.com/eddiefiggie/srd-rules-engine/issues/124)) is a design question about a phase that does not exist. Coverage is unchanged at 76 of 211 — provenance is not coverage. 1052 tests.
...
_Last updated: 2026-08-25 — build `08252026.7`._
