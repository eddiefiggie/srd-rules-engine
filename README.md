...
**Current build:** `08252026.31` — **the allowance a flying creature actually has, and the worked example that could not be expressed.** p. 188: *"If you have more than one speed, choose which one to use when you move; you can switch between the speeds during your move. Whenever you switch, subtract the distance already moved from the new speed."* The allowance is **per mode against one shared spend** — and this engine had one pool, sized by Speed. `with_movement` checked every mode against `movement_remaining`, which is derived from `speeds.walk` alone, so a creature with a Speed of 30 and a Fly Speed of 40 **could fly only 30 feet**. The document's own worked example on the same page is the one no single-pool reading reaches: *"you could fly 10 feet, walk 10 feet, and leap into the air to fly 20 feet more"* — 40 feet in total, and it is the **third leg** that fails, 20 more feet with 20 already spent. That example now runs as a test, and it raised `ValueError` against the pre-change tree. `Combatant.movement_remaining_in(mode)` supplies the number: one `movement_used`, subtracted from whichever speed is being asked about. **A mode the creature cannot use answers `None`, not 0** — pp. 178 and 182 grant flying and burrowing only through the speed itself, and "no flight" is a different fact from "no flight left"; climbing, swimming and crawling keep falling back to Speed, because pp. 178, 189 and 179 price them for a creature that lacks the speed. Dash still adds **feet rather than speed** (p. 180), so its pool is spendable in any mode. The read surface reports the whole map, since a number the caller consults before proposing a move is where the old bug was visible to an agent ([#206](https://github.com/eddiefiggie/srd-rules-engine/issues/206)). Coverage is unmoved at **89 of 211**: `split-movement` was already claimed, and was already wrong — a claimed clause is not a checked one. Four clauses added (151 total). 1270 tests.
...
_Last updated: 2026-08-25 — build `08252026.31`._
