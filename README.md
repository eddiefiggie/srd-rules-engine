...
**Current build:** `08312026.9` — **the poison the engine can see, and an ordering guard that caught the build.** Yesterday's reading found [#141](https://github.com/eddiefiggie/srd-rules-engine/issues/141)'s blocker false for half its shapes; this builds the half. p. 197 exposes a creature that *"takes **Piercing or Slashing** damage from an object coated with the poison"* — a **damage-type condition**, which this engine already resolves, so Injury needs no memory-port fact at all. The poisons themselves stay content ([#21](https://github.com/eddiefiggie/srd-rules-engine/issues/21)): a `Poison` carries a DC and the rule its effects resolve under, both **ruleset data**, for the reason `SpellSlots` ships no table of slot counts. **Two clauses an implementation drops by building the exposure half and stopping**: a coated **club** delivers nothing — Piercing *or Slashing*, and firing on any hit is the shortcut — and the coating is **spent**, because *"the poison remains potent until delivered through a wound"*, without which one smeared dagger is venomous forever. **The engine refused my first attempt.** The exposure effects went *before* the `DamageDice` they are conditioned on, and `_conditional_order` rejected the proposal outright: a `When.DAMAGE_TAKEN` predicate placed before its damage is false before the branch runs, so the effect would never fire — 0032 clause 2, working exactly as written. A corruption proof then failed and found a **missing test**: the miss case never runs the success branch, so it was not testing the predicate at all. What the predicate guards is p. 17's **Immunity** — a creature immune to Piercing takes nothing, so nothing goes through the wound — and that test now exists. Contact, Ingested and Inhaled stay **named and unbuilt** (R32). Six corruption proofs. 130 of 210, 14 clauses. 333 clauses verified against the document. 2230 tests.
...
**130 of 210 shapes resolve today.** The other 80 are listed, not omitted; run
...
_Last updated: 2026-08-31 — build `08312026.9`._
