...
**Current build:** `08312026.5` — **a buffer that absorbs damage without reducing it, and a state nothing stores.** p. 190's Temporary Hit Points entry is an **index** — *"granted by certain effects and act as a buffer… See also 'Playing the Game'"* — so the mechanic is **p. 18's five clauses**, which is exactly [0033](docs/decisions/0033-a-glossary-entry-is-an-index-not-a-shapes-boundary.md)'s reading. **The clause an implementation answers by accident is where the subtraction goes.** p. 18 calls them a buffer against *losing actual Hit Points*; p. 17's Resistance says *"halve the damage"* and this says nothing of the kind. So the buffer absorbs where damage **lands** and never shrinks it — and that reading is load-bearing for two other rules, because p. 179 compels a Concentration save on damage *taken* and p. 18 charges a death-save failure for *any damage* at 0 Hit Points. A subtraction one line earlier silently answers both, and I wrote it that way first ([#413](https://github.com/eddiefiggie/srd-rules-engine/issues/413) records the reading). Two more clauses are refusals rather than behaviour: they **do not stack** and *"you decide"* which set to keep — so a grant over an existing set **refuses** rather than taking the larger, since p. 18's own example is 12 offered over 10 where keeping 10 is legal — and they are **not healing**, so a grant does not reset death saves the way `with_healing` does (p. 17 resets on regaining *Hit Points*). p. 177's Bloodied is a **derived read**: *while* it has half or fewer, so nothing stores it, and it is compared **doubled** because the document supplies no rounding rule for an odd maximum. [#414](https://github.com/eddiefiggie/srd-rules-engine/issues/414). Six corruption proofs. 124 of 210, 14 clauses. 318 clauses verified against the document. 2206 tests.
...
**124 of 210 shapes resolve today.** The other 86 are listed, not omitted; run
...
_Last updated: 2026-08-31 — build `08312026.5`._
