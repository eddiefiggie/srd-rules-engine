...
**Current build:** `08302026.34` — **Armour Class derives, and a character in Plate with a Shield is 20 rather than 28.** p. 177 makes base calculations *alternatives* — "you choose which calculation to use; you can't use more than one" — while p. 92's Shield is a `+2` **bonus** on top of whichever base won. The table invites the mistake, since that `+2` sits in the same column as Padded Armor's `11 + Dex modifier`. `ArmourClassBase` holds the shape every row of that table has — flat, optionally plus Dexterity, optionally capped — and the *numbers* stay ruleset content (R31). **The record was wrong about one clause and building it is what showed.** [0077](docs/decisions/0077-armour-class-is-a-chosen-base-and-bonuses.md) read a stat-block AC as another base calculation p. 177 permits: right about provenance, wrong about shape — a stat block states a *result* and p. 177's alternatives are *calculations*. Built as written it put the stated total beside worn armour as a rival, and the first demonstration showed a creature in Plate reading as its unarmoured value **silently**, because `armour_class` is a required field every ruleset already sets. They are the same claim at two levels of detail, so described armour wins and an undescribed creature keeps its number — no existing AC moves. **And a disclosure that could never fire.** `untrained-shield-still-grants-ac` was gated on `untrained_armour`, which reads **worn** items because p. 104 is about armour you wear; p. 92's Shield is **held**. So the clause was told to a creature in untrained plate holding no Shield and never to one holding an untrained Shield — pinned, disclosed, and unreachable by its own case, found by the first test that tried to watch it fire. [#367](https://github.com/eddiefiggie/srd-rules-engine/issues/367)'s withholding is now expressible; [#394](https://github.com/eddiefiggie/srd-rules-engine/issues/394) holds p. 177's genuine choice between two bases, which the engine must not make. Six corruption proofs. 118 of 210, 15 clauses. 295 clauses verified against the document. 2133 tests.
...
_Last updated: 2026-08-30 — build `08302026.34`._
