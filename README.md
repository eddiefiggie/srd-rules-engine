...
**Current build:** `08292026.27` — **a forced save is one mechanism, whatever compelled it.** [#321](https://github.com/eddiefiggie/srd-rules-engine/issues/321): p. 90's Topple compels a Constitution save on a hit, against *DC 8 plus the ability modifier used to make the attack roll and your Proficiency Bonus*, and applies **Prone** on a failure. The engine had one mechanism for a save a creature owes and has not rolled — [0036](docs/decisions/0036-a-fourth-occasion-owed-by-whoever-took-the-damage.md)'s `concentration_saves_owed` — and it was Concentration-shaped in its **payload** and general in its **design**. **0036 decides its own generalisation**, twice: clause 3 says *"two mechanisms with different cardinalities are two structures"*, and Topple's cardinality is the queue's — one save per triggering instance, owed by one creature. Clause 6 says a second queue would need draining from the same three phases, and *"three copies is how one gets missed"*. So one queue, `forced_saves_owed`, holding a `ForcedSave` that carries its own rule, ability, DC and derivation ([0048](docs/decisions/0048-a-forced-save-is-one-mechanism.md)). **The DC is computed where the trigger fires**, which 0036 clause 4 argued for the damage amount and Topple pushes further: the DC uses the ability the attacker *chose* for that roll — p. 89 leaves a Finesse wielder both — and the **attacker's** Proficiency Bonus, and by the time the loop rolls the save it has the target and no attacker at all. Not stale afterwards; **gone**. **Each rule decides when its own save is owed, and the queue decides nothing**: p. 179 owes on *"the damage taken"*, so an Immune creature owes none, while p. 90 owes on the **hit**, so an Immune creature owes one anyway. **The Proficiency Bonus applies without weapon proficiency** — p. 89 conditions the *attack roll's* bonus and p. 90's DC formula conditions nothing, and reading one across would be the inferred rule value R31 forbids. **Concentration's behaviour does not change**, and its own tests are the evidence. **Three proofs came back green first.** The clamp test used Strength 6 against a +2 bonus — which sums to exactly the base 8, so a floor was invisible; Strength 3 makes it 6. Neither resolver had anything declaring another rule's debt. And nothing exercised a **Topple** debt reaching the drain, where applying p. 179's staleness to every rule would silently drop every Topple save a non-concentrating creature owed — a compelled save that never happens, which leaves no trace because the target simply stays upright. **106 of 210**, masteries **3 of 8**. 229 clauses verified against the document. 10 corruptions, each red on its own test. 1704 tests.
...
**106 of 210 shapes resolve today.** The other 104 are listed, not omitted; run
...
_Last updated: 2026-08-29 — build `08292026.27`._
