...
**Current build:** `08282026.3` — **an effect now ends when its creator's Concentration does.** p. 179's first consequence had no mechanism: a Concentration held a spell *name* and, when broken, ended nothing. `Duration.concentration_of` is the link — a second **early-out** beside `save`, **not** a fifth `DurationKind`, because p. 179's own third sentence says the description states a maximum duration and one `kind` cannot hold both. "Concentration, up to 1 minute" is a one-minute span *and* an early-out; a model holding only one keeps a spell up for an hour because nobody was hit ([0037](docs/decisions/0037-a-concentration-is-an-early-out-not-an-axis.md), [#240](https://github.com/eddiefiggie/srd-rules-engine/issues/240)). **`retirable` became a two-part question**: p. 179 says "*If* the effect has a maximum duration", so a Concentration spell stating none is `UNTIL_REMOVED` and still ends — reading only the kind would have the engine disclose that it cannot end something it ends itself. **The record's clause 3 named the wrong place and the correction is recorded rather than quietly applied**: it put retirement in `with_concentration_ended`, which is two of the four routes — Incapacitated and death end Concentration in `Combatant.__post_init__` and never reach it. Demonstrated rather than argued: the clause-3 hook was built by hand and passes fifteen of nineteen tests, missing exactly those routes. It is a whole-state invariant instead, which is #238's lesson one level up — the rule that survives is the one every writer passes through, not the one every writer must remember. R32: retirement reaches a condition carrying a duration and nothing else, disclosed and guarded, because a spell's area or summoned creature is not modelled at all. **95 of 209**, unchanged. 1404 tests.
...
_Last updated: 2026-08-28 — build `08282026.3`._
