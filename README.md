...
**Current build:** `08252026.19` — **`can_see` answers at last, and the biggest thing it found is that the SRD does not.** [#166](https://github.com/eddiefiggie/srd-rules-engine/issues/166) asked for a shape for the three senses that are not light shifts. Building it turned up something larger: **the document never says an obstruction blocks sight, and never defines "line of sight"** — the term appears on six pages and is defined on none, and Total Cover is defined by what it does to *targeting* (p. 179). The proof is p. 173, where a spell's wall has to state that it "blocks line of sight"; if obstructions did that by default the clause would be redundant. So visibility is **three-valued**: `CAN_SEE`, `CANNOT_SEE`, and `UNSTATED` — which means *the document does not say*, not *we have not built it*, and never reads as a yes ([#188](https://github.com/eddiefiggie/srd-rules-engine/issues/188)). **Blindsight is the one sense the document bounds** (p. 177), so a wall rules it out and settles nothing else. Three more absolutes landed with it: a **Blinded** observer sees nothing ("You can't see") and Blindsight alone overrides that in terms; **Tremorsense never contributes**, because p. 190 says it "doesn't count as a form of sight"; and an **Invisible** target is `UNSTATED` under ordinary sight, because p. 184 says what it is concealed *from* rather than that it cannot be seen — Blindsight and Truesight are the two routes it answers for. `can_see` lives on `EncounterState` for the reason `creatures_in` does (0026). Six clauses added, 131 total. **Coverage 79 → 81**: Blindsight and Darkvision resolve in full; the other eight sight shapes stay unclaimed, each for a reason the guard now spells out. 1168 tests.
...
_Last updated: 2026-08-25 — build `08252026.19`._
