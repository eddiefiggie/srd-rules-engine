...
**Current build:** `08312026.19` — **a Cure Wounds resurrected a corpse, and nothing was looking.** p. 180: *"A dead creature has no Hit Points and **can't regain them** unless it is first revived by magic."* `with_healing` added the hit points and then cleared `dead` — because p. 17 resets both death-save counts *"when you regain any Hit Points"*, `dead` lives in the same structure as `successes` and `failures`, and the reset never asked whether the creature was alive to regain them. **The reset is right for the case it was written for**, which is why this survived: a dying creature stabilised by healing must stop making saves, and nothing in the code said the death was not meant to clear with the failures. It is also unreachable from a turn — the read surface offers no attack on a creature that `is_down` — so it took a caller invoking `with_healing` directly, which is the surface every healing spell will use. **A no-op rather than a refusal**: pouring a potion into a dead friend is a scene, and p. 180 answers it with *nothing happens* rather than an error. The refusal is **total**, and that is honest rather than complete — p. 180's exception is revival magic and there are no spells ([#21](https://github.com/eddiefiggie/srd-rules-engine/issues/21)), so Raise Dead does not exist to be excepted; whoever builds one finds the note at the line rather than a wall. **Found by a systematic pass over the 74 unclaimed shapes**, checking each against the tree rather than against my impression of it — after three consecutive builds ended in gates and I claimed the buildable surface was empty. [#438](https://github.com/eddiefiggie/srd-rules-engine/issues/438). Two corruption proofs, one per direction. 137 of 210, 14 clauses. 351 clauses verified against the document. 2287 tests.
...
**137 of 210 shapes resolve today.** The other 73 are listed, not omitted; run
...
_Last updated: 2026-08-31 — build `08312026.19`._
