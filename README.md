...
**Current build:** `08252026.25` — **the success criterion now says what is measured, and the report says what is not.** [#197](https://github.com/eddiefiggie/srd-rules-engine/issues/197). The plan's primary criterion read *no asserted outcome that did not originate in a Ruling*; the report measures that **every narration has a Ruling**, which is a weaker claim, and the previous build demonstrated the gap with a missed attack narrated as a kill over a clean report. **The code was right and the sentence was wrong**, so the sentence moved: five places in the plan, each amended in place with the old wording quoted, because a criterion that quietly narrows is the same overclaim in a smaller font. It now also **names what it gave up** — whether a narration stayed inside the bounds its Ruling gave it, which R7 states and deliberately does not enforce. `SessionReport.not_measured` carries the same three limits in typed form and `render` prints them **on every report, flags or none**: a footnote shown only on failures appears exactly where nobody needs it, and a clean report is the one place a reader is invited to conclude more than was measured. That is the habit this engine already has wherever an answer stops short — `unenforced_clauses`, `Visibility.UNSTATED`, an `unverified` table — applied to the instrument the whole product is judged by. Three guards proven red, including restoring the old criterion to the plan. Coverage unchanged at 81 of 211. 1200 tests.
...
_Last updated: 2026-08-25 — build `08252026.25`._
