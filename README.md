...
**Current build:** `08302026.4` — **a pin whose expected set is written out by hand is a pin over itself.** [#334](https://github.com/eddiefiggie/srd-rules-engine/issues/334), found while adding a disclosure in the previous build. `unenforced_clauses` is how R32 stays honest, and `tests/test_disclosures_are_pinned.py` claims to hold **every** one. Its condition half does — it derives the set from `EFFECTS`. Its other half compared a literal set against a constant built from **the same four names**, which is an assertion true by construction over the thing it claims to check: it caught an edit to the pin and was blind to a disclosure that existed in the source and had never been added. **`VERBAL_UNCHECKED` sat in exactly that state from the day it shipped** — p. 105 refuses a Verbal component to a creature gagged or in magical silence, the engine enforces neither, the read surface has said so all along, and the pin had never heard of it. The repair derives: an AST walk over every `core` module collects what is appended to the disclosure list and resolves each name against the module that wrote it. **It raises on an argument shape it cannot read** rather than skipping one, because a walk that silently ignores what it cannot parse goes quiet in precisely the way the assertion it replaces did — and it fails when it finds no assembly site at all, so renaming the local cannot turn the guard off by accident. Five corruptions hold it, and a sixth is the one that matters: the **predecessor** assertion, run against the same corrupted source, stays green. That is the finding reproduced rather than asserted. 112 of 210, masteries 7 of 8. 244 clauses verified against the document. 1785 tests.
...
_Last updated: 2026-08-30 — build `08302026.4`._
