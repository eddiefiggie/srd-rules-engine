...
**Current build:** `08252026.26` — **82 of 211: Rituals, spell preparation, and a benefit the Long Rest was silently not applying.** [#19](https://github.com/eddiefiggie/srd-rules-engine/issues/19). p. 104 says *"Finishing a Long Rest restores any expended spell slots"* — and it says it on **p. 104, not p. 185**, so the Long Rest's own entry never mentions slots. `SpellSlots.restored()` had existed since #95 with a docstring saying nothing triggered it *because there was no rest*. There was a rest from #185, and this went a whole build with the operation and the occasion both present and nothing joining them — which is the shape a disclosed gap takes when its reason expires and the disclosure does not. **Rituals** (p. 187) are the shape that moves the count, and the clause an implementer drops is the one the document draws for itself: a Ritual expends no slot, *"which means the ritual version of a spell can't be cast at a higher level"* — an engine that accepted a level there would hand out free upcasting and look like it had a feature. **Preparation is one set, not two**: p. 104 separates always-prepared spells from the changeable list only for the *change limit*, and that limit is per-class data this engine ships none of, exactly as it ships no slot table. Three clauses added (136 total), three guards proven red. **Also found, and filed rather than folded in**: `core.__all__` names three symbols it never imports, so `from core import *` raises — all three are claimed as implemented capability and reachable, under R34, only by the route that does not work ([#200](https://github.com/eddiefiggie/srd-rules-engine/issues/200)). 1208 tests.
...
**82 of 211 shapes resolve today.** The other 129 are listed, not omitted; run
...
_Last updated: 2026-08-25 — build `08252026.26`._
