...
**Current build:** `08232026.20` — **the six areas of effect, resolved in feet without a grid.** Sphere, Cube, Cylinder, Cone, Line and Emanation, all on decision `0014`'s integer geometry — a Cone's widening becomes `4·(|v|²·|d|² − dot²) ≤ dot²`, so no square root is taken and no boundary is decided by a rounded value. **Whether the origin sits in its own area differs by shape**: Sphere and Cylinder include it, Cone, Cube, Line and Emanation do not, and one answer applied to all six would be right half the time. A Cone's stated width is the full spread rather than a radius, which the document's own example separates. **Obstructions are not modelled**, so these describe unobstructed volume and a creature behind a wall is reported as caught — the largest known gap in R16, disclosed in the module and filed as #91. Inventory coverage 44 → 51 of 211.
