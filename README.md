...
**Current build:** `08252026.21` — **0030 names a rule this project has been following for months without writing down, and following in two opposite directions.** `core.reactions` **withholds** an Opportunity Attack because it cannot check p. 185's "that you can see"; `core.conditions` **applies** Frightened's Disadvantage though it cannot check p. 182's "within line of sight". Both are right, neither summary generalises, and the next site would have been decided by whichever precedent its author read first. The rule is neither *apply* nor *withhold*: **an unanswerable qualifier resolves in whichever direction cannot manufacture an outcome.** Omitting an effect the rules required is a failure to act; producing one they never granted invents something that was never in the world, and only the second is a defect here. **The test is asked of the outcome, not of the creature** — which is the part that would have gone wrong, because "be generous to the creature" gives the right answer for Invisible and the wrong one for the Opportunity Attack. Two guards on the record itself: the read surface still reports all three visibility values, so a narrator can see the engine chose rather than knew; and **a clause leaves `unenforced_clauses` when it is enforced, not when it becomes answerable**, so `can_see` growing sharper cannot make the engine's own gap-count report progress that did not happen. **Decided, not built, and the reason is new**: Frightened's qualifier needs the *source of fear* stored, and `grappler_id` is the only source this engine tracks ([#192](https://github.com/eddiefiggie/srd-rules-engine/issues/192), [#193](https://github.com/eddiefiggie/srd-rules-engine/issues/193)). Coverage unchanged at 81 of 211. 1174 tests.
...
_Last updated: 2026-08-25 — build `08252026.21`._
