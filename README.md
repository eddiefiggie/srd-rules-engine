...
**Current build:** `08232026.23` — **spell slots, spell save DCs and spell attacks, and Concentration.** A spell expends a slot of its own level *or higher*, a cantrip expends none, and casting with nothing left is refused rather than improvised. Concentration is the reason this exists: starting a second concentration spell **ends the first at the moment casting starts** — modelled as replacement, so holding two is unrepresentable rather than discouraged. Damage forces a Constitution save at 10 or half the damage taken, *both* bounded — without the floor small hits are free, without the cap a big one is unmakeable. Incapacitated ends it with no save, read from the condition set so the two cannot disagree. No slot table ships: p. 26's is class content, and a copy here would read exactly like a verified rule. Inventory coverage 71 → 76 of 211.
