...
**Current build:** `08252026.28` — **an arrow flew through a stone wall, and nothing in `core.combat` had ever looked at cover.** [#20](https://github.com/eddiefiggie/srd-rules-engine/issues/20). p. 179 says Total Cover *"can't be targeted directly"*, and the attack resolver contained no reference to cover at all — the geometry was ready from #91, the walls have been state since [0026](docs/decisions/0026-terrain-enters-as-state.md), and what was missing was anyone asking. It is a **refusal, not a penalty**, for the reason a shot beyond long range is refused: the rules forbid the attack, so a ruling for it would be an outcome for something that never happened. Half and Three-Quarters Cover stay unmodelled and `Cover.bonus` still has no caller, because the document supplies no method for measuring what fraction of a target is covered. **Spell range** is the other half (p. 105): three forms, and only one is a number — Touch is the *caster's reach* and Self is the caster, so neither may carry a distance this engine would have to invent. `spell_reaches` applies two independent tests, because p. 106 adds a **clear path** — a wall five feet away defeats a 120-foot spell. Also fixed: a test that **pinned a stale disclosure**. It asserted the module docstring said "Long Rest recovery has no trigger"; #19 gave it one, and the guard held the false sentence in place for a build. A guard on a disclosure keeps it honest only if it moves when the fact does. Two clauses added (138 total), three guards proven red. Coverage unchanged at 82 of 211 — this refuses what the rules forbid rather than resolving a new shape. 1221 tests.
...
_Last updated: 2026-08-25 — build `08252026.28`._
