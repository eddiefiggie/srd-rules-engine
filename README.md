...
**Current build:** `08282026.9` — **a weapon is an item, and proficiency was never the weapon's.** [0040](docs/decisions/0040-a-weapon-is-an-item-and-proficiency-is-the-wielders.md) settles how a weapon becomes something a creature holds ([#262](https://github.com/eddiefiggie/srd-rules-engine/issues/262)), and the tree named the gap itself: `_attack_detail` declines to gate attacks on range because "the read surface does not know which weapon an attack will use" — a reason that ends the moment the creature holds one. **The clause this record exists for is proficiency.** `Weapon.proficient` is a field on the weapon; p. 89 makes it a fact about the **wielder** — "**you** must have proficiency with it" — and a creature-weapon relation stored on the weapon works exactly while a weapon belongs to one wielder, which is the closure world this ends. It would have travelled across the refactor unexamined, because it is a field that already works, and it fails in the direction that *adds* a bonus. **And no wrapper**, which is where the pattern would have misled: 0038 wrapped a spell's resolver because a spell's *effect* comes from outside the engine and a ruleset could forget the costs. An attack's effect is stated by the document and shipped here, so there is nothing outside the engine to wrap — the wrapper is a defence against a specific hazard, not a house style. Improvised is a **use** and not an object (p. 183: a real weapon "wielded in a way contrary to its design"), and its damage type is one the document leaves to a person. **95 of 209**, unchanged: a record decides and builds nothing. 1470 tests.
...
_Last updated: 2026-08-28 — build `08282026.9`._
