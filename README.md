...
**Current build:** `08292026.26` — **p. 89's extra attack, carried by the Attack action.** [#320](https://github.com/eddiefiggie/srd-rules-engine/issues/320): p. 90's Nick — *"When you make the extra attack of the Light property, you can make it as part of the Attack action **instead of** as a Bonus Action"*. It introduces no new attack and re-routes one that already shipped, so what is worth asserting is what stopped being charged and what stayed the same. **The property belongs to the weapon making the extra attack**, not the one that bought it: p. 89 says that attack must use a *different* Light weapon, every mastery describes an attack made *with that weapon*, and p. 91's table settles it — all four weapons carrying Nick are **Light**, and no weapon that is not Light has it. **"Instead of" needed a new record.** Until now the Bonus Action spend *was* the bookkeeping for p. 89's one extra attack — a second was refused because no Bonus Action remained — and a Nick attack spends nothing, so nothing stopped a creature taking one by each route. `extra_attacks_this_turn` is a fourth per-turn structure with a fourth meaning (0036 clause 3, a fourth time). **Both routes are offered**: p. 90 says "you **can**", granting an option rather than withdrawing one, and presenting only the cheaper of two identical outcomes would be the engine deciding a tactical question R18 leaves to the agent. **Five corruption proofs came back green first**, and each found something real. One was **code**: the `is_nick` skip in the action cost was unreachable — a Nick attack always arrives after an Attack action whose tally the clause beside it already reads — so it read as load-bearing and was removed rather than left unprovable. The other four were **tests asserting nothing**: the damage exception was read off the read surface's `detail`, which recomputes it independently, so `is_extra`→`is_bonus` stayed green; the turn-clearing test exercised one of `advanced_turn`'s two sites, leaving the **wrap** — where the last creature in initiative ends its turn — unguarded; the resolver's own refusal had nothing declaring a Nick key the surface had withheld; and the Light requirement was vacuous under a single-weapon fixture, `weapon.id in used` excluding it either way. **105 of 210**, masteries **2 of 8**. 11 corruptions, each red on its own test. 1686 tests.
...
**105 of 210 shapes resolve today.** The other 105 are listed, not omitted; run
...
_Last updated: 2026-08-29 — build `08292026.26`._
