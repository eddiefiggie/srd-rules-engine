...
**Current build:** `08242026.17` — **what provokes an Opportunity Attack is now computed, and every offer is withheld on purpose.** p. 185's sentence is asserted in the verifier and every clause in it is answerable against state this engine holds — reach, the Reaction budget, Incapacitated, Disengage — except one: the attack fires on a mover **"that you can see"**, and sight is the mapping that ships empty until [#150](https://github.com/eddiefiggie/srd-rules-engine/issues/150) ([#16](https://github.com/eddiefiggie/srd-rules-engine/issues/16), 0015). So `core.reactions` computes who *would* be provoked and names the clause it is waiting on, and `disengaged` is finally consumed by something after existing unread since the action economy landed. **The withholding runs opposite to the choice `core.conditions` makes for Frightened, for the same reason**: applying a penalty whose qualifier cannot be checked cannot invent a success, but firing an attack the rules may not grant produces damage out of nothing — an invention rather than an omission. A test pins that so it cannot be quietly reversed by someone reading the withholding as an oversight. The `opportunity-attacks` shape stays unimplemented, because a trigger that cannot fire has resolved nothing: coverage is 76 of 211. Also fixed: `core.combat` claimed criticals were "deliberately absent" for several builds after they landed. 1025 tests.
...
_Last updated: 2026-08-24 — build `08242026.17`._
