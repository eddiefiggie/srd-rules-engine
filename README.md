...
**Current build:** `08302026.11` — **a keyword per issue, not one keyword and a list.** A sweep of the open issues found **three fixed, shipped and still open** — [#324](https://github.com/eddiefiggie/srd-rules-engine/issues/324), [#335](https://github.com/eddiefiggie/srd-rules-engine/issues/335) and [#345](https://github.com/eddiefiggie/srd-rules-engine/issues/345) — which is precisely the failure `AGENTS.md` warns about, in the words it warns about it with. The cause: `Closes #349, #345, #324` closes **#349 and links the other two**, because GitHub binds the keyword to the reference that follows it and to nothing further along. The standing rule said *a keyword, not a bare `#N`* and does not reach this — a bare `#N` looks like what it is, while a list after a keyword reads as though every reference is handled. **And a markdown-linked reference does not close either**: `Closes [#345](…/issues/345)` reads correctly to a person and binds to nothing, which makes a PR body written in this repository's own prose style exactly where it hides. All three are now closed with the evidence, and `AGENTS.md` carries both refinements. Two issues were also filed from a second finding: three consecutive fidelity fixes moved no coverage figure, because the instrument counts **shapes** and not **clauses** ([#356](https://github.com/eddiefiggie/srd-rules-engine/issues/356)) — and the guard that issue proposes, run once by hand, found **seven** `ConditionEffects` fields populated from the glossary and read by nothing, none of them disclosed ([#357](https://github.com/eddiefiggie/srd-rules-engine/issues/357)). Two of the seven change outcomes rather than probabilities: a Paralyzed creature struck from within five feet is not taking p. 186's Critical Hit, and a Petrified one is not getting its Resistance to all damage. 116 of 210. 274 clauses verified against the document. 1926 tests, unchanged — this build ships no code.
...
_Last updated: 2026-08-30 — build `08302026.11`._
