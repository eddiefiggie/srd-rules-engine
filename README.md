...
**Current build:** `08312026.11` — **a disclosure that was half right, and the half that was wrong had been sitting there since #138.** `Combatant.skills` said p. 182's Expertise *"is a class feature this engine ships no data for"* — true of **which skills**, false of the **mechanic**. Which skills is character data, exactly as `skills` itself is and that field has always been caller-supplied; the doubling is stated on p. 182 and is engine arithmetic. It doubles the **Proficiency Bonus, not the check** — scaling whatever has been added up so far is wrong by the ability modifier every time, and the test uses Wisdom 14 so the two numbers can never coincide. **Three of p. 182's four sentences constrain rather than compute**: Expertise is granted only *"in one skill in which you have proficiency"*, so one without it is **refused** rather than quietly worth nothing — `check_bonus` would otherwise return the bare ability modifier, which reads as "no Expertise" and hides a creature the document cannot describe; and it *"can't"* be held twice, which a **set** makes unrepresentable rather than needing a guard nobody would think to write. Its fourth clause has no antecedent: nothing else here doubles a Proficiency Bonus. One change, **six consumers** — Search, Study, Influence, Perception, the grapple escape and the read surface all reach `check_bonus`. [#424](https://github.com/eddiefiggie/srd-rules-engine/issues/424). Three corruption proofs. 132 of 210, 14 clauses. 337 clauses verified against the document. 2239 tests.
...
**132 of 210 shapes resolve today.** The other 78 are listed, not omitted; run
...
_Last updated: 2026-08-31 — build `08312026.11`._
