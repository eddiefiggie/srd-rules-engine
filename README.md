...
**Current build:** `09062026.2` — **a count of days in which nothing happened, and it is the engine's to keep.** [#401](https://github.com/eddiefiggie/srd-rules-engine/issues/401) was p. 185's second rule: *"A creature that eats nothing for 5 days automatically gains 1 Exhaustion level at the end of the fifth day as well as an additional level at the end of each subsequent day without food."* [0081](docs/decisions/0081-a-campaign-days-end-is-the-fifth-occasion.md) had built the first sentence — the DC 10 save for eating too little — and left this one, because it needed a kind of state nothing held: a running count of days in which something did *not* happen. **It is `Hazards.days_without_food`**, beside `burning` and `suffocating`, and unlike them it is never set by a caller: it is arithmetic over the food the caller already states at each day's end, and a caller that set it would be choosing when a creature starves. **A single mouthful breaks the run** — "eats nothing" is the condition — even a quarter-pound that also compels the save, so one day's food is read twice for two questions. **The run does not latch**: a level on the fifth day and on every day after, until level six is death and the count carries on past a creature that gains nothing more. A day the caller says nothing about neither advances the count nor resets it. No die, so `end_day` produces no ruling for it; the level carries `MALNUTRITION_RULE_ID` and 0028's lock holds it against a Long Rest. **What still does not exist is the unlock** — p. 185's "until the creature eats the full amount of food required for a day", and p. 181's twin — which is [#461](https://github.com/eddiefiggie/srd-rules-engine/issues/461). `malnutrition` is claimed now that both sentences are built. [0088](docs/decisions/0088-a-run-of-days-without-food-is-a-hazard-the-creature-carries.md). Twelve corruption proofs. 145 of 210, 14 clauses. 381 clauses verified against the document. 2426 tests.
...
**145 of 210 shapes resolve today.** The other 65 are listed, not omitted; run
...
_Last updated: 2026-09-06 — build `09062026.2`._
