...
**Current build:** `08252026.20` — **0029: whether a wall blocks sight is a property of the wall, because the document answers it per barrier and answers it both ways.** [#188](https://github.com/eddiefiggie/srd-rules-engine/issues/188) asked whether to declare a house rule, since the SRD never says an obstruction blocks sight. Reading all five wall spells says a house rule would have been wrong whichever way it went: **Wall of Force is "An Invisible wall of force" (p. 172) and Wall of Thorns "blocks line of sight" (p. 173)** — both give Total Cover, three pages apart. So `Obstruction.blocks_sight` is a field, **`None` until somebody says**, and `None` still means `UNSTATED`: this engine supplies no default because the document supplies none, and Wall of Stone, Wall of Ice and Wall of Fire say nothing either way. **Opaque beats unstated beats transparent** — one barrier known to block sight settles it, and a wall nobody described cannot borrow its neighbour's answer, which would let a caller see through a wall by standing a pane of glass beside it. It converts a global refusal into a local one: not "this engine cannot answer sight through cover" but "nobody has said what *this* wall is made of", which a caller closes by describing their world rather than working around. **It adds no rule**, which is what keeps it inside R31 where #188's house-rule option was not. `core.areas` ignores the field entirely: p. 177 blocks a line of *effect*, and a Fireball is stopped by Wall of Force while a glance is not. Corrects the previous build's claim that p. 173 proves no general rule exists — Wall of Stone is plainly opaque and says nothing, so that clause reads as well as a clarification; Wall of Force is the evidence that survives. Coverage unchanged at 81 of 211. 1174 tests.
...
**81 of 211 shapes resolve today.** The other 130 are listed, not omitted; run
...
_Last updated: 2026-08-25 — build `08252026.20`._
