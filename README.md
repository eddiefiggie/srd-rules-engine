...
**Current build:** `08252026.22` — **a clause leaves `unenforced_clauses` for the first time.** Frightened's Disadvantage holds "while the source of fear is within line of sight" (p. 182). `can_see` could answer that after 0029 and nothing could **ask** it: this engine recorded no source of fear — `grappler_id` was the only source it kept, for the only other conditional clause it enforced. So the source became state, and **generally**: a mapping of condition to a **set** of sources, because p. 179 says "a condition doesn't stack with itself; a recipient either has a condition or doesn't" — a creature frightened by two monsters holds *one* condition with two sources. That same sentence names Exhaustion as the exception, which is why [0028](docs/decisions/0028-a-level-carries-the-rule-that-caused-it.md) gave it levels. **Any source in sight is enough**: p. 182's singular describes one application, and with two the document does not say which it meant. **Not knowing keeps the penalty** — [0030](docs/decisions/0030-an-unanswerable-qualifier-resolves-away-from-invention.md) clause 1, so an undescribed wall and an unrecorded source both leave the Disadvantage where it was. `line-of-sight-qualifier` is now enforced and has left the list, which is 0030 clause 5 working as intended: the entry moved when the **behaviour** changed, not when `can_see` got sharper. `Effect.grappler_id` becomes `source_id` and the ledger's `grappler` becomes `source` — `RULING_VERSION` 6, a rename rather than an addition, and still not a compat move. Coverage unchanged at 81 of 211. 1187 tests.
...
_Last updated: 2026-08-25 — build `08252026.22`._
