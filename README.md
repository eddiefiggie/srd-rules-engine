...
**Current build:** `08242026.10` — **the HTTP adapter, so all three of R34's transports ship ([#133](https://github.com/eddiefiggie/srd-rules-engine/issues/133)).** The design question `0016` left open needed no record after all: `AGENTS.md` declines multiplayer and shared sessions as a **non-goal**, so **one server holds one session** — a registry keyed by a caller-supplied id would have built the multi-user surface the product declined, in the one layer nobody inspects for it. MCP had already answered it the same way. **Reads are GET and everything else is POST**, which makes R19 — read calls mutate nothing and record nothing — checkable from the method rather than asserted in prose. `handle(method, path, body)` is the adapter and the stdlib server is a thin binding over it, so every route is testable without a socket and this transport needs **no dependency**; it binds to loopback, because a live campaign is not a thing to expose by accident. The JSON rendering now lives in `adapters.surface` shared by MCP and HTTP — two copies of the payload an agent decides from is not where a discrepancy should be discovered. 965 tests.
...
_Last updated: 2026-08-24 — build `08242026.10`._
