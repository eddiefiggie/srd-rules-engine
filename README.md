...
**Current build:** `08282026.6` — **the action economy was claimed and reachable by nothing.** Dash, Dodge and Disengage had been offered by the read surface since it existed and did **nothing**: declaring one returned `no-test-accepted`, no effects, no Action spent, no movement granted — while `core.combat` had been reading `is_dodging` the whole time, so the *consequence* was wired and the **occasion** was missing. That is the third time this repository has found that exact shape ([#252](https://github.com/eddiefiggie/srd-rules-engine/issues/252); `concentration_save_dc` before #215, `Concentration` before #235). All three now resolve through rulings, and **an attack costs the Action** — p. 176 gives a turn one action and p. 177 makes an attack one, and until #248 nothing an adjudication did cost anything at all. **p. 180's choice of speed is enumerated** rather than assumed: one Dash offer per speed the creature has, because "You choose which speed to use each time you take it" — and crawling is excluded, which is the entry that would have slipped in, since `for_mode` answers Speed for it and would have offered a "Dash (crawl)" the document does not describe. **Four claimed shapes named symbols that do not exist** — `core.state.action`, `core.combat.attack`, `core.combat.initiative`, `core.combat.hit_points`. The guards compared `ENGINE_SHAPES`' *keys* against the inventory in both directions and nothing checked the *values*, so a shape could count toward the figure while pointing at nothing. Fixed, and guarded. **95 of 209**, unchanged — and now true for `action`, `attack`, `dash`, `dodge` and `disengage`, which were claimed while unreachable. 185 clauses verified. 1451 tests.
...
_Last updated: 2026-08-28 — build `08282026.6`._
