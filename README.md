...
**Current build:** `08312026.20` — **the second die 0059 built finally has a third user.** p. 189's Surprise is one sentence: *"caught unawares by the start of combat... causes it to have **Disadvantage on its Initiative roll**"* — one consequence, and the entry states nothing else, so reading it as a lost turn would be the direction R31 names. [0059](docs/decisions/0059-initiative-draws-a-pair-for-everyone.md) had already built the hard part, drawing **two dice for every combatant** so Advantage on Initiative was expressible without a creature's seed offset depending on its neighbours' conditions; its docstring says the second die is *"simply unused where nothing modifies the roll"*, and until now only two conditions modified it. **Surprise is the first source that is not a condition** — it has its own glossary entry and is not among p. 179's fifteen, so it lives on the creature and the sources are combined at the roll through p. 8's cancellation: an Invisible creature that is surprised rolls **flat**. **A test caught me writing "found rather than assumed" about a seed I had assumed**: seed 3 gives the same face twice, so the pick was invisible and the assertion read `7 < 7`. Seed 4 was found by searching, and the helper is asserted directly as well — a roll can only show which die was picked, and two sources agreeing by accident would look identical. [#440](https://github.com/eddiefiggie/srd-rules-engine/issues/440). Two corruption proofs. 138 of 210, 14 clauses. 352 clauses verified against the document. 2292 tests.
...
**138 of 210 shapes resolve today.** The other 72 are listed, not omitted; run
...
_Last updated: 2026-08-31 — build `08312026.20`._
