...
**Current build:** `08292026.10` — **one Action, three rolls, and one swap.** [#289](https://github.com/eddiefiggie/srd-rules-engine/issues/289) builds [0043](docs/decisions/0043-one-action-several-attacks-and-one-swap.md): p. 257's Multiattack is the Attack action buying several rolls, so the Action is charged **once** — the line `attack_resolver` has carried a comment about since the economy landed, naming this exact feature and saying which line would change. **A disclosed limitation found by reading the code that documented it.** 0042 clause 6's silence is now half-answered in code: at most **one swap per turn** whatever the attack count, taken as the *intersection* of two readings the document does not choose between, and named as the engine's cap rather than a printed rule an agent could cite. **The build found the record conflated two things**: clause 4 argued for a count over a set, which is right for the attack tally and wrong for the swap — the swap's cap is one, so a set of actors answers it exactly and a count would carry a number nothing may read past 1. **And a bug the tests caught**: `attacks_remaining` counts rolls and cannot say what the Action *bought*, so a creature that spent its Action on Dodge had rolls remaining by arithmetic and was offered an attack. Having already attacked this turn is what says the Action went here. p. 191's Unconscious drop deliberately does **not** spend p. 177's allowance, which is why `WEAPON_SWAPPED` is its own effect. **98 of 209 -> 99 of 209** — `multiattack` is the first Monsters shape to resolve, and it unblocks [#271](https://github.com/eddiefiggie/srd-rules-engine/issues/271). 214 clauses. 1563 tests.
...
**99 of 209 shapes resolve today.** The other 110 are listed, not omitted; run
...
**Next up:** [#271](https://github.com/eddiefiggie/srd-rules-engine/issues/271) — p. 90's Loading, whose "regardless of the number of
attacks you can normally make" finally has something to bite on now that one Action can
buy several rolls. Then [#288](https://github.com/eddiefiggie/srd-rules-engine/issues/288), which inherits a narrower question than it was
filed with: not how p. 13 and p. 177 compose, but whether the one swap this engine
permits also spends p. 13's free interaction.
...
_Last updated: 2026-08-29 — build `08292026.10`._
