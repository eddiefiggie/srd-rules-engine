...
**Current build:** `08252026.23` — **one sentence, two halves, and they default in opposite directions.** p. 184 gives an Invisible creature Disadvantage on attacks against it *and* Advantage on its own, then takes both back "if a creature can somehow see you". [#193](https://github.com/eddiefiggie/srd-rules-engine/issues/193) enforces that exception, and it is the sharpest demonstration of [0030](docs/decisions/0030-an-unanswerable-qualifier-resolves-away-from-invention.md) clause 3 in the tree: the Disadvantage is **kept** when sight is unknown, because dropping one the rules require makes an attacker hit more often; the Advantage is **withheld**, because granting one they may not grant makes the invisible creature hit more often. Both manufacture damage, from opposite ends. **Kindness to the creature would have moved them together and been wrong once** — the test is asked of the outcome, not of the creature. Each needs *certainty* to leave its default, so an `UNSTATED` view changes nothing. **This removes an Advantage the engine granted unconditionally before**, which is a behaviour change in the safe direction rather than a fix to a crash. `unless-seen-exception` has left `unenforced_clauses` — the second entry ever to go, and the list is down to one on Invisible. `concealed-from-effects-requiring-sight` stays, because nothing marks which effects require sight: a resolver knows whether its own rule needs it and records it nowhere. Four guards proven red, two of them only after the resolver's own wiring got tests — the first attempt stayed green and said so. Coverage unchanged at 81 of 211. 1194 tests.
...
_Last updated: 2026-08-25 — build `08252026.23`._
