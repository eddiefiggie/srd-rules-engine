...
**Current build:** `08242026.14` — **the sight subsystem has a design, and the part of it that would have been invented from memory ships empty on purpose.** Ten shapes, nothing built toward any of them, and four condition clauses already waiting ([#138](https://github.com/eddiefiggie/srd-rules-engine/issues/138)) — so [0025](docs/decisions/0025-sight-is-a-relation-over-stored-state.md) settles the four questions before code rather than after. **Light is state, never a query argument**: an input the caller hands over at the moment an outcome is computed is an input the caller *chooses*, and choosing between Bright Light and Darkness is choosing between Advantage and Disadvantage. Visibility is *derived* from stored senses and stored light, which 0021 clause 3's re-derivation warning permits because neither input comes from the caller. The mapping that resolves the two into obscurement is a rule value nine times over, and **not one of those pages is asserted anywhere in this repository** ([#150](https://github.com/eddiefiggie/srd-rules-engine/issues/150)) — so it ships in the `unverified` state with no rows and refuses, the way `core.spellcasting` ships no slot table. Two shapes turned out to belong elsewhere and were split out rather than carried: Telepathy is a communication channel, not sight ([#149](https://github.com/eddiefiggie/srd-rules-engine/issues/149)), and the record knowingly leaves obstructions entering by a different route than light ([#151](https://github.com/eddiefiggie/srd-rules-engine/issues/151)). A design pass moves no shape, so coverage stands at 76 of 211. 972 tests.
...
| **v1.0 — mechanics** | **76 of 211 effect shapes.** Every entry in the inventory ([#14](https://github.com/eddiefiggie/srd-rules-engine/issues/14)) must resolve. Conditions are 15/15 and the d20 test 12/14; senses and light are **0 of 10** and are a coupled subsystem nothing has been built toward — its design is settled ([0025](docs/decisions/0025-sight-is-a-relation-over-stored-state.md)) and its implementation waits on pages nobody here has read ([#138](https://github.com/eddiefiggie/srd-rules-engine/issues/138), [#150](https://github.com/eddiefiggie/srd-rules-engine/issues/150)). |
...
_Last updated: 2026-08-24 — build `08242026.14`._
