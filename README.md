...
**Current build:** `08312026.2` — **the resource three Glossary shapes were waiting on, and none of them is claimed.** p. 183 gives Hit Point Dice one mechanic — *"A creature can spend Hit Dice during a Short Rest to regain Hit Points"* — and p. 185 restores them in the **same sentence** as the hit points: *"You regain all lost Hit Points and all spent Hit Point Dice."* `EncounterState.with_long_rest` had been doing half that sentence since #185 and saying so. `HitDice` is the count, and the Long Rest now returns all of them ([#407](https://github.com/eddiefiggie/srd-rules-engine/issues/407)). **No shape moves, deliberately**: p. 183 states the mechanic as the *spend*, and the Short Rest that spends one is an occasion this engine does not have — filed as a gate ([#406](https://github.com/eddiefiggie/srd-rules-engine/issues/406)), because a Short Rest is neither of the two occasion kinds that exist. `end_day` and Concentration **drain** a queue the creature cannot refuse; a declaration slot asks **once**. p. 187 says *"You can decide to spend an additional Hit Point Die **after each roll**"*, which is an offer repeated until the caller stops — and each roll is an outcome, so each is a Ruling (R1, R4). Claiming `hit-point-dice` on the resource alone would be the half-claim #371 and #264 each found, so the counter stands still while a route lands, exactly as the note below the table describes. **No table of die sizes ships**: p. 183 defers the size to Character Creation and to stat blocks, so compiling one would be R31's inferred rule value — the reason `SpellSlots` ships no table of slot counts. `None` hit dice mean **unrecorded**, not zero, so a Long Rest invents none (0051's reading of Size). Six corruption proofs, **two of which failed first**: one corruption missed the refusal it aimed at, and one assertion was **vacuous** — `replace(creature, hit_dice=...)` leaves hit points alone by construction, so the test claiming spending heals nobody stayed green against every corruption of the code it named. It is a field pin now. 302 clauses verified against the document, up from 300. 119 of 210, 14 clauses. 2173 tests.
...
_Last updated: 2026-08-31 — build `08312026.2`._
