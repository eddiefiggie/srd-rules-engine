...
**Current build:** `08252026.18` — **Exhaustion is a mechanic a creature can recover from, which it was not.** Until now nothing finished a Long Rest, so [0028](docs/decisions/0028-a-level-carries-the-rule-that-caused-it.md)'s general removal rule had nowhere to attach and a creature that marched through the night held a level it could never lose. `EncounterState.with_long_rest` is clauses 3, 4 and 6 ([#185](https://github.com/eddiefiggie/srd-rules-engine/issues/185)): it restores every lost hit point, drops **one** Exhaustion level, and **refuses a creature at 0 hit points** — p. 185's "To start a Long Rest, you must have at least 1 Hit Point" is the precondition an implementation drops, because every other benefit reads as unconditional and dropping it lets a dying creature rest itself to full. **A locked level is not a candidate**: dehydration's and malnutrition's rule ids are named ahead of the hazards themselves, so a rest takes an unlocked level *or none*, and a creature holding only locked ones loses nothing. **A real bug fell out of it.** `Conditions` re-derived `applied` from an already-closed `held` whenever `applied` was legitimately empty, so removing a creature's last level left the Exhaustion condition behind — outliving the level p. 181 says ends it. It was unreachable until a Long Rest could take one. Three clauses added (125 total). Four guards proven red, including that fix. **Coverage stays at 79 of 211**: `long-rest` is not claimed, because Hit Point Dice, a reduced maximum, ability scores, feature recharge, the 8-hour duration, the 16-hour gap and every interruption are still absent. 1143 tests.
...
_Last updated: 2026-08-25 — build `08252026.18`._
