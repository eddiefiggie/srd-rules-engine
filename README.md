...
**Current build:** `08252026.30` — **89 of 211: what keeps a creature in the air, and the page that connects it to being grappled.** p. 182: *"While flying, you fall if you have the Incapacitated or Prone condition or your Fly Speed is reduced to 0. You can stay aloft in those circumstances if you can hover."* Three triggers, and **two of them arrive by completely different routes** — which is what makes a half-implementation look finished. Incapacitated drops a creature whose Fly Speed is untouched: a **Stunned flyer still has its 60 feet** and still falls. A Fly Speed "reduced to 0", meanwhile, is almost never reduced directly — it goes to 0 because *Speed* went to 0 and **p. 188 carries the change across**: *"if your Speed is reduced to 0 and you have a Climb Speed, your Climb Speed is also reduced to 0."* So a **Grappled flyer falls**, and nothing in Grappled's own text mentions flight. This engine applied conditions to the walking Speed alone, so that creature flew away at 60 feet — and every test about Grappled and every test about flying passed. `Conditions.speeds_after` now applies both feet-denominated effects to every speed; the **proportional** case is left alone and disclosed, because p. 188 contradicts itself there ([#205](https://github.com/eddiefiggie/srd-rules-engine/issues/205)): "an equal amount" subtracts feet while "your Fly Speed is also halved" scales, and they differ the moment two speeds do. `with_movement` also now **refuses to fly or burrow without the speed** — the SRD prices climbing, swimming and crawling for a creature that lacks one and prices these nowhere, and [0030](docs/decisions/0030-an-unanswerable-qualifier-resolves-away-from-invention.md) clause 1 says the silence resolves toward refusing. **Two of the five shapes were already implemented and merely unclaimed**: `climb-speed` and `swim-speed` say only that the speed removes the extra movement cost, which `movement_cost` has enforced since #17 — the instrument that measures this engine's gaps had two false ones. `burrow-speed` stays unclaimed: p. 178 refuses solid rock "unless the creature has a trait", and neither the medium nor the trait is modelled. Five clauses added (147 total). 1261 tests.
...
| **v1.0 — mechanics** | **89 of 211 effect shapes.** Every entry in the inventory ([#14](https://github.com/eddiefiggie/srd-rules-engine/issues/14)) must resolve. Conditions are 15/15 and the d20 test 12/14; senses and light are **2 of 10**, but no longer for the same reason — the nine pages are read and asserted ([#150](https://github.com/eddiefiggie/srd-rules-engine/issues/150)), so the engine can say what Darkvision does to a light level. Nothing consumes the answer yet: no roll is modified and `can_see` refuses, and a value nothing reads has resolved nothing ([0025](docs/decisions/0025-sight-is-a-relation-over-stored-state.md), [#138](https://github.com/eddiefiggie/srd-rules-engine/issues/138), [#166](https://github.com/eddiefiggie/srd-rules-engine/issues/166)). |
...
**89 of 211 shapes resolve today.** The other 122 are listed, not omitted; run
...
_Last updated: 2026-08-25 — build `08252026.30`._
