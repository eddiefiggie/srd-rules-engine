...
**Current build:** `08302026.9` — **a move is refused where it is made.** [#350](https://github.com/eddiefiggie/srd-rules-engine/issues/350): p. 182's *"You can't **willingly** move closer to the source of fear"* had been disclosed as unenforceable since Frightened shipped, and **the disclosure's stated reason was wrong twice over**. It said the clause needed a direction relative to a creature — and repeated that in two issues as a dependency on the forced-movement primitive 0055 built. "Closer" is a comparison of two distances, which `squared_distance` has answered exactly since 0014, and the source has been recorded on the condition since #192. Both operands were there the whole time. What was actually missing was a **refusal**. And the tension #350 stated — that refusing at the state transition puts a rule outside R1's one entry point — is false, with the evidence inside the method: **`with_movement` already refuses three ways**, including a cost exceeding what is left. R1 forbids another API producing a *result*; a refusal produces none ([0056](docs/decisions/0056-a-move-is-refused-where-it-is-made.md)). **Two words carry the rule.** *Closer*, not *toward*: a creature circling at a constant distance is doing something p. 182 permits, and a refusal keyed on direction would stop it. And *willingly*: p. 182 forbids a creature moving **itself** closer and says nothing about one being *thrown* closer, which is why `with_forced_movement` is exempt and the check lives in the other method. A source whose distance cannot be measured forbids nothing, because refusing on a distance the engine could not measure would forbid what the rules may permit. **Frightened now discloses nothing at all** — the first of the fifteen conditions to reach that state after having disclosed something. **Prone is deliberately left alone**: p. 186's crawl restriction and its right-yourself exit are one rule, and building the refusal alone would leave a creature able to crawl and unable to stand — 0052's ordering read again, and filed as [#353](https://github.com/eddiefiggie/srd-rules-engine/issues/353). No coverage figure moves — 116 of 210 — because `frightened` was already claimed and one of its clauses reached nothing, the same shape 0054 recorded. 272 clauses verified against the document. 6 corruptions, each red on its own test. 1910 tests.
...
_Last updated: 2026-08-30 — build `08302026.9`._
