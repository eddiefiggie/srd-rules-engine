...
**Current build:** `08312026.25` — **closing [#224](https://github.com/eddiefiggie/srd-rules-engine/issues/224) unblocked a shape four hours after the audit named it.** p. 16's Underwater Combat was held entirely by one clause — *"a ranged attack roll with a weapon underwater **automatically misses** a target beyond the weapon's normal range"* — which is neither of the two things `_out_of_range` produces: p. 90 gives **Disadvantage** beyond normal range and **refuses** beyond long, because a ruling for a forbidden attack would be an outcome for something that never happened. p. 16's shot **happens** and settles without a die. With #224's kind in hand, all three of its rules ship. The miss is built by **replacing the finished proposal** rather than as a second construction, so the swap effects, the action charge and every other cost of having attacked stay exactly what an ordinary attack pays — p. 16 misses the shot, it does not forbid it. The melee clause carries **two exemptions and they are not alike**: a **Piercing** weapon is exempt and a **Swim Speed** exempts the creature, and the speed attaches to the melee sentence **only** — carrying it to the ranged clause would help the swimmer by a rule the document does not state, and a corruption proof exercises exactly that. Fire Resistance is read at the point of damage rather than written onto the creature, because it is a fact about **where the fight is**; a creature already Resistant gains nothing, which p. 17's "not cumulative" gets from `Defences` holding sets. [#446](https://github.com/eddiefiggie/srd-rules-engine/issues/446). Four corruption proofs. 140 of 210, 14 clauses. 357 clauses verified against the document. 2310 tests.
...
**140 of 210 shapes resolve today.** The other 70 are listed, not omitted; run
...
_Last updated: 2026-08-31 — build `08312026.25`._
