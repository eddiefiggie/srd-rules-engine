...
**Current build:** `08252026.34` — **the generator and the data it generated had disagreed for eleven shapes, and nothing compared them.** `scripts/derive_effect_shapes.py` is the stated source of `effect_shapes.json`, and **re-running it would have dropped coverage from 91 to 80** — `KINDS` still said `False` for Burning, Falling, Suffocation, Ritual, Flying, Hover, Jumping and four more, every one of them claimed in the data by hand when it landed and never written back. Every existing guard was green: `test_every_engine_shape_is_marked_implemented` compares `ENGINE_SHAPES` against the **data**, and the data was right. **Nothing compared the data against the thing that writes it.** It does now, and the guard was proved red through `prove_guard_red.sh`; better, the script was **run against the PDF and its output diffed byte-for-byte against the shipped file** — regeneration is a no-op again. On the way, #138's sight sweep was re-read against the document. Its standard is one sentence — *a shape is claimed when the engine produces the consequence its entry states* — and it had grouped four shapes together without separating them. It splits **two and two**. **Heavily Obscured** resolves: p. 182 gives Blinded *"while trying to see something"* in the space, **scoped to the attempt**, so it is a relation between observer and target rather than a condition on a creature — which is exactly what `can_see` answers and cites. **Darkness** resolves: its whole entry is *"an area of Darkness is Heavily Obscured"*, and the consequence flows through to `CANNOT_SEE`. **Dim Light does not** — it classifies into Lightly Obscured, whose Disadvantage on Perception nothing applies, so the value is computed and read by nobody; **Bright Light does not**, because *"normal illumination"* states no consequence to produce and claiming it would count a definition. Senses and light go **2 of 10 → 4 of 10**. `core/sight.py`'s docstring also still said the tables were empty and the verification `unverified`, three builds after #150 filled both. **91 of 211.** 1277 tests.
...
_Last updated: 2026-08-25 — build `08252026.34`._
