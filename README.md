...
**Current build:** `09042026.1` — **a target is a role in a mechanic, not a mechanic.** [#453](https://github.com/eddiefiggie/srd-rules-engine/issues/453) asked whether p. 190's *Target* is a shape or vocabulary, and asked for a second look rather than a late-session answer, because the entry sat beside `cover` and `area-of-effect`, which are mechanics. The entry is **one sentence** naming the creature or object at the receiving end of an attack roll, a forced save or a spell's effect — three routes the inventory already holds — and it states no consequence of its own, which is 0013's criterion and the one that files `creature`. **Neither prior classification test reaches it**: 0034's count discriminates two renames of a parent and 0035's identity test needs a synonym, and `target` fixes no parameter and is another name for nothing. **The strongest reading against was p. 106's *Targets* section** — five rules, and under 0033 clause 1 a shape's content is whatever the document states about the term anywhere. Every one is the *spell's*: a clear path is `cover`'s own p. 179 refusal from the other side, areas are `area-of-effect`, and the rest need a spell ([#21](https://github.com/eddiefiggie/srd-rules-engine/issues/21)). All five are asserted now, including the one `spell_reaches` had **quoted in a docstring and nobody had verified** — the [#371](https://github.com/eddiefiggie/srd-rules-engine/issues/371) shape, found in passing. The absence is asserted narrowly and with **two controls, because the row asserts zero**: the document states a held state as *"has the X condition"* or *"is Bloodied"* and never says *"is a target"*, and a zero row is the one assertion a sweep that read nothing passes — the issue's own first count did exactly that and was caught by counting `creature`. [0085](docs/decisions/0085-a-target-is-a-role-in-a-mechanic-not-a-mechanic.md). 142 of 209, 14 clauses. 367 clauses verified against the document. 2326 tests.
...
**142 of 209 shapes resolve today.** The other 67 are listed, not omitted; run
...
_Last updated: 2026-09-04 — build `09042026.1`._
