...
**Current build:** `08232026.25` — **an MCP adapter, so a model can finally reach the engine.** The turn loop is a generator and MCP calls are stateless, so a `Session` holds the suspended turn and answers what the engine is waiting for. **There is no `adjudicate` tool, and that absence is the design** — a consumer calling adjudication directly gets outcome authority without skip prevention, so exposing it would ship the failure this engine removes as a feature. A test asserts the absence. `begin_turn` carries the situation, because the trigger catalogue matches against it and omitting it would leave the challenge mechanism silently unreachable; a refusal carries the triggers that fired, with their message and whether they are authored or cited. The MCP SDK is an extra, imported inside `build_server`, so `[project].dependencies` stays empty (R33). Decision `0016`.
