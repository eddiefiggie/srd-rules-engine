...
**Current build:** `08252026.40` — **a band is now a thing with a capacity, and initiative was sharing the d20's.** [#82](https://github.com/eddiefiggie/srd-rules-engine/issues/82): `core.d20` splits a seed's index space so one adjudication cannot draw the same die twice — but the bands were **three integers a reader had to compare** and nothing checked them. `roll` took an arbitrary `count` and `offset`, so a long enough run walked out of its band onto the next one's dice, and **that collision does not raise**: it produces a replacement die identical to a damage die of the same seed, a reroll agreeing with the thing it exists to be independent of. **The issue called this hypothetical. It wasn't.** `initiative_order` rolled **one die per combatant from index 0** — the d20's band — with **no bound at all**, so a large enough encounter aliased a combatant's initiative onto a die the same seed had already produced. `Band` now carries a name, a start and a **capacity**; `BANDS` states the map **once**; `_no_band_overlaps` refuses an overlapping map **at import**; `roll` refuses a run that would leave its band; and every internal caller goes through `Band.at`, which is the structural half — an index past the end cannot be computed rather than merely being caught. Initiative moved to a band of its own, which **rewrites no history** because nothing records initiative in the ledger — but it does change which order a seed produces, and that broke four tests pinned to a lucky seed. The fixture now **says what it requires** ("this slice needs the player character to act first") and fails with that sentence instead of four cryptic ones. Three guards proved red, including the `Band.at` off-by-one and the import-time overlap check. What this does **not** settle is whether banding is the right shape at all versus hashing a **named purpose** into the die's material — that alters every die ever derived, which `replay_entry` already treats as a **reconciliation** rather than an integrity failure, so it is a cost rather than a blocker and it is filed as [#221](https://github.com/eddiefiggie/srd-rules-engine/issues/221). Coverage unchanged at **92 of 211**. 1313 tests.
...
_Last updated: 2026-08-25 — build `08252026.40`._
