...
**Current build:** `08252026.13` — **the script that proves a guard fails could leave the corruption behind, which is the one thing it exists to prevent.** [#175](https://github.com/eddiefiggie/srd-rules-engine/issues/175), found by running it: proving a guard for [#124](https://github.com/eddiefiggie/srd-rules-engine/issues/124) corrupted a discharge so it discharged nothing, the phase span forever, and the guarded run hung. **Bash defers a trap until the foreground command returns**, so the restore was queued behind the very process that would not exit — and a plausible one-line `return self` sat in a state mutator until it was put back by hand. That is precisely the failure [#155](https://github.com/eddiefiggie/srd-rules-engine/issues/155) built the script for, reached by a route it did not cover. The guarded run is now **backgrounded and waited on** (`wait` is interruptible where a foreground command is not) and **watched**, exiting `124` after `PROOF_TIMEOUT` seconds; the trap kills the children before restoring, or it waits on the hang itself. **A timeout is reported as not a proof**: a run that never terminates shows the corruption broke something, not that your assertion fires, and calling that PROOF HELD would be the script agreeing with itself. The first fix had a bug of its own — killing the watchdog does not kill the `sleep` inside it, and the orphan held stdout, so every piped proof took two minutes; the watchdog is redirected now. Verified both ways: the old script reproduces the abandoned corruption on TERM, the new one restores, and all three assertions were proven red by hand because the script refuses to corrupt itself. No engine behaviour changed; coverage stays at 77 of 211. 1099 tests.
...
**77 of 211 shapes resolve today.** The other 134 are listed, not omitted; run
...
_Last updated: 2026-08-25 — build `08252026.13`._
