...
**Current build:** `08252026.9` — **0027: the turn's start is a phase too, and not every outcome rolls a d20.** [0023](docs/decisions/0023-the-turns-end-is-a-loop-owned-phase.md) found the save-ends save and the death save were one missing phase rather than two oversights, and built the turn's **end**. Two readings since put the death save (p. 17) and Burning (p. 178) at the turn's **start**, which is a phase this engine has never had. [0027](docs/decisions/0027-occasions-and-outcomes-without-a-roll.md) builds the design for it and settles what an obligation is: **identified by its rule id, not by its condition** — generalised by *removing* the condition-specific field rather than adding a `kind` for every consumer to branch on, which is [0019](docs/decisions/0019-kind-is-a-filing-label.md) applied. **The clause the record is least sure of is named as such**, mirroring 0023's own hedge: the start-of-turn skip guarantee cannot live in `advanced_turn`, because by then the obligations are newly due rather than overdue, so the *declaration* refuses instead. **Clause 6 is the one that reaches furthest**: `Proposal.test` is a required field and `adjudicate` rolls it unconditionally, so **no outcome can exist without a d20** — and Falling is damage per 10 feet with no test in it. It becomes optional, one entry point unchanged (R1), seed still drawn and damage dice still engine-rolled (R4) ([#170](https://github.com/eddiefiggie/srd-rules-engine/issues/170), which also carries the trap: a testless ruling must not replay as `UNREPLAYABLE`, because it *is* reproducible). Three clauses **remove** things from the occasion question that looked like they belonged: Falling is 0023 clause 5 already deciding the case; Dehydration is bookkeeping while Malnutrition is a DC 10 save and so cannot share its implementation; and both turn-based hazards end on events, not on either clock. **Decided, not built** — coverage stays at 76 of 211. 1052 tests.
...
_Last updated: 2026-08-25 — build `08252026.9`._
