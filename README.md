...
**Current build:** `08312026.17` — **an entry that spans a page break, and the constraints were all in the half I had not read.** p. 182's Help ends mid-sentence at *"one ally who is near"* — the rest is on p. 183, and it is where the difficulty lives. Help looks like [0049](docs/decisions/0049-advantage-that-outlives-its-roll.md)'s `PendingAdvantage`, which is exactly the shape it wants: advantage that outlives its roll, with a holder, a scope and a turn-bounded expiry. **Three of its requirements land on that and three do not.** The holder is *"one of your **allies**"* — whichever attacks first, not assigned when the Help is taken, where Vex grants to the attacker and Sap to the creature hit and both are known. The scope is a **skill**, not a target, and `against_id` answers only which target. And p. 183 says *"skill **or tool**"* while `core.skills` states outright that tools are not modelled — so the skill half alone would be a rule stated at half. **Two new axes on a token that has none is the shape that arrives one PR at a time**, which is what 0049 got right, so this is a gate ([#434](https://github.com/eddiefiggie/srd-rules-engine/issues/434)) rather than a design chosen at speed. Its three sentences are asserted anyway — 0080's practice — including the one that turns on a single word: the ability-check benefit expires **if unused before** the start of your next turn, the attack benefit **at** it. **No shape moves**, and the reading is the deliverable. 136 of 210, 14 clauses. 347 clauses verified against the document, up from 300 this morning. 2283 tests.
...
_Last updated: 2026-08-31 — build `08312026.17`._
