...
**Current build:** `08282026.8` — **a creature can hold, wear and carry things, and the engine does not know how many hands it has.** [0039](docs/decisions/0039-equipment-is-what-a-creature-holds-wears-and-carries.md) clauses 1-4 and 7 ([#257](https://github.com/eddiefiggie/srd-rules-engine/issues/257)): an `Item` the creature carries as ruleset data, one closed `Carriage` field — worn, held or stowed — and the hand arithmetic p. 105 turns on. **The rule value this build refused to infer is the hand count.** A sweep of every "free hand", "one hand", "two hands" and "a hand" in the document returns only *relational* rules — "you need a free hand" (p. 89), "requires two hands" (p. 90), "must have a hand free" (pp. 105, 182, 190) — and **not one of them says how many hands a creature has**. Two is plausible, universal in recall, and stated nowhere, which is exactly R31's shape. So `Combatant.hands` is `int | None` and defaults to `None`: unknown, not zero, because an agent told it has no free hands would decline a spell the document might permit. **One free hand serves Somatic *and* Material** (p. 105), now an asserted clause — proved red by rewriting it to require a different hand. What an item holds is weight, hands-when-held and the two component flags: no name, no price, and **not** whether a spell's materials are consumed or priced, because p. 188 makes those the *spell's* properties. A corruption proof also caught a weak test of my own — it stowed items costing no hands, so charging every item regardless of carriage gave the same answer. **95 of 209**, unchanged: the shapes this unblocks are built by #245, #247, #258 and #259. 188 clauses verified. 1470 tests.
...
_Last updated: 2026-08-28 — build `08282026.8`._
