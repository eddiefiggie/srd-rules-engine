...
**Current build:** `09042026.3` — **every ingredient existed, and the geometry did not.** [#451](https://github.com/eddiefiggie/srd-rules-engine/issues/451) was p. 14's *Moving around Other Creatures*, four sentences that [0084](docs/decisions/0084-a-space-is-a-control-area-not-a-volume.md) had made expressible, and all four were built — the passage refused unless ally, Incapacitated, Tiny or two sizes apart; the stretch of a move inside another's space priced as Difficult Terrain, merged and non-cumulative; the destination refused for every other creature; and the Prone for ending a turn in a space with someone, as an obligation the loop derives and a ruling applies. **The suite then showed the collision, and it is not a bug.** Reach is point-to-point (0084 clause 7, pinned), so a Medium creature in melee reach of anything Huge or larger has its point *inside* the target's space — `tests/test_forced_movement.py`'s ogre at `(3, 4, 0)` is the ordinary case — and p. 14 reads that position as forbidden to walk into, doubled to move in, and Prone at the end of every turn. On the grid p. 14 is written for, the same fighter's centre is ten feet from the giant's and in reach because reach is measured **between squares**. That is a question about how reach is measured, which 0084 deliberately declined, so it is a gate ([#456](https://github.com/eddiefiggie/srd-rules-engine/issues/456)) rather than a scope invented at speed — and the design that was built is recorded there rather than merged. The four sentences and p. 176's *Ally* are asserted anyway, 0080's practice. **No shape moves**, and the finding is the deliverable. 143 of 209, 14 clauses. 376 clauses verified against the document. 2345 tests.
...
_Last updated: 2026-09-04 — build `09042026.3`._
