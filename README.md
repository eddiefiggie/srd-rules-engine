...
**Current build:** `08262026.4` — **`save` and `saving-throw` already resolved to the same symbol.** [#230](https://github.com/eddiefiggie/srd-rules-engine/issues/230) asked whether a term whose entire entry is *"X is another name for Y"* belongs in `vocabulary`. It does, and the deciding evidence was in the tree rather than the document: `ENGINE_SHAPES` mapped **both** `save` and `saving-throw` to `core.d20.TestKind.SAVE`, on adjacent lines — one mechanic counted twice, in the numerator *and* the denominator of the figure R17 makes falsifiable. p. 187 states the identity in both directions, the parent entry declaring the alias itself ([0035](docs/decisions/0035-two-names-for-one-thing-are-one-shape.md)). **The test is identity, not usage** — the document says "save" 1544 times, so [0034](docs/decisions/0034-a-term-the-document-defines-and-never-uses.md)'s zero-consumers test would have kept it a shape; `criteria` gains a fourth rule rather than stretching the third. **Both figures fall and nothing is lost**: `TestKind.SAVE` is still claimed under `saving-throw`, no resolver changed, and a coverage number that drops because a duplicate stopped being double-counted is the figure becoming more true. `death-saving-throw` is the counter-case that keeps the rule from over-firing — same name, its own resolver, still a shape. **95 of 209.** 1339 tests.
...
_Last updated: 2026-08-26 — build `08262026.4`._
