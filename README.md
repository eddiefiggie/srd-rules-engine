...
**Current build:** `08302026.27` — **p. 185 is offered, and movement became a phase the loop drives.** [#381](https://github.com/eddiefiggie/srd-rules-engine/issues/381) made the Opportunity Attack's sight clause answerable and left the offer unbuilt; this is the other half, and the architecture it needed was not the one [0015](docs/decisions/0015-reactions-and-the-agent-seam.md) predicted. **The seam held** — `DeclarationRequest` carries its own `actor_id` and `_concentration_saves` has driven another creature's roll mid-turn since #215 — but 0015 assumed the loop would be present when movement resolved, and it is not: `EncounterState.with_movement` had **no caller in the engine**, because [0056](docs/decisions/0056-a-move-is-refused-where-it-is-made.md) deliberately put movement's rules in the state method rather than in adjudication. An attack is not a refusal, so R1 forbids resolving one there, and a pure state method cannot yield to a driver in any case. So `TurnLoop.move` is a phase — the fifth occasion on which the existing outcome path is taken rather than a second path to one — and it asks who is provoked, offers each of them the attack, spends the **Reaction** (p. 185's attack carries its own key precisely because the ordinary one charges the Action), adjudicates through the one entry point, and only then applies the move. **The attacks resolve first by geometry, not by reading p. 185's silence**: provoking means the mover was in reach and is leaving it, so at the destination a melee attack has nothing in range. [0072](docs/decisions/0072-movement-is-a-phase-the-loop-drives.md) was written before the code and **one of its clauses was wrong**: it claimed a dropped mover already stopped, via Unconscious implying Prone. Nothing in this engine applies Unconscious at 0 hit points, and a probe written to confirm the clause showed a creature at 0 walking twenty feet — a defect that predates the reaction and that any caller could reach. `is_down` was the engine's own doctrine, enforced by the read surface since it shipped and never asked by `with_movement`. The refusal is added, the record says plainly that it was wrong, and the clause is what found it. `opportunity-attack-detected-but-never-offered` retires with the offer it named — the second of two names for one gap, one build apart — and the `opportunity-attacks` shape is claimed six days after its trigger shipped, because a detection nothing called was machinery rather than a resolved rule. Nine corruption proofs; the one assertion with none is named rather than counted. Report attribution for interleaved entries stays [#120](https://github.com/eddiefiggie/srd-rules-engine/issues/120), which now has the second source 0015 predicted would bite. 118 of 210, 15 clauses. 282 clauses verified against the document. 2075 tests.
...
**118 of 210 shapes resolve today.** The other 92 are listed, not omitted; run
...
**15 clauses are disclosed but unenforced**, and that is the instrument's second figure
...
_Last updated: 2026-08-30 — build `08302026.27`._
