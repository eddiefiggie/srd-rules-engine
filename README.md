...
**Current build:** `08242026.9` — **the CLI adapter, so two of R34's three exist ([#133](https://github.com/eddiefiggie/srd-rules-engine/issues/133)).** A transport binding over `Session` at the same layer as MCP — *not* `HumanCliDriver`, which is R8's reference driver, and the conflation is why the status table read one adapter rather than two. It holds the session for the process because a `Session` is a suspended generator: one-shot `argv` would make every command its own turn. The two absences are the design and both are tested — **no command reaches adjudication**, and **none waives an end-of-turn obligation**, since a supported way to skip a compulsory save does not belong in front of the caller the challenge mechanism constrains. The forbidden-surface guard is now shared across every adapter rather than copied per adapter, and `TurnEnded`/`TurnEnd` join the committed API — four of `Pending`'s five members were already committed and a consumer matching the union has to handle the fifth. No executable ships: a console script must pick a ruleset, and there is not yet one to play. 941 tests.
...
| **v1.0 — adapters** | **Two of the three R34 names.** MCP ships behind the `mcp` extra; CLI ships needing no dependency at all ([#133](https://github.com/eddiefiggie/srd-rules-engine/issues/133)). HTTP does not exist. Neither adapter has an executable — a console script has to pick a ruleset, and there is not yet one to play. (`HumanCliDriver` is R8's reference *driver*, not R34's CLI adapter — an easy pair to conflate.) |
...
_Last updated: 2026-08-24 — build `08242026.9`._
