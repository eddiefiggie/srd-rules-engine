...
**Current build:** `08302026.7` — **a save is rolled by a creature.** [#344](https://github.com/eddiefiggie/srd-rules-engine/issues/344): three printed rules act on a saving throw because of what the **roller** is holding, and **none of them reached a roll**. Four conditions fail Strength and Dexterity saves outright (pp. 186, 189, 191), Restrained gives Dexterity saves Disadvantage (p. 187), Dodge gives them Advantage (p. 181) — all three modelled field-by-field in `ConditionEffects` and `ActionBudget`, and consulted by **none** of the six resolvers that build a save. Every save in this engine was a bare ability modifier against a DC. The tests that existed asserted the *data*, never that a Restrained creature's Dexterity save was actually rolled at Disadvantage — because nothing made it so. **Found by 0053**, which rejected 'let the engine pick the better save' on p. 187's strength and then could not express it. Applied **centrally**, between the resolver and the die, because six copies plus a seventh resolver that forgets is the failure this repository names most often ([0054](docs/decisions/0054-a-save-is-rolled-by-a-creature.md)). **An automatic failure is not a roll**: no die is drawn and the ruling carries no result, because a number in the ledger that decided nothing reads exactly like a save that was rolled and lost. The obvious implementation — rewrite the proposal to `test=None, outcome=on_failure` — is **wrong, and the case that breaks it was already in the tree**: `core.save_ends` builds a save with an *empty* `on_failure`, since failing to shake a condition off simply leaves it, and `Proposal` rightly refuses one that decides nothing. So the branch is selected by the adjudicator and the proposal is never rewritten. **`ability` is first-class on `D20Test`**, and `None` is a value rather than an omission — p. 17: *"Unlike other saving throws, this one isn't tied to an ability score"*, and the creature making death saves is Unconscious, one of the four. A rule that reached it would kill every character who ever dropped. An interaction I expected to cancel does not: **Restrained sets Speed 0**, and p. 181 ends the Dodge for exactly that, so a Restrained dodger rolls at plain Disadvantage — which is `is_dodging` re-asking rather than trusting a flag. A new static guard walks every `core` module's AST and fails a save built without naming its ability. **No coverage figure moves — 114 of 210** — because all five conditions were already claimed, and correctly by R17's terms: the shape resolved and two of its clauses reached nothing. That is the kind of gap the instrument cannot see. 265 clauses verified against the document, one of them written from memory, failed, and corrected against the page. 11 corruptions, each red on its own test; the eleventh found a guard clause carrying no weight, which was removed. 1848 tests.
...
**114 of 210 shapes resolve today.** The other 96 are listed, not omitted; run
...
_Last updated: 2026-08-30 — build `08302026.7`._
