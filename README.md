...
**Current build:** `08252026.37` — **a resistant creature that rolls a 1 on a 10-foot fall is no longer knocked down.** [0032](docs/decisions/0032-an-outcome-conditional-on-its-own-damage.md) clauses 1-5, built against Falling, closing [#173](https://github.com/eddiefiggie/srd-rules-engine/issues/173). p. 182: *"it has the Prone condition **unless it avoids taking any damage from the fall**"* — a qualifier about damage **taken**, decided by a resolver that ran before the dice. It covered two of the three ways to take none and **special-casing those two is what hid the third**. Reproduced before it was fixed: against `main`, Resistance to Bludgeoning + a 10-foot fall + seed 4 rolls a **1**, takes **0**, and is **Prone**. Same scenario now: **not Prone**. **The branch moved instead of growing a third case** — `Effect.when` carries `When.DAMAGE_TAKEN`, and `_apply` decides it against `damage_after_defences`, so all three ways go down one road and the resolver stops branching on Immunity entirely. **Asking one step earlier would have shipped the same bug**: `_roll_declared` has the *rolled* number, and the rolled number here is 1 — p. 17's Resistance is the whole difference. Proved by corrupting the accumulator to read `effect.amount`, which is that exact wrong implementation, and four tests go red. The predicate is **data**, so the ledger records what was **asked** as well as answered: every effect carries its `when`, and a withheld one lands in a new `withheld` list rather than vanishing — an effect the rules withheld must not read like one nobody considered (`RULING_VERSION` 7, additive). **Clause 5 is the subtle half**: bounds are fixed at proposal time, so `falling_resolver` now names Prone in **neither** `may_claim` nor `may_not_claim`, and `_bounds` writes the refusal after `_apply`, where it is known. Three guards proved red, and the fix reproduced against the base tree rather than argued. Coverage unchanged at **91 of 211** — `falling` was already claimed, and was already wrong. 1292 tests.
...
_Last updated: 2026-08-25 — build `08252026.37`._
