...
**Current build:** `08232026.35` — **every ruling entry the engine had ever written read as uninterpretable, and now does not (#106).** The payload's `compat` floor was set from its own schema version while the reader is versioned separately, so the moment `RULING_VERSION` left 1 the reader R35 makes public began telling every consumer it could not read **the one entry type that carries an outcome**. It survived because the reader's tests hand-write payloads at `compat: 1` — the one type whose floor had drifted was the one they never contained. `0022` settles what the floor is measured against: **a reader version, never a schema version**, amending `0011` clause 5. Each payload now names its floor in its own constant, and a guard reads a ledger the engine actually wrote. The same defect was armed under the memory store, where an uninterpretable fact write **raises** rather than flags.
