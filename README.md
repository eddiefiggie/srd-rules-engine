...
**Current build:** `08302026.3` — **a size is stated, or it is unknown.** [#259](https://github.com/eddiefiggie/srd-rules-engine/issues/259): nothing in this engine had a `Size`, so p. 178's Carrying Capacity — a table keyed on one — could not be computed at all, and four other rules asking how many categories apart two creatures are could not be asked. **The issue proposed defaulting to Medium and this build refuses it**, because the argument against was already written down one field away: `Combatant.hands` is `int | None` precisely because two is *plausible, universal in memory, and stated nowhere*. p. 14 says where a size comes from instead — a **species** or a **stat block**, both content this repository does not ship (R31) — so Medium would be the engine answering a question only a ruleset can, in the voice of a rule. It is also wrong where it hurts: read at Medium, p. 178 gives an Ancient Red Dragon 450 lb against its true 3,600, arriving as a plain number with nothing marking it invented ([0051](docs/decisions/0051-a-size-is-stated-or-it-is-unknown.md)). **p. 178 is a table and not a doubling.** The document prints **Small/Medium as one row**, so p. 86's Powerful Build — "you count as one size larger when determining your carrying capacity" — moves a Small creature to Medium and finds the identical multipliers, gaining it *nothing*. `carry * 2 ** steps` is right for four of the five steps and silently doubles the one that matters. Both columns are transcribed for the same reason: Drag/Lift/Push is twice Carry in all six rows and the document states a table, not that relation. **The Strength score, not the modifier** — a factor of seven, and 225 lb and 30 lb are each a believable load, which is how that one survives review. **The bound is computed and the Speed cap is not applied**, disclosed rather than enforced: p. 178's sentence fires on *dragging, lifting or pushing* rather than on carrying too much, and p. 12 leaves the whole subsystem to a person — "the GM **might** require you to abide by the rules for carrying capacity" ([#336](https://github.com/eddiefiggie/srd-rules-engine/issues/336)). A guard written for this change was **vacuous under its own fixture** and the corruption proof caught it: nothing in `opening_state` states a size, so the dataclass it existed to catch on the transport never reached the payload and rendering it raw kept the assertion green. **112 of 210**, masteries still **7 of 8** — Push has lost its Size blocker and keeps its forced-movement one. 244 clauses verified against the document. 18 corruptions, each red on its own test. 1783 tests.
...
**112 of 210 shapes resolve today.** The other 98 are listed, not omitted; run
...
_Last updated: 2026-08-30 — build `08302026.3`._
