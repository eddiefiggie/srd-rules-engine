...
**Current build:** `08312026.6` — **the page read before the build, and the build did not happen.** Cover looked like a wiring job — three degrees, +2, +5, and a refusal — until p. 15 supplied the sentence p. 179 leaves out: *"A target can benefit from cover only when an attack or other effect **originates on the opposite side** of the cover."* **Cover is directional, and this engine has nowhere to put a relation for the duration of one attack.** `Combatant.effective_armour_class` is read in three places in `core.combat` and is a property of the target alone; it cannot know who is shooting, so a creature behind a wall would get +5 against the archer beside it. That is a design question with no default, which is what `gate` means ([#416](https://github.com/eddiefiggie/srd-rules-engine/issues/416)), and it holds `hide` behind it too — p. 183 requires Three-Quarters or Total Cover. **The three sentences it rests on are asserted anyway**, which is 0080's practice: a clause the verifier carries although unbuilt is one the next author does not re-read and cannot quietly disagree with. 321 clauses, up from 300 this morning. **No shape moves**, and the honest reason is that the unblocked surface is thinning: what remains is mostly gate-shaped or waiting on [#142](https://github.com/eddiefiggie/srd-rules-engine/issues/142), [#409](https://github.com/eddiefiggie/srd-rules-engine/issues/409) and [#21](https://github.com/eddiefiggie/srd-rules-engine/issues/21). 124 of 210, 14 clauses. 2206 tests.
...
_Last updated: 2026-08-31 — build `08312026.6`._
