...
**Current build:** `08312026.14` — **p. 191 not saying when Unconscious ends is the rule, not an omission.** Its entry states the condition's *effects* and stops; p. 184 states two endings for a creature knocked out — regaining **any** hit points, and first aid at DC 10 Wisdom (Medicine) — and neither may touch a creature Unconscious for some other reason. `Conditions.applied` was a bare set, so those were the **same state** and honouring p. 184 would have woken a sleeper with a cure wound. `causes` fixes it, and **both halves of its shape were already in the class**: `exhaustion_levels` keys each level by the rule that caused it ([0028](docs/decisions/0028-an-exhaustion-level-carries-its-cause.md)) and `sources` holds a **set per condition** because p. 179 says a condition does not stack while its causes plainly do. So a creature knocked out *and* drugged holds one Unconscious with two causes, and healing removes one — an implementation that ended the condition outright would wake a sleeper with a bandage. **Two corruption proofs failed and forced better tests.** One assertion survived deleting the guard it named, because the sleeper was protected by a different clause; the other never reached clause 7 at all — p. 191's *"you remain Prone"* only carries a stale cause when Prone **and** Unconscious end together, which the test now constructs. `knocking-out-a-creature` is claimed. Conditions stay **15 of 15**: a cause is a property of an application, not a member of the set. Six corruption proofs. [0083](docs/decisions/0083-a-condition-carries-the-rule-that-caused-it.md). 134 of 210, 14 clauses. 340 clauses verified against the document. 2258 tests.
...
**134 of 210 shapes resolve today.** The other 76 are listed, not omitted; run
...
_Last updated: 2026-08-31 — build `08312026.14`._
