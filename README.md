...
**Current build:** `08302026.23` — **a guard that found six rules on its first run.** Four times in one session a rule turned out to be computed once, consumed by the menu, and absent from the path that produces outcomes — and **every one was found by a person** ([#365](https://github.com/eddiefiggie/srd-rules-engine/issues/365), [0068](docs/decisions/0068-a-rule-the-menu-asks-and-nothing-else-does.md)). The obvious formulation is "compare the menu against the resolvers", it is what the issue suggested first, and it is **wrong for this engine**: outcomes come from resolvers *and* from state transitions, because [0056](docs/decisions/0056-a-move-is-refused-where-it-is-made.md) put a movement refusal in `with_movement` deliberately. That version reported 27 names and was wrong about most of them — the failure mode that gets a guard deleted rather than fixed. The honest question is narrower: which names does the read surface use that **nothing else in the core uses at all**? Thirteen, which is a list a person reads once. Seven are values the surface publishes; **six are rules** — p. 89's Extra Attack limit and Cleave, p. 90's Loading and Ammunition, and a stat block's Multiattack — so a caller reaching adjudication directly can attack five times with an Extra Attack of two and fire a crossbow twice with an empty quiver ([#376](https://github.com/eddiefiggie/srd-rules-engine/issues/376)). The set is derived and each name carries a hand-written verdict, so the annotations cannot fall behind the code; a third verdict of *"it is fine"* is refused, which caught this author hedging on `Multiattack.allows` the day the file was written. **The guard ships with six unfixed entries and that is the deliverable** — the seventh cannot now arrive unnoticed. 117 of 210, 16 clauses. 282 clauses verified against the document. No engine changes. 2040 tests.
...
_Last updated: 2026-08-30 — build `08302026.23`._
