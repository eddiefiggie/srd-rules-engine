...
**Current build:** `08302026.30` — **initiative had a record, a seed band and a pair of dice, and no page.** `verify_d20_rules.py` carried no initiative clause at all, so two rules had been settled by default rather than by reading — and the code said so in both places. `initiative_order` took `ability` as an argument because *"which ability the modifier comes from is a rule with a section citation"*, and `with_initiative` broke ties *"by the order given"*. Neither was wrong; both were unfalsifiable. p. 13 answers the first outright — *"they make a **Dexterity check** that determines their place in the Initiative order"* — so the parameter is gone and the value is a constant beside a `Verification` naming its pages, which is [0026](docs/decisions/0026-terrain-enters-as-state.md)'s dial argument one level smaller: a caller able to roll Initiative off Strength could reorder an encounter and leave a ledger that looks legitimate. **The second answer is the one worth having.** p. 13 does not leave ties open — it *assigns* them, in three clauses, to a person: *"the GM decides the order among tied monsters, and the players decide the order among tied characters."* So there is no rule here to implement, and an engine that invented one would be overriding an instruction rather than filling a gap. Insertion order is declared a **convention** in the words `Lighting` already uses for overlapping volumes, and the person's decision reaches the engine as the order the combatants are passed in. That makes ties the third thing the SRD hands to a human that this engine declines to decide, after the *degree* of Cover and an improvised weapon's damage type ([#264](https://github.com/eddiefiggie/srd-rules-engine/issues/264)) — and the cleanest, because the document is explicit rather than vague. A test asserting the **opposite** of the new design is replaced and says why: it was right while the page was unread, and inverted by the page being read. p. 184's optional Initiative *score* variant is modelled by nothing and is asserted so the absence is findable. Five corruption proofs. [0075](docs/decisions/0075-ties-are-a-persons-and-initiative-is-a-dexterity-check.md). 118 of 210, 15 clauses. 287 clauses verified against the document. 2105 tests.
...
_Last updated: 2026-08-30 — build `08302026.30`._
