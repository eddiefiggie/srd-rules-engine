...
**Current build:** `09042026.4` — **the document does say how a range meets a space, and it says it on p. 13.** [#456](https://github.com/eddiefiggie/srd-rules-engine/issues/456) asked whether reach is measured to a creature's point or to its space, because with point-to-point reach every melee position against a Huge creature was inside it. [0084](docs/decisions/0084-a-space-is-a-control-area-not-a-volume.md) had answered in passing that *"the document never describes"* measuring between extents — an absence, argued without reading the grid variant. p. 13's *Ranges*: *"count squares from a square **adjacent** to one of them and stop counting **in the space** of the other one."* A range runs from a space's edge, and the grid is where the document says so. **Read in feet along 0014's straight line, it is the distance between two points less what each space exceeds one square**: nothing for Tiny, Small or Medium, 2½ feet for Large, 5 for Huge, 7½ for Gargantuan. On an axis that is p. 13 exactly — a Medium fighter beside a Huge giant has its centre ten feet from the giant's, and ten less five is p. 15's five. **Off an axis it departs from the grid only as 0014 already does**, which is why the nearest-edge gap was rejected: corner-to-corner it would have put two Medium creatures at `(5, 5, 0)` in reach of each other, reversing 0014 for everyone to fix the giants. **Every range between two things uses it, both spaces counting**: melee reach and weapon range, the Opportunity Attack's reach left, p. 191's and p. 186's five feet, Cleave's spread and second swing, a grapple's range, a sense's range, an object within reach, and a spell's range from its caster — thirteen sites, each with a test that a Huge creature is in range from where a Medium one is not. **Occupancy is untouched.** 0084's pin did exactly what it was for: it fired, and it is turned round rather than deleted. [0086](docs/decisions/0086-a-range-runs-from-the-edge-of-a-space.md). [#451](https://github.com/eddiefiggie/srd-rules-engine/issues/451) is unblocked. Twenty corruption proofs. 143 of 209, 14 clauses. 380 clauses verified against the document. 2364 tests.
...
**143 of 209 shapes resolve today.** The other 66 are listed, not omitted; run
...
_Last updated: 2026-09-04 — build `09042026.4`._
