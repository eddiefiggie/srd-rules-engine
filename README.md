...
**Current build:** `08242026.16` — **a blocked turn had no route forward over any transport, and the fix is mostly about what a caller is not allowed to say.** All three adapters declared a `supply_facts`/`facts` command and all three raised, so a declaration the memory port could not resolve suspended with nothing able to answer it ([#144](https://github.com/eddiefiggie/srd-rules-engine/issues/144)). `Session.supply_values` is the shared route, and it takes a name and a value and nothing else. **A `Fact` has four fields and three of them are ways for a supplied value to become something it is not**: the *subject* would let a turn suspended on one creature write about another, the *kind* would let a caller disagree with the engine about what it just stored (R20), and the *writer* is the sharpest — `ruling` means a value an adjudicated outcome produced, so a caller able to claim it would be dressing an unrolled fact as a ruling's product. All three are decided by the engine, and a fact type nobody asked for is refused by name because a suspension answers a question rather than opening the store to writes. The declared `FactType` supplies the kind, so `nerve=true` off a terminal reaches the port as a Boolean and `nerve=yes` is refused rather than read generously. The MCP tool schema loses the `subject` it asked for in a draft that never ran. 1009 tests.
...
_Last updated: 2026-08-24 — build `08242026.16`._
