...
**Current build:** `08242026.12` — **a decision record's unbuilt clauses were tracked by nothing, and closing the gate is what hid them.** A `gate` issue closes by producing a record ([#126](https://github.com/eddiefiggie/srd-rules-engine/issues/126)), so the design is written down at the same moment the issue holding it disappears — and a *closed* issue reads as finished work rather than as absent work. `AGENTS.md` now extends the plan-deferral rule to records: an unbuilt clause gets its own issue, named beside it in **Status of implementation**. The sweep that rule implies found exactly two records naming work with no issue behind it. One was tracked and merely unnamed; the other was not tracked at all — all three adapters declare a `supply_facts`/`facts` command that raises, and MCP's pointed at #97, closed ([#144](https://github.com/eddiefiggie/srd-rules-engine/issues/144)). `tests/test_decision_records.py` holds the section's presence, which is the half a machine can check without rotting; accuracy stays with the author. It is green against the pre-change tree by design, so each of its three properties was proven red by corrupting the records instead. 970 tests.
...
_Last updated: 2026-08-24 — build `08242026.12`._
