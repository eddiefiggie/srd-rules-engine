...
**Current build:** `08282026.1` — **p. 179's Concentration save has an occasion.** [#215](https://github.com/eddiefiggie/srd-rules-engine/issues/215) asked which of three shapes should carry it, and the two constraints that decide the answer were in the tree rather than in the issue. The existing obligation machinery is **once per turn** by construction — `discharged` is `(actor_id, rule_id)`, cleared on every advance — while a Concentration save is owed **once per damage instance**, so reusing it would silently swallow the second save of a Multiattack. And damage lands in **three** loop phases, not one: Burning deals its at the *start* of a turn (p. 178), so a design discharging only after the declaration slot would serve every attack in the game and miss the hazard. The answer is a **fourth occasion** ([0036](docs/decisions/0036-a-fourth-occasion-owed-by-whoever-took-the-damage.md)): `with_damage` records one debt per instance carrying the amount its DC derives from, and `TurnLoop` drains it through the one adjudication entry point. **No second path to an outcome — one more occasion on which the existing path is taken.** The `concentration` claim is now honest rather than merely asserted: `Concentration` was referenced nowhere in `src/` outside its own module, so nothing in an encounter could be concentrating while `ENGINE_SHAPES` claimed the shape, and a **behavioural** guard replaces the assertion. Two defects the corruption proofs found and review would not have: the DC's floor and cap were **pinned to nothing** — every test compared against the constant, so a floor of 11 left the file green — and `prove_guard_red.sh` restored a source file without invalidating its bytecode ([#237](https://github.com/eddiefiggie/srd-rules-engine/issues/237)), so a same-size corruption survived the restore in the *running engine* while `git diff` read clean. **95 of 209**, unchanged: the shape was already claimed, and what changed is that the claim became true. 1377 tests.
...
**95 of 209 shapes resolve today.** The other 114 are listed, not omitted; run
...
_Last updated: 2026-08-28 — build `08282026.1`._
