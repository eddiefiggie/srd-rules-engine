...
**Current build:** `08282026.7` — **equipment: three subsystems have now asked the same question and got the same answer.** Nothing in this engine holds, wears or carries anything: a `Weapon` is closure data on a resolver, `legal_actions` enumerates attacks **without consulting a weapon at all**, and there is no worn/held/carried distinction for p. 104's "any armor you are wearing" or p. 105's "a hand free" to read. **Thirteen unimplemented shapes** wait behind it. [0039](docs/decisions/0039-equipment-is-what-a-creature-holds-wears-and-carries.md) settles it the way [0026](docs/decisions/0026-terrain-enters-as-state.md) settled terrain and [0038](docs/decisions/0038-a-spell-is-data-the-caster-carries.md) settled spells: the thing a query needs in order to decide legality rides on the **state**, because the alternative lets a caller choose the outcome by choosing the argument. **The clause this would have got wrong is the free hand.** "Somatic needs a hand, Material needs a hand" reads as two, and p. 105 says one — "it can be the **same hand** used to perform Somatic components" — in a subordinate clause at the end of a paragraph about pouches. A model built from the summary would refuse spells the document permits and look careful doing it. **A weapon is equipment**, decided now and built later ([#258](https://github.com/eddiefiggie/srd-rules-engine/issues/258)), because deciding twice is what 0037 clause 3 just cost. **95 of 209**, unchanged: a record decides and builds nothing. 1451 tests.
...
_Last updated: 2026-08-28 — build `08282026.7`._
