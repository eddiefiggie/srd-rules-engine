...
**Current build:** `08252026.15` — **an Exhaustion level can finally be gained through a ruling, and the reason the three hazards still do not land has moved again.** [#178](https://github.com/eddiefiggie/srd-rules-engine/issues/178): `Conditions.exhaustion_level` was a field set at construction, with no `EffectKind`, no state mutator and no method able to move it — [#119](https://github.com/eddiefiggie/srd-rules-engine/issues/119)'s shape one field along, where a mechanical change reached state with no roll, no seed, no citation and no ledger entry. `EffectKind.EXHAUSTION_GAINED` and `EncounterState.with_exhaustion` close it. **Levels are cumulative** (p. 181), so it adds rather than sets — an engine that set the level would leave a creature exhausted three times no worse off than one exhausted once. **Six is death and seven is nothing**: a gain past 6 is *refused* rather than clamped, because clamping discards the caller's arithmetic and leaves a creature at 5 the rules have killed. **What blocks Suffocation now is removal, and it is a design question**: four rules remove levels and no two agree — a Long Rest removes one, breathing again removes every level *suffocation* caused, and dehydration's and malnutrition's are removable by nothing until the creature drinks or eats. Two of those turn on a level's **provenance**, and one integer cannot say which of a creature's levels are which ([#180](https://github.com/eddiefiggie/srd-rules-engine/issues/180), a `gate`). Building Suffocation with a suffocation-shaped counter would answer that in code rather than in a record, so it waits. Six clauses added (120 total), two proven red against a Long Rest that removes *all* levels and a death at 5. **Coverage stays at 78 of 211** — this built the mechanism three hazards need, and no hazard. 1119 tests.
...
_Last updated: 2026-08-25 — build `08252026.15`._
