...
**Current build:** `09122026.3` — **the last gate closes, and two of its three mismatches dissolved on contact with the tree.** [#434](https://github.com/eddiefiggie/srd-rules-engine/issues/434) asked whether p. 183’s Help is a second mechanism, on three things `PendingAdvantage` cannot do: an **unnamed holder** (*"by one of your allies"*), a **scope that is a skill rather than a target**, and **tool proficiencies nobody has modelled**. Checked one at a time, the first needs `EncounterState.are_allies`, which 0087 built — "one of your allies" resolves at spend time, which is where scope has *always* resolved, so the unassigned holder is the axis `against_id is None` already had, applied to the other end. **The third mismatch was a sentence read twice**: the gate recorded Help’s two expiries as "not the same", and both name the start of the helper’s next turn — one phrased as a condition on use, one as a moment, one boundary. What is left is a single widening, and it is **two closed vocabularies rather than two nullable fields** — a holder is *this creature* or *the allies of this creature*, a scope is *attacks against this creature*, *any attack*, or *ability checks with this skill*. Vex, Sap and Help’s two cases become four points in one space, and the combinations the document never states stop being representable rather than merely unwritten. **A count of mismatches is not a measure of distance until each one has been checked against the code.** The two cases are **reversed** — named holder with a skill scope against unnamed holder with a target scope — which is 0049’s four-axis reading one axis shallower, and an argument *for* one mechanism. Tools refuse with their reason ([#473](https://github.com/eddiefiggie/srd-rules-engine/issues/473)), because **a refusal is not an overstatement — it is the opposite move**, and `help` stays unclaimed until it comes off ([#471](https://github.com/eddiefiggie/srd-rules-engine/issues/471)). One citation was checked and found wrong on the way: `core.turn_span` attributes the vocabulary-over-boolean rule to 0019, which is about the inventory’s `kind` field and says nothing about booleans ([#472](https://github.com/eddiefiggie/srd-rules-engine/issues/472)). Coverage does not move. [0094](docs/decisions/0094-helps-two-cases-are-reversed-and-both-expire-at-the-same-boundary.md). **No `gate` issue is open for the first time since the label existed.** No corruption proofs: the record adds no code. 146 of 210, 14 clauses. 384 clauses verified against the document. 2476 tests.
...
| **SRD fidelity** | **Two open, both disclosed rather than wrong-and-quiet:** Temporary Hit Points absorb damage without reducing it, and two rules read that ([#413](https://github.com/eddiefiggie/srd-rules-engine/issues/413)); and nothing models being gagged or in magical silence, so p. 105's Verbal component is unenforceable ([#246](https://github.com/eddiefiggie/srd-rules-engine/issues/246)). A downed player character now makes death saves: p. 17 puts the save at the **start** of a turn, and the turn's start is a phase this engine has ([#124](https://github.com/eddiefiggie/srd-rules-engine/issues/124), [0027](docs/decisions/0027-occasions-and-outcomes-without-a-roll.md)). `MAX_SPELL_LEVEL`'s provenance closed with [#130](https://github.com/eddiefiggie/srd-rules-engine/issues/130). Falling's Prone when Resistance rounded the damage to zero was the last known-wrong behaviour, and it closed with [#173](https://github.com/eddiefiggie/srd-rules-engine/issues/173). |
...
**146 of 210 shapes resolve today.** The other 64 are listed, not omitted; run
...
is editorial and lives in that script where it can be reviewed. Twenty-three entries are
...
- [0083 — A condition carries the rule that caused it](docs/decisions/0083-a-condition-carries-the-rule-that-caused-it.md) — settles [#428](https://github.com/eddiefiggie/srd-rules-engine/issues/428)
- [0084 — A space is a control area, not a volume](docs/decisions/0084-a-space-is-a-control-area-not-a-volume.md) — settles [#337](https://github.com/eddiefiggie/srd-rules-engine/issues/337)
- [0085 — A target is a role in a mechanic, not a mechanic](docs/decisions/0085-a-target-is-a-role-in-a-mechanic-not-a-mechanic.md) — settles [#453](https://github.com/eddiefiggie/srd-rules-engine/issues/453)
- [0086 — A range runs from the edge of a space, as the grid measures it](docs/decisions/0086-a-range-runs-from-the-edge-of-a-space.md) — settles [#456](https://github.com/eddiefiggie/srd-rules-engine/issues/456)
- [0087 — A move is a path through spaces, and a turn ended in one is Prone](docs/decisions/0087-a-move-is-a-path-through-spaces-and-a-turn-ended-in-one-is-prone.md) — settles [#451](https://github.com/eddiefiggie/srd-rules-engine/issues/451)
- [0088 — A run of days without food is a hazard the creature carries](docs/decisions/0088-a-run-of-days-without-food-is-a-hazard-the-creature-carries.md) — settles [#401](https://github.com/eddiefiggie/srd-rules-engine/issues/401)
- [0089 — A lock lifts on the day the requirement is met, and closes on the next level](docs/decisions/0089-a-lock-lifts-on-the-day-the-requirement-is-met.md) — settles [#461](https://github.com/eddiefiggie/srd-rules-engine/issues/461)
...
- [0094 — Help's two cases are reversed, and both expire at the same boundary](docs/decisions/0094-helps-two-cases-are-reversed-and-both-expire-at-the-same-boundary.md) — settles [#434](https://github.com/eddiefiggie/srd-rules-engine/issues/434)
<!-- /record-index -->
...
Magic action's feature-and-item half, and [#471](https://github.com/eddiefiggie/srd-rules-engine/issues/471) — p. 183's Help, the
other gate settled this build, which widens `PendingAdvantage` into a holder and a scope vocabulary.
[#246](https://github.com/eddiefiggie/srd-rules-engine/issues/246) stays blocked on a
...
_Last updated: 2026-09-12 — build `09122026.3`._
