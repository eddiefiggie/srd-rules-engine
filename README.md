...
**Current build:** `08302026.26` — **a blocker closed and the code that waited on it did not notice.** `core.reactions` computes who an Opportunity Attack provokes (p. 185) and withheld every offer on the one clause it could not answer — *"a creature that you can see"* — naming [#150](https://github.com/eddiefiggie/srd-rules-engine/issues/150). **#150 closed on 2026-08-25**, and `core.sight` has answered ever since; it says so in its own docstring. The withholding stayed five days and four builds, and so did four prose claims in live source that sight was unanswerable, [0015](docs/decisions/0015-reactions-and-the-agent-seam.md)'s Status row citing the closed issue, and a test module asserting every offer is withheld — so the suite **defended** the staleness rather than catching it. `provocations` now consults `can_see` per reactor and `Visibility`'s three verdicts are three results: `CAN_SEE` offers, `CANNOT_SEE` yields **no provocation at all** (p. 185 grants the attack *when* you can see, so a reactor who cannot was never owed one), and `UNSTATED` is withheld — the document states no answer for that pair, and "the SRD does not say" must not become a no. The withholding therefore narrows to a pair rather than disappearing: an encounter that states no light is exactly that case, because `Lighting.ambient` of `None` means nobody has said. `withheld` loses its default, since a computed answer must not have a silent one. **The disclosure is renamed for what is actually missing** — `opportunity-attack-detected-but-never-offered`, not the sight qualifier: the third time in six builds a disclosure has been wrong about *why* ([0056](docs/decisions/0056-a-move-is-refused-where-it-is-made.md), [0060](docs/decisions/0060-a-disclosure-can-be-wrong-about-why.md)), and the first that was **correct when written** and made wrong by another change landing. No guard caught it and none is proposed: 0015's row said *Built*, so `check_status_rows.py` read the claim, found it did not say `not built`, and passed — a third direction that reads as a deliberate limit rather than as an absence ([#381](https://github.com/eddiefiggie/srd-rules-engine/issues/381), [0071](docs/decisions/0071-a-blocker-that-closed-and-a-disclosure-that-did-not-notice.md)). The offer itself is still unbuilt, filed as [#382](https://github.com/eddiefiggie/srd-rules-engine/issues/382) rather than left to #381's closure, and is now *provable* — which 0015 said it was not. Six corruption proofs, one per assertion, because a base tree that cannot import the module proves nothing about any of them. 117 of 210, 16 clauses. 282 clauses verified against the document. 2062 tests.
...
**117 of 210 shapes resolve today.** The other 93 are listed, not omitted; run
...
_Last updated: 2026-08-30 — build `08302026.26`._
