...
**Current build:** `08302026.16` — **the menu is not a promise.** [#245](https://github.com/eddiefiggie/srd-rules-engine/issues/245), a slice of [#19](https://github.com/eddiefiggie/srd-rules-engine/issues/19)'s spellcasting umbrella. p. 105's *"If the spellcaster can't provide one or more of a spell's components, the spellcaster can't cast the spell"* has been computed by `component_refusal` since #257 — Somatic's hand, Material's **free** hand, the one hand that serves both, the Pouch and the Focus. **It had exactly one caller**: `legal_actions`. So the rule was enforced by the menu and by nothing else, and a caller reaching adjudication without consulting the read surface cast the spell anyway ([0062](docs/decisions/0062-the-menu-is-not-a-promise.md)). **p. 104's preparation had the identical shape, one screen away in the same function** — `legal_actions` has checked it since #249 and `spell_resolver` never did. Found by reading the one while fixing the other. **That is four instances in one session**, after p. 90's Push bound, p. 182's escape DC and p. 186's righting cost — a rule computed once, consumed at the read surface and absent from the adjudication path, **every one found by a person** writing a test or reading a neighbour. Nothing mechanical looks for it, and a guard is harder than #334's or #356's because the four look nothing alike — a helper returning a string, a set membership, a bound on a declared number, a state precondition. Filed as [#365](https://github.com/eddiefiggie/srd-rules-engine/issues/365) rather than attempted at the end of a change about spells. The offer keeps its check: R18 asks for legality to be *computable* rather than checkable afterwards, so the resolver's refusal is the floor under the menu and not a replacement for it. **No figure moves — 116 of 210, 17 clauses** — because `spellcasting-focus` was already claimed and correctly, the shape did resolve. A rule enforced in one of the two places it belongs is the third kind of gap the instruments cannot see, after 0058's unread field and 0061's unenforced clause. 278 clauses verified against the document. 4 corruptions, each red on its own test. 1959 tests.
...
_Last updated: 2026-08-30 — build `08302026.16`._
