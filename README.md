...
**Current build:** `08252026.35` — **terrain already reaches a d20 roll, and the replay gap it was filed against never opened.** [#159](https://github.com/eddiefiggie/srd-rules-engine/issues/159) was filed as a prohibition that would become work *"the moment anything reads terrain into a roll"*, and said nothing did yet. **That stopped being true without anyone noticing**: #192 and #193 wired `can_see` into `attack_resolver`, and `can_see` reads the encounter's obstructions and its lighting — so **a wall now decides whether p. 182's Frightened qualifier holds**, which decides whether the attack has Disadvantage, which decides **how many dice are rolled and which one is used**. Held everything constant but the wall, at one seed: open ground rolls `[17, 10]` and takes the **10**; behind the wall rolls `[17]` and takes the **17**. That is precisely the difference `REPLAYABLE_FROM = 2` exists to prevent. **The gap did not open, and not by luck.** #159 asked that the entry record the *derived* value rather than the terrain, and it structurally cannot do otherwise: the ruling records `declared_disadvantage`, and **`replay_entry` takes no `EncounterState` at all**, so nothing hands it the terrain to record. Both rulings replay from the entry alone. **No `REPLAYABLE_FROM` bump is needed** — the v2 payload already carried the derived flags, which is the whole reason it exists. Closed as already-correct with the evidence rather than left open looking untouched, and pinned by a test that goes red when the wiring is cut (proved through `prove_guard_red.sh`). The wall is placed between the attacker and its **source of fear**, not its target — on the line to the target it gives Total Cover and p. 179 refuses the attack outright, so there would be no roll to replay. Coverage unchanged at **91 of 211**. 1278 tests.
...
_Last updated: 2026-08-25 — build `08252026.35`._
