...
**Current build:** `08302026.13` — **initiative draws a pair for everyone.** [#359](https://github.com/eddiefiggie/srd-rules-engine/issues/359): p. 184 gives Incapacitated **Disadvantage** on Initiative and Invisible **Advantage**, and `initiative_order` rolled one d20 per combatant and consulted neither. Disclosed rather than built by 0058, because Advantage means **two dice** and one die per combatant is not a forgotten modifier — it is the seed layout. **The option that arrives by accident is drawing the pair only where it is needed**, which makes a combatant's offset depend on the *conditions of the combatants before it*: reproducible, and fragile in exactly the way [#82](https://github.com/eddiefiggie/srd-rules-engine/issues/82) was, where a run walked out of its band and silently agreed with another band's dice. So the layout is uniform and the second die is drawn and unused where nothing modifies the roll ([0059](docs/decisions/0059-initiative-draws-a-pair-for-everyone.md)). The pick goes through `core.d20.pick`, public now, because two copies of *higher for Advantage, lower for Disadvantage* would be two chances to answer it differently. **Every initiative result moved, and the fixtures said so in the words written for it**: `opening_state` refused a seed by name — *"a literal stops meaning what it says the moment the derivation moves"* — and the layout assertion compared against `faces[1]` where the second creature now reads `faces[2]`. Nothing records initiative in the ledger, so no history was rewritten. **A seed that cannot discriminate makes a test pass for the wrong reason**: the first draft used one seed for both directions and could not tell Disadvantage from no condition at all, since seed 7's pair is (11, 13) and `min` of that is the die a creature holding nothing would have taken. Each direction now uses a seed whose pair runs the right way and asserts the discriminating property first. One test **states a property rather than guarding an implementation** and says so — no one-line corruption violates it, and its neighbour is what actually holds the layout. Both disclosures retire, so six conditions still disclose something, down from eight. 116 of 210, unchanged — the fourth record to say so, and #356 now holds the general point. 278 clauses verified against the document. 6 corruptions, each red on its own test. 1951 tests.
...
_Last updated: 2026-08-30 — build `08302026.13`._
