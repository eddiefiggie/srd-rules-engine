...
**Current build:** `08302026.35` — **p. 177's third drawback, and it never needed what it said it needed.** [#367](https://github.com/eddiefiggie/srd-rules-engine/issues/367) recorded the Shield clause as blocked on `Item.is_armour` not distinguishing a Shield from worn armour, and on that distinction requiring p. 177's armour *category* — content this repository does not ship. That framing survived four builds and both halves were wrong: **carriage** distinguishes them (armour is worn, a Shield is held, which is what 0039 clause 3 made a closed vocabulary for) and **training is by item id**, which 0040 clause 2 settled precisely so the category would not be needed. What was actually missing was a derivation to withhold from — nothing can be taken out of a stored total whose parts are unknown — and [#393](https://github.com/eddiefiggie/srd-rules-engine/issues/393) supplied it. So the rule is one condition over a relation that already existed, and `untrained-shield-still-grants-ac` retires in the change that builds it. **p. 177 now discloses nothing**, its casting prohibition (0063) and Disadvantage (0064) having landed earlier. The one-Shield refusal is asked *before* the training filter, because p. 92's "wield only one Shield at a time" is about wielding — filtering first would let an untrained Shield hide a second one. **And two shipped rules were resting on a page nobody had read**: the verifier carried no armour-training clause at all, for either built drawback. Third time in three workloads, and the pattern is specific enough to name — the rules most likely to be unasserted are the ones that were *easy to implement*, because a hard mechanic sends somebody to the page and a one-sentence drawback gets built from the sentence somebody already quoted. Three corruption proofs. [0078](docs/decisions/0078-the-last-armour-training-drawback.md). 118 of 210, **14 clauses** — the figure that improves by going down. 297 clauses verified against the document. 2135 tests.
...
_Last updated: 2026-08-30 — build `08302026.35`._
