...
**Current build:** `08232026.28` — **decision `0018` settles the API stability policy, closing #39.** The fact that decided its shape: `core` re-exports **110 names**, most of them present only because R34 makes outer layers use re-exports rather than submodules. Promising stability across all of them would be promising something nobody could keep — so there are three tiers. **Committed** is *enumerated by name* in `srd_rules_engine.stability` and pinned by a test, because a policy in prose degrades the first time somebody renames a symbol. **Provisional** covers the MCP tool names, six names old and expected to move; the `Session` beneath them is committed, so a consumer wanting stability builds there. `API_VERSION` is a monotonic integer, independent of the build stamp and of the data schemas — `RULING_VERSION` is already 2 while the API is 1, which is that independence visible in the data.
