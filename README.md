...
**Current build:** `08252026.14` — **78 of 211: Burning fires, and the reason the other three hazards do not turns out not to be the one [#140](https://github.com/eddiefiggie/srd-rules-engine/issues/140) expected.** [0027](docs/decisions/0027-occasions-and-outcomes-without-a-roll.md) clause 5 is built — `Hazards` on `Combatant`, **not** on `Conditions`, because "this creature is Burning" is not one of the fifteen and filing it there would make 15/15 a different number. It lives in `core.state` rather than `core.hazards` for the reason `DeathSaves` does: that module imports `core.adjudicate`, which imports this one. Burning deals 1d4 Fire **at the start of each of its turns** (p. 178) — the same phase the death save fires in, and the obligation order is death save first, because p. 17's third failure is a death and resolving a fire against a creature whose death this turn is unsettled resolves it against nobody. **The blocker on the other three is not the occasion.** Suffocation has had one since 0023 built the turn's end. All three inflict an Exhaustion **level**, and nothing can raise one through a ruling — no `EffectKind`, no state mutator, no method on `Conditions` ([#178](https://github.com/eddiefiggie/srd-rules-engine/issues/178)). #140 reasoned they were cheap partly because "Exhaustion is 15/15 implemented"; the *condition* is, and raising a *level* is not, and the inventory cannot show that difference. Burning escaped because it deals damage, which has worked since #88. What puts a fire out is disclosed rather than modelled: doused, submerged and suffocated are facts this engine cannot observe, and the fourth way is an action nothing can spend — so a burning creature burns until a caller clears the flag, and the ruling's bounds refuse to claim otherwise. 1110 tests.
...
_Last updated: 2026-08-25 — build `08252026.14`._
