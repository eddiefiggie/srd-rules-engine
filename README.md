...
**Current build:** `08252026.1` — **both of this repository's verification proofs prescribed a procedure that does not work here, and they now live in scripts that do.** The pre-change-tree proof was defeated by the editable install: a `.pth` in `site-packages` points at the working tree's `src`, so `pytest` run inside an exported base tree imports the *working tree*, the new code is already there, and the run reports a pass for the wrong reason. It produced a false green three times — [#123](https://github.com/eddiefiggie/srd-rules-engine/issues/123), [#136](https://github.com/eddiefiggie/srd-rules-engine/issues/136), and [#153](https://github.com/eddiefiggie/srd-rules-engine/issues/153), the last of them after reading the instruction file ([#154](https://github.com/eddiefiggie/srd-rules-engine/issues/154)). `scripts/prove_against_base.sh` sets `PYTHONPATH` to the export and then **asserts `srd_rules_engine.__file__` resolves inside it**, aborting rather than reporting when it does not — the only step that can tell a real red from a false green. The corruption proof said "then restore" as though restoring were the trivial part; the reflex, `git checkout -- <file>`, is wrong in both states such a proof runs in — it reverts the whole file and discards uncommitted work, or fails on an untracked file and **leaves the corruption in the tree** ([#155](https://github.com/eddiefiggie/srd-rules-engine/issues/155)). `scripts/prove_guard_red.sh` snapshots, corrupts, runs, and restores from a trap on every exit path including interrupt. **Both failure modes are reproduced and both fixes are demonstrated in the PR, not asserted** — including the abort firing when `PYTHONPATH` is stripped. No engine behaviour changed and coverage stays at 76 of 211; this is the machinery that decides whether the next change's evidence means anything. 1036 tests.
...
_Last updated: 2026-08-25 — build `08252026.1`._
