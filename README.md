...
**Current build:** `08232026.22` — **the action economy, and the reactions question answered rather than deferred.** One action, one Bonus Action *only if a rule grants one*, and one Reaction that is free of both and refreshes at **the start of your next turn** — not the end of the round, which is the version usually played by mistake and which would hand a creature two Reactions in quick succession. Dash, Dodge and Disengage resolve; Dodge is re-checked rather than trusted, because a creature grappled after Dodging loses the benefit while the flag stays set. Decision `0015` records that reactions need **no redesign**: the turn loop is already a generator yielding typed requests carrying their own `actor_id`, so out-of-turn invocation is an operation it already performs. What remains is trigger detection and a ledger-interleaving rule. Inventory coverage 66 → 71 of 211.
