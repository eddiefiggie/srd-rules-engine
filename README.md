...
**Current build:** `08252026.16` — **0028: an Exhaustion level carries the rule that caused it, because four of the five removal rules turn on which level it is.** [#180](https://github.com/eddiefiggie/srd-rules-engine/issues/180) framed this over three sources and four rules. **The document has seven sources and five removal shapes**, and the two the issue did not know about are the two that decide the design: p. 194's contagion **suppresses a Long Rest entirely** rather than locking its own levels, and a Potion of Vitality (p. 236) removes **any** level — which pp. 181 and 185 say cannot happen. So a level now carries the **rule id** that caused it, not a source enum: seven sources appear across three parts of the document, nothing suggests that is all of them, and a closed set in the data is a branch in every consumer ([0019](docs/decisions/0019-kind-is-a-filing-label.md)). It is the same shape [0027](docs/decisions/0027-occasions-and-outcomes-without-a-roll.md) clause 2 reached for obligations. **Removal is a rule, not a subtraction**, and a locked level is *invisible* to the general rule rather than subtracted from it — removing one and re-applying the lock reports the same total by a route that is wrong for the next rule to read. p. 181 never says *which* level a Long Rest takes, so the engine declares most-recently-gained-first as a convention and says it is one. **Two printed rules contradict each other** for the first time in this project — the potion against the dehydration lock — and that is filed ([#182](https://github.com/eddiefiggie/srd-rules-engine/issues/182)) rather than resolved, because picking a winner would infer a rule value. **Decided, not built** ([#183](https://github.com/eddiefiggie/srd-rules-engine/issues/183)): nothing finishes a Long Rest, so the general rule has nowhere to attach yet. Two clauses added, 122 total. Coverage stays at 78 of 211. 1119 tests.
...
**78 of 211 shapes resolve today.** The other 133 are listed, not omitted; run
...
_Last updated: 2026-08-25 — build `08252026.16`._
