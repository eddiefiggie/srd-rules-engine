...
**Current build:** `08302026.5` — **the exit is built before the entrance.** [#335](https://github.com/eddiefiggie/srd-rules-engine/issues/335) asked for p. 190's Grapple and Shove, and reading p. 182 first changed what the first change should be: *"**However a grapple is initiated, it follows these rules**"* — so the rules for getting out are common to p. 190's Unarmed Strike and to every stat block that grapples, and they are one shape in the inventory rather than part of another. The Grappled condition was already two-thirds built and there was **no way out of it**: no escape DC, no check, no release, and no answer to either ending p. 182 states. Those failure modes are not comparable — an engine that cannot start a grapple declines a rule, and one that cannot end a grapple ends the session ([0052](docs/decisions/0052-the-exit-is-built-before-the-entrance.md)). **Four endings, three mechanisms.** The escape check is an adjudication; the grappler releasing is a testless proposal that rolls nothing and costs nothing; and *Incapacitated grappler* and *distance exceeds range* are **derived** — nothing decides them, so they carry no rule id and are applied wherever state settles rather than on a turn boundary. That is where this parts company with 0050: Slow **expires** at a boundary, while a creature whose grappler drops unconscious is freed at that moment, and a sweep alone would hold it by a rule the document had already stopped applying. **The escape DC is stored, never recomputed** — a stat block states it outright (*"escape DC 13"*, p. 259) and p. 190 derives it from the grappler at the moment of the grapple, so recomputing drifts for one and ignores the other. Each `None` **refuses and leaves the condition held**: lifting a grapple against a bound the engine invented removes a condition the rules did not. **Which check is the creature's choice and needed no new seam**, because the escape is an action it *declares* — two offers, each carrying its bonus, since two entries an agent cannot tell apart are not a choice. Two clauses ship disclosed: *Movable* ([#340](https://github.com/eddiefiggie/srd-rules-engine/issues/340)) and the release offered only on the grappler's turn where p. 182 says *at any time* ([#341](https://github.com/eddiefiggie/srd-rules-engine/issues/341)) — narrowed in the safe direction, since routing it through Reactions would charge a Reaction the document does not. A corruption proof found a **negative case that asserted nothing**: it checked that the victim was not offered a key naming a creature nobody was grappling, which no implementation could have produced. **113 of 210.** p. 190's three options remain the rest of #335, blocked now on precisely one thing — the target of a forced save chooses between two abilities and declares nothing. 252 clauses verified against the document. 13 corruptions, each red on its own test. 1807 tests.
...
**113 of 210 shapes resolve today.** The other 97 are listed, not omitted; run
...
_Last updated: 2026-08-30 — build `08302026.5`._
