...
**Current build:** `08302026.29` — **a Ritual took no time at all, and the clause saying it should was asserted nowhere.** p. 105 names rituals in the sentence that introduces longer casting times — *"Certain spells, including a spell cast as a Ritual, require more time to cast"* — and [#250](https://github.com/eddiefiggie/srd-rules-engine/issues/250) built that machinery for spells whose `CastingTime` is `MINUTES` only. `ritual_cast` computed p. 187's ten extra minutes and **had no caller anywhere in the engine**, so a caller could ritual a spell and take zero turns over it. `Spell` now carries the Ritual tag, the read surface offers `ritual:<spell>` — carrying no slot level, because p. 187 draws that consequence itself — and a Ritual begins a `LongCast` whose `slot_level` is `None` rather than zero, so the completion spends nothing. **Three things this found that the issue did not name.** First, the licensing sentence was **verified nowhere**: `core.casting` quoted it in a docstring to explain the gap while `verify_d20_rules.py` had never read it, so the change rests on a clause asserted for the first time here (283 now). Second, the `ritual` shape was already claimed **implemented** over that callerless function — the overstatement R17's inventory exists to prevent, and the same finding as [#381](https://github.com/eddiefiggie/srd-rules-engine/issues/381)'s except honestly withheld there and asserted here; the coverage figure does not move, so this build made a published number *true* rather than larger. Third, and worst: **every long casting was one turn too long.** `LongCast.turns_remaining` counts the actions owed "this turn's included" and the opening adjudication charged one *and* stored the full count, so a one-minute casting cost eleven Magic actions and a ten-minute ritual a hundred and one. `test_long_casting.py` asserted the wrong number from the day it shipped, so the suite **encoded** the defect rather than catching it; it was found by counting a ritual's actions end to end. Fixed in both paths, with the three assertions corrected and saying why. A one-turn error is six seconds, which is why nothing noticed. Eight corruption proofs. [0074](docs/decisions/0074-a-ritual-is-a-long-casting.md). 118 of 210, 15 clauses. 283 clauses verified against the document. 2101 tests.
...
_Last updated: 2026-08-30 — build `08302026.29`._
