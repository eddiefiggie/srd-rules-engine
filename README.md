...
**Current build:** `08302026.6` — **the target chooses, and the engine rolls.** [#343](https://github.com/eddiefiggie/srd-rules-engine/issues/343), the gate over p. 190's *"a Strength or Dexterity saving throw **(it chooses which)**"* — two hits in the whole document, both on p. 190, the same sentence twice. One record ago the identical shape cost nothing: p. 182's escape check offers Athletics or Acrobatics, and because the escaping creature *declares* its action the choice is simply which key it declares (0052 clause 5). p. 190 is the mirror image — the chooser is the **target of a forced save** and declares nothing, and `_obligation_declaration` says why nobody is asked: *"no read surface offered this, **because nothing was choosing**"*. True for Concentration and Topple, false for these two. **The engine may not pick the better modifier, and the reason is sharper than 'that is strategy'**: p. 187's Restrained gives Disadvantage on **Dexterity** saves alone, so a Restrained creature with better Dexterity should still choose Strength — the right answer depends on conditions held at the moment of the save. And the engine could not compute it anyway: `dexterity_saves` and `auto_fail_strength_and_dexterity_saves` are modelled in data and reach **none** of the six save sites ([#344](https://github.com/eddiefiggie/srd-rules-engine/issues/344), found writing this). So `ForcedSave` gains `ability_choices`, a save with one is **unsettled** and no resolver may roll it, and `SaveAbilityRequest` joins declaration, narration and blocked facts as the fourth typed request — addressed to the target, carrying each ability's modifier ([0053](docs/decisions/0053-the-target-chooses-and-the-engine-rolls.md)). **Declining is a refusal, not a default**: the save goes unresolved, because every fallback is the rejected option through the back door. **This change had to find a bug 0052 shipped green**: `with_condition` rebuilds a creature's `Conditions` field by field and the new `grapple` was not among them, so applying any condition to a grappled creature **erased its escape DC** — and the ordinary case that reaches it is a Shove knocking a grappled creature Prone, which is exactly what this build adds. Grapple and Shove-to-Prone now ship; the push half does not, because *"push it 5 feet away"* is forced movement relative to another creature, the primitive Frightened and the Push mastery also wait on ([#345](https://github.com/eddiefiggie/srd-rules-engine/issues/345)). A grapple made here carries p. 190's escape DC and **no range**: p. 190 states the reach of the *strike*, not the range of the grapple ([#346](https://github.com/eddiefiggie/srd-rules-engine/issues/346)). **114 of 210**, masteries 7 of 8. 260 clauses verified against the document. 15 corruptions, each red on its own test. 1827 tests.
...
_Last updated: 2026-08-30 — build `08302026.6`._
