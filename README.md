...
**Current build:** `08312026.22` — **a fourth shape was waiting on [#337](https://github.com/eddiefiggie/srd-rules-engine/issues/337) and nothing said so.** p. 190's Teleportation splits cleanly. Two of its mechanics are **behavioural contrasts** the engine can express today — a teleport *"doesn't expend movement"* where a move spends a speed, and *"never provokes Opportunity Attacks"* where a move offers them — and asserting either as an absence would be vacuous, since a method that simply never calls the provocation path passes trivially. Its **destination rule is blocked**: *"if the destination space is occupied by another creature... you appear in the nearest unoccupied space"* is twice a question about **extent**, and #337 is that every creature in `core.position` is still a point. A point-equality version would be the rule at half, in the direction that lets two creatures share a square. **#337 listed three shapes waiting on it and this was not among them** — `occupied-space`, `unoccupied-space`, `target`, and now `teleportation`; the cross-link is filed. Touching another creature is narrative and the default is the useful half. Teleportation is *"magical"* and there are no spells ([#21](https://github.com/eddiefiggie/srd-rules-engine/issues/21)), so nothing can cause one — the transition would exist for a ruleset to call, which is where `with_temporary_hit_points` already stands. [#444](https://github.com/eddiefiggie/srd-rules-engine/issues/444). **No shape moves**; the reading and the missing dependency are the deliverable. 139 of 210, 14 clauses. 355 clauses verified against the document, up from 300 this morning. 2296 tests.
...
_Last updated: 2026-08-31 — build `08312026.22`._
