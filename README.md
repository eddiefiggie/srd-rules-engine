...
**Current build:** `08302026.28` — **the report filed a reaction as a turn of its own.** A `Turn` in `session_report` is a *declaration slot*, and two things open one inside another creature's turn: an engine-authored obligation ([0023](docs/decisions/0023-the-turns-end-is-a-loop-owned-phase.md)) and a reaction ([0072](docs/decisions/0072-movement-is-a-phase-the-loop-drives.md)). `_turns` closed a slot when a different actor declared, so an Opportunity Attack made mid-move ended the mover's turn **in the report**. [0015](docs/decisions/0015-reactions-and-the-agent-seam.md) predicted this on 2026-08-23, deferred it as *"the piece that will bite"*, and the seam question then closed without it being filed ([#120](https://github.com/eddiefiggie/srd-rules-engine/issues/120)). #120 asked first whether an entry records enough to tell "inside turn N" from "at the end of turn N": **it does not, and the adjudicator held the answer and was not writing it down** — `state.active_id` and `round_number` are in hand when the declaration entry is appended. `DECLARATION_VERSION` 3 records both, `SessionReport.game_turns` groups by consecutive runs of them, and `render` publishes game turns and declaration slots as two counts because a reader given only the second reads it as the first. **Recorded, never reconstructed**: inferring the turn from actor changes is wrong in exactly the case the field exists for, since a reaction *is* an actor change inside a turn — so an older ledger reports `attributable=False` rather than a guess. The sharpest clause is that **presence decides that flag, not truthiness**: `during=None` present means the engine says there was no turn, an absent key means this ledger cannot say, and only the second is a limit. **That clause's corruption proof came back green on its first run** — the legacy fixture has the field absent, so both readings agreed there and nothing tested the distinction; the out-of-combat test exists because the proof failed, which is `AGENTS.md`'s vacuous-under-its-own-fixture shape caught in the act. `NOT_MEASURED` also said *"`Turn.improvised` tells them apart today"*, and that half was wrong: a reaction is the agent's own declaration and arrives `improvised=False`, so the advice would have caught one source of interleaving and missed the other silently. Seven corruption proofs. [0073](docs/decisions/0073-a-slot-records-the-turn-it-happened-in.md). 118 of 210, 15 clauses. 282 clauses verified against the document. 2084 tests.
...
_Last updated: 2026-08-30 — build `08302026.28`._
