...
**Current build:** `08252026.36` — **the seven rules that depend on damage already dealt are three shapes, and sorting them wrongly would have put a defence outside the one place defences are applied.** [0032](docs/decisions/0032-an-outcome-conditional-on-its-own-damage.md) answers the question [#173](https://github.com/eddiefiggie/srd-rules-engine/issues/173) left open — *is this shape worth building for one 1-in-6 case on one hazard?* The document was swept: **five** rules want a predicate over settled damage (pp. 182, 18, 124, 314), **one** wants a derived *magnitude* (p. 311), and **one is not this shape at all** — p. 180's Damage Threshold says *"has **Immunity** to all damage unless..."*, which places it beside p. 17 ([#214](https://github.com/eddiefiggie/srd-rules-engine/issues/214)); p. 179's Concentration DC needs a **further test**, not an effect ([#215](https://github.com/eddiefiggie/srd-rules-engine/issues/215)). An earlier note on #173 said "seven places" without separating them; corrected here. **Clause 2 is the whole of #173 and it is one word — *taken*.** There are three moments in `adjudicate`: the resolver has no number, `_roll_declared` has the **rolled** number, and `_apply` has the **taken** one. The middle moment is the trap — it would look right and fail in exactly the case #173 is about, because **Resistance is the entire difference between rolled and taken**. `_apply` already computes it per effect, for a different reason ([#105](https://github.com/eddiefiggie/srd-rules-engine/issues/105)). The predicate is **data rather than a callable**, because a callable cannot be recorded and a withheld Prone that leaves no trace is indistinguishable from one nobody considered (R5). **Clause 5 is the one a reviewer would miss**: `_bounds` is built from the proposal *before* `_apply` runs, so making an effect conditional silently makes a *claim* wrong. About twenty near-misses — Sleep, Frightful Presence, the poisons — are **duration terminators** and counting them would have built the wrong thing. Five clauses added (158 total), two proved red at the plausible-wrong value. **#173 stays open**: this decides how to fix it, not that it is fixed. Coverage unchanged at **91 of 211**. 1278 tests.
...
**91 of 211 shapes resolve today.** The other 120 are listed, not omitted; run
...
_Last updated: 2026-08-25 — build `08252026.36`._
