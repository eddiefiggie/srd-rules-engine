#!/usr/bin/env python3
"""Fail when a decision record's Status table cites a closed issue for unbuilt work (#291).

    scripts/check_status_rows.py

Exits 0 when every `not built` row in `docs/decisions/` cites only open issues and every
issue number cited anywhere in a Status table exists. Exits non-zero with the offending rows
named otherwise.
...
met by hand three times in one day (#277/#278, the 0039 and 0026 audit, and #263).
"""
...
def main() -> int:
    rows: tuple[StatusRow, ...] = all_rows(DECISIONS)
    if not rows:
    ...
    unbuilt = sum(row.unbuilt for row in rows)
    print(
        f"  ok  {len(rows)} Status rows across {len({r.record for r in rows})} records; "
        f"{unbuilt} claim unbuilt work and every issue they cite is open"
    )
    return 0
